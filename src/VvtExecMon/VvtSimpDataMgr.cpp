...
bool VvtSimpDataMgr::initStore(VvTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
bool VvtSimpDataMgr::init(VvTSVariant* cfg, VvtExecRunner* runner)
{
	_runner = runner;
	return initStore(cfg->get("store"));
}

void VvtSimpDataMgr::on_all_bar_updated(uint32_t updateTime)
{

}

IBaseDataMgr* VvtSimpDataMgr::get_basedata_mgr()
{
	return _runner->get_bd_mgr();
}

IHotMgr* VvtSimpDataMgr::get_hot_mgr()
{
	return _runner->get_hot_mgr();
}

uint32_t VvtSimpDataMgr::get_date()
{
	return _cur_date;
}

uint32_t VvtSimpDataMgr::get_min_time()
{
	return _cur_min_time;
}

uint32_t VvtSimpDataMgr::get_secs()
{
	return _cur_secs;
}

void VvtSimpDataMgr::reader_log(VvTSLogLevel ll, const char* message)
{
	VvTSLogger::log_raw(ll, message);
}

void VvtSimpDataMgr::on_bar(const char* code, VvTSKlinePeriod period, VvTSBarStruct* newBar)
{

}

void VvtSimpDataMgr::handle_push_quote(const char* stdCode, VvTSTickData* curTick)
{
	if (curTick == NULL)
		return;
...
VvTSTickData* VvtSimpDataMgr::grab_last_tick(const char* code)
{
	if (_rt_tick_map == NULL)
		return NULL;
...
VvTSTickSlice* VvtSimpDataMgr::get_tick_slice(const char* code, uint32_t count, uint64_t etime /*= 0*/)
{
	if (_reader == NULL)
		return NULL;
...
VvTSKlineSlice* VvtSimpDataMgr::get_kline_slice(const char* stdCode, VvTSKlinePeriod period, uint32_t times, uint32_t count, uint64_t etime /*= 0*/)
{
	if (_reader == NULL)
		return NULL;
