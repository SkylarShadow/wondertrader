...
bool VvtDataManager::initStore(VvTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
bool VvtDataManager::init(VvTSVariant* cfg, VvtDtRunner* runner)
{
	_runner = runner;
	if (_runner)
	{
...
void VvtDataManager::reader_log(VvTSLogLevel ll, const char* message)
{
	VvTSLogger::log_raw(ll, message);
}

VvTSTickSlice* VvtDataManager::get_tick_slices_by_range(const char* stdCode,uint64_t stime, uint64_t etime /* = 0 */)
{
	stime = stime * 100000;
	etime = etime * 100000;
...
VvTSTickSlice* VvtDataManager::get_tick_slice_by_date(const char* stdCode, uint32_t uDate /* = 0 */)
{
	return _reader->readTickSliceByDate(stdCode, uDate);
}

VvTSOrdQueSlice* VvtDataManager::get_order_queue_slice(const char* stdCode,uint64_t stime, uint64_t etime /* = 0 */)
{
	stime = stime * 100000;
	etime = etime * 100000;
...
VvTSOrdDtlSlice* VvtDataManager::get_order_detail_slice(const char* stdCode,uint64_t stime, uint64_t etime /* = 0 */)
{
	stime = stime * 100000;
	etime = etime * 100000;
...
VvTSTransSlice* VvtDataManager::get_transaction_slice(const char* stdCode,uint64_t stime, uint64_t etime /* = 0 */)
{
	stime = stime * 100000;
	etime = etime * 100000;
...
VvTSSessionInfo* VvtDataManager::get_session_info(const char* sid, bool isCode /* = false */)
{
	if (!isCode)
...
VvTSKlineSlice* VvtDataManager::get_skline_slice_by_date(const char* stdCode, uint32_t secs, uint32_t uDate /* = 0 */)
{
	...
	{
		//第一次将全部数据缓存到内存中
		VvTSTickSlice* ticks = _reader->readTickSliceByDate(stdCode, uDate);
		if (ticks != NULL)
		{
			VvTSKlineData* kData = g_dataFact.extractKlineData(ticks, secs, sInfo, true);
	...
	VvTSBarStruct* rtHead = barCache._bars->at(0);
	VvTSKlineSlice* slice = VvTSKlineSlice::create(stdCode, KP_Tick, secs, rtHead, barCache._bars->size());
	return slice;
}

VvTSKlineSlice* VvtDataManager::get_kline_slice_by_date(const char* stdCode, VvTSKlinePeriod period, uint32_t times, uint32_t uDate /* = 0 */)
{
...
VvTSKlineSlice* VvtDataManager::get_kline_slice_by_range(const char* stdCode, VvTSKlinePeriod period, uint32_t times,uint64_t stime, uint64_t etime /* = 0 */)
{
	if (times == 1)
	{
	...
	if(barCache._bars == NULL)
	{
		...
		if (rawData != NULL)
		{
			VvTSKlineData* kData = g_dataFact.extractKlineData(rawData, period, times, sInfo, false);
			barCache._bars = kData;
	...
	uint32_t eIdx, sIdx;
	auto& bars = barCache._bars->getDataRef();
	auto eit = std::lower_bound(bars.begin(), bars.end(), eBar, [isDay](const VvTSBarStruct& a, const VvTSBarStruct& b) {
		if (isDay)
	...
	auto sit = std::lower_bound(bars.begin(), eit, sBar, [isDay](const VvTSBarStruct& a, const VvTSBarStruct& b) {
		if (isDay)
			return a.date < b.date;
		else
			return a.time < b.time;
	...
	VvTSBarStruct* rtHead = barCache._bars->at(sIdx);
	VvTSKlineSlice* slice = VvTSKlineSlice::create(stdCode, period, times, rtHead, rtCnt);
	return slice;
}

VvTSKlineSlice* VvtDataManager::get_kline_slice_by_count(const char* stdCode, VvTSKlinePeriod period, uint32_t times, uint32_t count, uint64_t etime /* = 0 */)
{
	if (times == 1)
	{
...
VvTSTickSlice* VvtDataManager::get_tick_slice_by_count(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	etime = etime * 100000;
	return _reader->readTickSliceByCount(stdCode, count, etime);
}

double VvtDataManager::get_exright_factor(const char* stdCode, VvTSCommodityInfo* commInfo /* = NULL */)
{
	if (commInfo == NULL)
		return 1.0;
...
void VvtDataManager::subscribe_bar(const char* stdCode, VvTSKlinePeriod period, uint32_t times)
{
	...
	if (times == 1)
	{
		VvTSKlineSlice* slice = _reader->readKlineSliceByCount(stdCode, period, 10, etime);
		if (slice == NULL)
			return;

		VvTSKlineData* kline = VvTSKlineData::create(stdCode, slice->size());
		kline->setPeriod(period);
		uint32_t offset = 0;
		for(uint32_t blkIdx = 0; blkIdx < slice->get_block_counts(); blkIdx++)
		{
			memcpy(kline->getDataRef().data() + offset, slice->get_block_addr(blkIdx), sizeof(VvTSBarStruct)*slice->get_block_size(blkIdx));
	...
	else
	{
		//只有非基础周期的会进到下面的步骤
		VvTSSessionInfo* sInfo = get_session_info(stdCode, true);
		VvTSKlineSlice* rawData = _reader->readKlineSliceByCount(stdCode, period, 10*times, 0);
		if (rawData != NULL)
		{
			VvTSKlineData* kData = g_dataFact.extractKlineData(rawData, period, times, sInfo, true);
			{
...
void VvtDataManager::clear_subbed_bars()
{
	StdUniqueLock lock(_mtx_rtbars);
	if (_rt_bars)
		_rt_bars->clear();
...
void VvtDataManager::update_bars(const char* stdCode, VvTSTickData* newTick)
{
	if (_rt_bars == NULL)
		return;
	...
	{
		VvTSKlineData* kData = (VvTSKlineData*)it->second;
		...
		VvTSBarStruct* lastBar = kData->at(-1);
...
void VvtDataManager::clear_cache()
{
	if (_reader == NULL)
	{
