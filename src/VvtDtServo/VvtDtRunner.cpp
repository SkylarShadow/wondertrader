...
void VvtDtRunner::initialize(const char* cfgFile, bool isFile /* = true */, const char* modDir /* = "" */, const char* logCfg /* = "logcfg.yaml" */, 
			FuncOnTickCallback cbTick /* = NULL */, FuncOnBarCallback cbBar /* = NULL */)
{
	if(_is_inited)
	{
		VvTSLogger::error("VvtDtServo has already been initialized");
		return;
	}
	...
	if(config == NULL)
	{
		...
		VvTSLogger::log_raw(LL_INFO, cfgFile);
		return;
	}
	...
	if (cfgBF->get("hot"))
	{
		...
		VvTSLogger::info("Hot rules loaded");
	}

	if (cfgBF->get("second"))
	{
		_hot_mgr.loadSeconds(cfgBF->getCString("second"));
		VvTSLogger::info("Second rules loaded");
	}
...
void VvtDtRunner::initDataMgr(VvTSVariant* config)
{
	if (config == NULL)
		return;
...
VvTSKlineSlice* VvtDtRunner::get_bars_by_range(const char* stdCode, const char* period, uint64_t beginTime, uint64_t endTime /* = 0 */)
{
	if(!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
		return NULL;
	}
...
VvTSKlineSlice* VvtDtRunner::get_bars_by_date(const char* stdCode, const char* period, uint32_t uDate /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
	...
	{
		VvTSLogger::log_raw(LL_ERROR, "get_bars_by_date only supports minute period");
		return NULL;
	}
...
VvTSTickSlice* VvtDtRunner::get_ticks_by_range(const char* stdCode, uint64_t beginTime, uint64_t endTime /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
...
VvTSTickSlice* VvtDtRunner::get_ticks_by_date(const char* stdCode, uint32_t uDate /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
...
VvTSKlineSlice* VvtDtRunner::get_bars_by_count(const char* stdCode, const char* period, uint32_t count, uint64_t endTime /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
...
VvTSTickSlice* VvtDtRunner::get_ticks_by_count(const char* stdCode, uint32_t count, uint64_t endTime /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
...
VvTSKlineSlice* VvtDtRunner::get_sbars_by_date(const char* stdCode, uint32_t secs, uint32_t uDate /* = 0 */)
{
	if (!_is_inited)
	{
		VvTSLogger::error("VvtDtServo not initialized");
...
void VvtDtRunner::initParsers(VvTSVariant* cfg)
{
...
void VvtDtRunner::start()
{
	_parsers.run();
}

void VvtDtRunner::proc_tick(VvTSTickData* curTick)
{
...
void VvtDtRunner::trigger_tick(const char* stdCode, VvTSTickData* curTick)
{
	if (_cb_tick != NULL)
	{
...
void VvtDtRunner::sub_tick(const char* codes, bool bReplace, bool bInner /* = false */)
{
	if(bInner)
	{
		StdUniqueLock lock(_mtx_innersubs);
		...
		flags.insert(flag);
		VvTSLogger::info("Tick dada of {} subscribed with flag {} for inner use", stdCode, flag);
	}
	else
	{
		StdUniqueLock lock(_mtx_subs);
		...
		for (const std::string& code : ayCodes)
		{
			...
			flags.insert(flag);
			VvTSLogger::info("Tick dada of {} subscribed with flag {}", stdCode, flag);
		}
	}
}

void VvtDtRunner::sub_bar(const char* stdCode, const char* period)
{
	thread_local static char basePeriod[2] = { 0 };
	basePeriod[0] = period[0];
	uint32_t times = 1;
...
void VvtDtRunner::trigger_bar(const char* stdCode, const char* period, VvTSBarStruct* lastBar)
{
	if (_cb_bar == NULL)
		return;
