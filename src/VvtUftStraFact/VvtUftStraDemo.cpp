...
VvtUftStraDemo::VvtUftStraDemo(const char* id)
	: UftStrategy(id)
	, _last_tick(NULL)
...
VvtUftStraDemo::~VvtUftStraDemo()
{
	if (_last_tick)
		_last_tick->release();
}

const char* VvtUftStraDemo::getName()
{
	return "UftDemoStrategy";
}

const char* VvtUftStraDemo::getFactName()
{
	return FACT_NAME;
}

bool VvtUftStraDemo::init(VvTSVariant* cfg)
{
	//这里演示一下外部传入参数的获取
...
void VvtUftStraDemo::on_entrust(uint32_t localid, bool bSuccess, const char* message)
{
	if(!bSuccess)
	{
...
void VvtUftStraDemo::on_init(IUftStraCtx* ctx)
{
...
void VvtUftStraDemo::on_tick(IUftStraCtx* ctx, const char* code, VvTSTickData* newTick)
{	
...
void VvtUftStraDemo::check_orders()
{
...
void VvtUftStraDemo::on_bar(IUftStraCtx* ctx, const char* code, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
	
}

void VvtUftStraDemo::on_trade(IUftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double qty, double price)
{
	
}

void VvtUftStraDemo::on_position(IUftStraCtx* ctx, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	if (_code != stdCode)
		return;
...
void VvtUftStraDemo::on_order(IUftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double totalQty, double leftQty, double price, bool isCanceled)
{
	//如果不是我发出去的订单,我就不管了
	auto it = _orders.find(localid);
	if (it == _orders.end())
...
void VvtUftStraDemo::on_channel_ready(IUftStraCtx* ctx)
{
...
void VvtUftStraDemo::on_channel_lost(IUftStraCtx* ctx)
{
	_channel_ready = false;
}

void VvtUftStraDemo::on_params_updated()
{
	//ctx->watch_param("second", _secs);
	//ctx->watch_param("freq", _freq);
	//ctx->watch_param("offset", _offset);
