...
VvtHftStraDemo::VvtHftStraDemo(const char* id)
	: HftStrategy(id)
	, _last_tick(NULL)
...
VvtHftStraDemo::~VvtHftStraDemo()
{
	if (_last_tick)
		_last_tick->release();
}

const char* VvtHftStraDemo::getName()
{
	return "HftDemoStrategy";
}

const char* VvtHftStraDemo::getFactName()
{
	return FACT_NAME;
}

bool VvtHftStraDemo::init(VvTSVariant* cfg)
{
	//这里演示一下外部传入参数的获取
...
void VvtHftStraDemo::on_entrust(uint32_t localid, bool bSuccess, const char* message, const char* userTag)
{

}

void VvtHftStraDemo::on_init(IHftStraCtx* ctx)
{
...
void VvtHftStraDemo::do_calc(IHftStraCtx* ctx)
{
...
void VvtHftStraDemo::on_tick(IHftStraCtx* ctx, const char* code, VvTSTickData* newTick)
{	
...
void VvtHftStraDemo::check_orders()
{
...
void VvtHftStraDemo::on_bar(IHftStraCtx* ctx, const char* code, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
	
}

void VvtHftStraDemo::on_trade(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double qty, double price, const char* userTag)
{
	do_calc(ctx);
}

void VvtHftStraDemo::on_position(IHftStraCtx* ctx, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	
}

void VvtHftStraDemo::on_order(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	//如果不是我发出去的订单,我就不管了
...
void VvtHftStraDemo::on_channel_ready(IHftStraCtx* ctx)
{
...
void VvtHftStraDemo::on_channel_lost(IHftStraCtx* ctx)
{
	_channel_ready = false;
}