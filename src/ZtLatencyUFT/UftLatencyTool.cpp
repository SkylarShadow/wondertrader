...
namespace uft
{
	...
	public:
		void	run(uint32_t times)
		{
			...
			ZTSLogger::warn("{} ticks simulated in {:.0f} ns, UftEngine Innner Latency: {:.3f} ns", times, total*1.0, t2t);
		}
	...
	public:
		...
		*/
		...
		virtual void on_tick(IUftStraCtx* ctx, const char* code, ZTSTickData* newTick)
		{
			//ZTSLogger::debug("{}", __FUNCTION__);
	...
	bool UftLatencyTool::init()
	{
		...
		ZTSVariant* _config = ZTSCfgLoader::load_from_file(cfgFile);
		if (_config == NULL)
		{
	...
	bool UftLatencyTool::initEngine(ZTSVariant* cfg)
	{
		ZTSLogger::warn("Trading enviroment initialzied with engine: UFT");
