...
class ZtHftStraDemo : public HftStrategy
{
public:
	ZtHftStraDemo(const char* id);
	~ZtHftStraDemo();
...
public:
	...
	virtual void on_tick(IHftStraCtx* ctx, const char* code, ZTSTickData* newTick) override;

	virtual void on_bar(IHftStraCtx* ctx, const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar) override;
