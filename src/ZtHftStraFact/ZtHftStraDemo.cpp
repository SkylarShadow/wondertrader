...
ZtHftStraDemo::ZtHftStraDemo(const char* id)
	: HftStrategy(id)
	, _last_tick(NULL)
...
ZtHftStraDemo::~ZtHftStraDemo()
{
	if (_last_tick)
		_last_tick->release();
}

const char* ZtHftStraDemo::getName()
{
	return "HftDemoStrategy";
}

const char* ZtHftStraDemo::getFactName()
{
	return FACT_NAME;
}

bool ZtHftStraDemo::init(ZTSVariant* cfg)
{
	//这里演示一下外部传入参数的获取
...
void ZtHftStraDemo::on_entrust(uint32_t localid, bool bSuccess, const char* message, const char* userTag)
{

}

void ZtHftStraDemo::on_init(IHftStraCtx* ctx)
{
	//ZTSTickSlice* ticks = ctx->stra_get_ticks(_code.c_str(), _count);
	//if (ticks)
	//	ticks->release();

	ZTSKlineSlice* kline = ctx->stra_get_bars(_code.c_str(), "m1", 30);
	if (kline)
		kline->release();
...
void ZtHftStraDemo::do_calc(IHftStraCtx* ctx)
{
	...
	ZTSTickData* curTick = ctx->stra_get_last_tick(code);
	if (curTick == NULL)
		return;
	...
	{//如果spread上次计算的时候小于当前分钟,则重算spread
		//ZTSKlineSlice* kline = ctx->stra_get_bars(code, "m5", 30);
		//if (kline)
		//	kline->release();
	...
	if (signal != 0)
	{
		...
		ZTSCommodityInfo* cInfo = ctx->stra_get_comminfo(code);
...
void ZtHftStraDemo::on_tick(IHftStraCtx* ctx, const char* code, ZTSTickData* newTick)
{	
...
void ZtHftStraDemo::check_orders()
{
...
void ZtHftStraDemo::on_bar(IHftStraCtx* ctx, const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	
}

void ZtHftStraDemo::on_trade(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double qty, double price, const char* userTag)
{
	do_calc(ctx);
}

void ZtHftStraDemo::on_position(IHftStraCtx* ctx, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	
}

void ZtHftStraDemo::on_order(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	//如果不是我发出去的订单,我就不管了
...
void ZtHftStraDemo::on_channel_ready(IHftStraCtx* ctx)
{
...
void ZtHftStraDemo::on_channel_lost(IHftStraCtx* ctx)
{
	_channel_ready = false;
}