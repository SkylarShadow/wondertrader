...
ZtRtRunner& getRunner()
{
	static ZtRtRunner runner;
	return runner;
}
...
void feed_adj_factors(ZTString stdCode, ZTUInt32* dates, double* factors, ZTUInt32 count)
{
...
{
	ZTSLogger::error("API not implemented");
}
...
ZTUInt32 hft_get_ordque(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetOrdQueCallback cb)
{
	...
	try
	{
		ZTSOrdQueSlice* dataSlice = ctx->stra_get_order_queue(stdCode, itemCnt);
		if (dataSlice)
		{
...
ZTUInt32 hft_get_orddtl(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetOrdDtlCallback cb)
{
	...
	try
	{
		ZTSOrdDtlSlice* dataSlice = ctx->stra_get_order_detail(stdCode, itemCnt);
		if (dataSlice)
		{
			uint32_t thisCnt = min(itemCnt, (ZTUInt32)dataSlice->size());
...
ZTUInt32 hft_get_trans(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetTransCallback cb)
{
	...
	try
	{
		ZTSTransSlice* dataSlice = ctx->stra_get_transaction(stdCode, itemCnt);
		if (dataSlice)
		{
			uint32_t thisCnt = min(itemCnt, (ZTUInt32)dataSlice->size());
...
void parser_push_quote(const char* id, ZTSTickStruct* curTick, ZTUInt32 uProcFlag)
{
