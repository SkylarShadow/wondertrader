...
#include "../ZtCore/CtaStraContext.h"
#include "../ZtCore/HftStraContext.h"
#include "../ZtCore/SelStraContext.h"
...
bool ZtRtRunner::init(const char* logCfg /* = "logcfg.prop" */, bool isFile /* = true */, const char* genDir)
{
#ifdef _MSC_VER
	...
	if(isFile)
	{
		...
		ZTSLogger::init(path.c_str(), true, this);
	}
	else
	{
		ZTSLogger::init(logCfg, false, this);
	}
	

	ZtHelper::setInstDir(getBinDir());
	ZtHelper::setGenerateDir(StrUtil::standardisePath(genDir).c_str());
	return true;
}

void ZtRtRunner::registerEvtCallback(FuncEventCallback cbEvt)
{
	_cb_evt = cbEvt;
...
void ZtRtRunner::registerParserPorter(FuncParserEvtCallback cbEvt, FuncParserSubCallback cbSub)
{
...
void ZtRtRunner::registerExecuterPorter(FuncExecInitCallback cbInit, FuncExecCmdCallback cbExec)
{
	_cb_exec_init = cbInit;
	_cb_exec_cmd = cbExec;

	ZTSLogger::info("Callbacks of Extented Executer registration done");
}

void ZtRtRunner::registerCtaCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, 
		FuncSessionEvtCallback cbSessEvt, FuncStraCondTriggerCallback cbCondTrigger /* = NULL */)
{
	...
	ZTSLogger::info("Callbacks of CTA engine registration done");
}

void ZtRtRunner::registerSelCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, FuncSessionEvtCallback cbSessEvt)
{
	_cb_sel_init = cbInit;
	_cb_sel_tick = cbTick;
	_cb_sel_calc = cbCalc;
	...
	ZTSLogger::info("Callbacks of SEL engine registration done");
}

void ZtRtRunner::registerHftCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraBarCallback cbBar, 
	...
	FuncStraOrdDtlCallback cbOrdDtl, FuncStraOrdQueCallback cbOrdQue, FuncStraTransCallback cbTrans, FuncSessionEvtCallback cbSessEvt, FuncHftPosCallback cbPosition)
{
	...
	ZTSLogger::info("Callbacks of HFT engine registration done");
}

bool ZtRtRunner::loadFinalHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb)
{
	...
	default:
	{
		ZTSLogger::error("Unsupported period of extended data loader");
		return false;
	}
	}
}

bool ZtRtRunner::loadRawHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool ZtRtRunner::loadAllAdjFactors(void* obj, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool ZtRtRunner::loadAdjFactors(void* obj, const char* stdCode, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
	if (_ext_adj_fct_loader == NULL)
		return false;
...
void ZtRtRunner::feedAdjFactors(const char* stdCode, uint32_t* dates, double* factors, uint32_t count)
{
	_feeder_fcts(_feed_obj, stdCode, dates, factors, count);
}


void ZtRtRunner::feedRawBars(ZTSBarStruct* bars, uint32_t count)
{
	if (_ext_fnl_bar_loader == NULL)
	{
		ZTSLogger::error("Cannot feed bars because of no extented bar loader registered.");
		return;
	}
...
bool ZtRtRunner::createExtParser(const char* id)
{
	...
	ZTSLogger::info("Extended parser created");
	return true;
}

bool ZtRtRunner::createExtExecuter(const char* id)
{
	ExpExecuter* executer = new ExpExecuter(id);
	executer->init();
	_cta_engine.addExecuter(ExecCmdPtr(executer));
	ZTSLogger::info("Extended Executer created");
	return true;
}

uint32_t ZtRtRunner::createCtaContext(const char* name, int32_t slippage /* = 0 */)
{
...
uint32_t ZtRtRunner::createHftContext(const char* name, const char* trader, bool bAgent, int32_t slippage /* = 0 */)
{
	...
	{
		ZTSLogger::error("Trader {} not exists, Binding trader to HFT strategy failed", trader);
	}
	return ctx->id();
}

uint32_t ZtRtRunner::createSelContext(const char* name, uint32_t date, uint32_t time, const char* period, int32_t slippage, const char* trdtpl /* = "CHINA" */, const char* session/* ="TRADING" */)
{
...
const char* ZtRtRunner::get_raw_stdcode(const char* stdCode)
{
...
CtaContextPtr ZtRtRunner::getCtaContext(uint32_t id)
{
	return _cta_engine.getContext(id);
}

HftContextPtr ZtRtRunner::getHftContext(uint32_t id)
{
	return _hft_engine.getContext(id);
}

SelContextPtr ZtRtRunner::getSelContext(uint32_t id)
{
	return _sel_engine.getContext(id);
}

void ZtRtRunner::ctx_on_bar(uint32_t id, const char* stdCode, const char* period, ZTSBarStruct* newBar, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtRtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtRtRunner::ctx_on_cond_triggered(uint32_t id, const char* stdCode, double target, double price, const char* usertag, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtRtRunner::ctx_on_init(uint32_t id, EngineType eType/* = ET_CTA*/)
{
	switch (eType)
	{
...
void ZtRtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate, bool isBegin /* = true */, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtRtRunner::ctx_on_tick(uint32_t id, const char* stdCode, ZTSTickData* newTick, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtRtRunner::hft_on_channel_lost(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, CHNL_EVENT_LOST);
}

void ZtRtRunner::hft_on_channel_ready(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, CHNL_EVENT_READY);
}

void ZtRtRunner::hft_on_entrust(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool bSuccess, const char* message, const char* userTag)
{
	if (_cb_hft_entrust)
		_cb_hft_entrust(cHandle, localid, stdCode, bSuccess, message, userTag);
}

void ZtRtRunner::hft_on_order(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	if (_cb_hft_ord)
		_cb_hft_ord(cHandle, localid, stdCode, isBuy, totalQty, leftQty, price, isCanceled, userTag);
}

void ZtRtRunner::hft_on_trade(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool isBuy, double vol, double price, const char* userTag)
{
	if (_cb_hft_trd)
		_cb_hft_trd(cHandle, localid, stdCode, isBuy, vol, price, userTag);
}

void ZtRtRunner::hft_on_position(uint32_t cHandle, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	if (_cb_hft_position)
		_cb_hft_position(cHandle, stdCode, isLong, prevol, preavail, newvol, newavail);
}

bool ZtRtRunner::config(const char* cfgFile, bool isFile /* = true */)
{
	...
	if (cfgBF->get("holiday"))
	{
		...
		ZTSLogger::log_raw(LL_INFO, "Holidays loaded");
	}
	...
	if (!_is_hft)
	{
		ZTSVariant* cfgExec = _config->get("executers");
		if(cfgExec != NULL)
		{
			if(cfgExec->type() == ZTSVariant::VT_String)
			{
				const char* filename = cfgExec->asCString();
				if (StdFile::exists(filename))
				{
					ZTSLogger::info("Reading executer config from {}...", filename);
					ZTSVariant* var = ZTSCfgLoader::load_from_file(filename);
					if (var)
					{
					...
					{
						ZTSLogger::error("Loading executer config {} failed", filename);
					}
				}
				else
				{
			...
			else if(cfgExec->type() == ZTSVariant::VT_Array)
			{
				initExecuters(cfgExec);
			}
		}

		ZTSVariant* cfgRouter = _config->get("routers");
		if (cfgRouter != NULL)
...
bool ZtRtRunner::initCtaStrategies()
{
...
bool ZtRtRunner::initSelStrategies()
{
	ZTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != ZTSVariant::VT_Object)
		return false;
...
bool ZtRtRunner::initHftStrategies()
{
	ZTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != ZTSVariant::VT_Object)
		return false;
	...
	for (uint32_t idx = 0; idx < cfg->size(); idx++)
	{
		...
		{
			ZTSLogger::error("Trader {} not exists, Binding trader to HFT strategy failed", traderid);
		}
...
bool ZtRtRunner::initEngine()
{
	...
	if (_is_hft)
	{
		ZTSLogger::info("Trading environment initialized, engine name: HFT");
		_hft_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_hft_engine;
	}
	else if (_is_sel)
	{
		ZTSLogger::info("Trading environment initialized, engine name: SEL");
		_sel_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_sel_engine;
	}
	else
	{
		ZTSLogger::info("Trading environment initialized, engine name: CTA");
...
bool ZtRtRunner::initDataMgr()
{
	...
	ZTSLogger::log_raw(LL_INFO, "Data manager initialized");
	return true;
}

bool ZtRtRunner::initParsers(ZTSVariant* cfgParsers)
{
	if (cfgParsers == NULL || cfgParsers->type() != ZTSVariant::VT_Array)
		return false;

	uint32_t count = 0;
	for (uint32_t idx = 0; idx < cfgParsers->size(); idx++)
	{
		ZTSVariant* cfgItem = cfgParsers->get(idx);
		if (!cfgItem->getBoolean("active"))
			continue;
...
bool ZtRtRunner::initExecuters(ZTSVariant* cfgExecuter)
{
	if (cfgExecuter == NULL || cfgExecuter->type() != ZTSVariant::VT_Array)
		return false;
	...
	{
		ZTSVariant* cfgItem = cfgExecuter->get(idx);
		if (!cfgItem->getBoolean("active"))
			continue;
		...
		{
			ZtArbiExecuter* executer = new ZtArbiExecuter(&_exe_factory, id, &_data_mgr);
			if (!executer->init(cfgItem))
				return false;
...
bool ZtRtRunner::initEvtNotifier()
{
...
bool ZtRtRunner::initTraders(ZTSVariant* cfgTraders)
{
	if (cfgTraders == NULL || cfgTraders->type() != ZTSVariant::VT_Array)
		return false;
	
	uint32_t count = 0;
	for (uint32_t idx = 0; idx < cfgTraders->size(); idx++)
	{
		ZTSVariant* cfgItem = cfgTraders->get(idx);
		if (!cfgItem->getBoolean("active"))
			continue;
...
void ZtRtRunner::run(bool bAsync /* = false */)
{
	try
	{
		_parsers.run();
...
void ZtRtRunner::handleLogAppend(ZTSLogLevel ll, const char* msg)
{
	_notifier.notify_log(LOG_TAGS[ll-100], msg);
}

void ZtRtRunner::release()
{
	ZTSLogger::stop();
}

bool ZtRtRunner::initActionPolicy()
{
...
bool ZtRtRunner::addCtaFactories(const char* folder)
{
	return _cta_mgr.loadFactories(folder);
}

bool ZtRtRunner::addSelFactories(const char* folder)
{
	return _sel_mgr.loadFactories(folder);
}

bool ZtRtRunner::addExeFactories(const char* folder)
{
	return _exe_factory.loadFactories(folder);
}

bool ZtRtRunner::addHftFactories(const char* folder)
{
	return _hft_mgr.loadFactories(folder);
}

void ZtRtRunner::hft_on_order_queue(uint32_t id, const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_cb_hft_ordque)
		_cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void ZtRtRunner::hft_on_order_detail(uint32_t id, const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_cb_hft_orddtl)
		_cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void ZtRtRunner::hft_on_transaction(uint32_t id, const char* stdCode, ZTSTransData* newTrans)
{
	if (_cb_hft_trans)
...
void ZtRtRunner::parser_init(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_INIT, id);
}

void ZtRtRunner::parser_connect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_CONNECT, id);
}

void ZtRtRunner::parser_disconnect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_DISCONNECT, id);
}

void ZtRtRunner::parser_release(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_RELEASE, id);
}

void ZtRtRunner::parser_subscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, true);
}

void ZtRtRunner::parser_unsubscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, false);
}

void ZtRtRunner::on_ext_parser_quote(const char* id, ZTSTickStruct* curTick, uint32_t uProcFlag)
{
...
void ZtRtRunner::executer_init(const char* id)
{
	if (_cb_exec_init)
		_cb_exec_init(id);
}

void ZtRtRunner::executer_set_position(const char* id, const char* stdCode, double target)
{
	if (_cb_exec_cmd)
