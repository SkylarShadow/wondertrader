...
ExpSelContext::ExpSelContext(ZtSelEngine* env, const char* name, int32_t slippage)
...
void ExpSelContext::on_bar_close(const char* stdCode, const char* period, ZTSBarStruct* newBar)
{
	getRunner().ctx_on_bar(_context_id, stdCode, period, newBar, ET_SEL);
}

void ExpSelContext::on_tick_updated(const char* stdCode, ZTSTickData* newTick)
{
