...
extern ZtRtRunner& getRunner();

void ExpHftContext::on_bar(const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void ExpHftContext::on_tick(const char* code, ZTSTickData* newTick)
{
...
void ExpHftContext::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	getRunner().hft_on_order_queue(_context_id, stdCode, newOrdQue);
}

void ExpHftContext::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	getRunner().hft_on_order_detail(_context_id, stdCode, newOrdDtl);
}

void ExpHftContext::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
