...
void ExpHftMocker::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void ExpHftMocker::on_tick_updated(const char* stdCode, ZTSTickData* newTick)
{
...
void ExpHftMocker::on_ordque_updated(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	getRunner().hft_on_order_queue(_context_id, stdCode, newOrdQue);
}

void ExpHftMocker::on_orddtl_updated(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	getRunner().hft_on_order_detail(_context_id, stdCode, newOrdDtl);
}

void ExpHftMocker::on_trans_updated(const char* stdCode, ZTSTransData* newTrans)
{
