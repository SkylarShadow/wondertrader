...
ZtBtRunner& getRunner()
{
	static ZtBtRunner runner;
	return runner;
}
...
void feed_raw_bars(ZTSBarStruct* bars, ZTUInt32 count)
{
	getRunner().feedRawBars(bars, count);
}

void feed_raw_ticks(ZTSTickStruct* ticks, ZTUInt32 count)
{
...
void set_time_range(ZTUInt64 stime, ZTUInt64 etime)
{
...
ZTString get_raw_stdcode(const char* stdCode)
{
...
void write_log(ZTUInt32 level, const char* message, const char* catName)
{
	if (strlen(catName) > 0)
	{
...
CtxHandler init_sel_mocker(const char* name, ZTUInt32 date, ZTUInt32 time, const char* period, const char* trdtpl/* = "CHINA"*/, const char* session/* = "TRADING"*/, int slippage/* = 0*/, bool bRatioSlp/* = false*/)
{
...
ZTUInt32 cta_get_bars(CtxHandler cHandle, const char* stdCode, const char* period, ZTUInt32 barCnt, bool isMain, FuncGetBarsCallback cb)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
	try
	{
		ZTSKlineSlice* kData = ctx->stra_get_bars(stdCode, period, barCnt, isMain);
		if (kData)
		{
			ZTUInt32 reaCnt = (ZTUInt32)kData->size();
...
ZTUInt32	cta_get_ticks(CtxHandler cHandle, const char* stdCode, ZTUInt32 tickCnt, FuncGetTicksCallback cb)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
	try
	{
		ZTSTickSlice* tData = ctx->stra_get_ticks(stdCode, tickCnt);
		if (tData)
		{
			uint32_t thisCnt = min(tickCnt, (ZTUInt32)tData->size());
			if (thisCnt != 0)
				cb(cHandle, stdCode, (ZTSTickStruct*)tData->at(0), thisCnt, true);
			else
...
ZTUInt64 cta_get_detail_entertime(CtxHandler cHandle, const char* stdCode, const char* openTag)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 cta_get_first_entertime(CtxHandler cHandle, const char* stdCode)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 cta_get_last_entertime(CtxHandler cHandle, const char* stdCode)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 cta_get_last_exittime(CtxHandler cHandle, const char* stdCode)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
...
ZTString cta_get_last_entertag(CtxHandler cHandle, const char* stdCode)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt32 cta_get_tdate()
{
	return getRunner().replayer().get_trading_date();
}

ZTUInt32 cta_get_date()
{
	return getRunner().replayer().get_date();
}

ZTUInt32 cta_get_time()
{
	return getRunner().replayer().get_min_time();
}

void cta_log_text(CtxHandler cHandle, ZTUInt32 level, const char* message)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return;
...
ZTString cta_load_userdata(CtxHandler cHandle, const char* key, const char* defVal)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return defVal;
...
void cta_register_index(CtxHandler cHandle, const char* idxName, ZTUInt32 indexType)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return;
...
bool cta_register_index_line(CtxHandler cHandle, const char* idxName, const char* lineName, ZTUInt32 lineType)
{
	CtaMocker* ctx = getRunner().cta_mocker();
	if (ctx == NULL)
		return false;
...
ZTString sel_load_userdata(CtxHandler cHandle, const char* key, const char* defVal)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return defVal;
...
void sel_log_text(CtxHandler cHandle, ZTUInt32 level, const char* message)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return;
...
ZTUInt32 sel_get_date()
{
	return getRunner().replayer().get_date();
}

ZTUInt32 sel_get_time()
{
	return getRunner().replayer().get_min_time();
}
...
ZTUInt32 sel_get_bars(CtxHandler cHandle, const char* stdCode, const char* period, ZTUInt32 barCnt, FuncGetBarsCallback cb)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
	try
	{
		ZTSKlineSlice* kData = ctx->stra_get_bars(stdCode, period, barCnt);
		if (kData)
		{
			ZTUInt32 reaCnt = (ZTUInt32)kData->size();
...
ZTUInt32	sel_get_ticks(CtxHandler cHandle, const char* stdCode, ZTUInt32 tickCnt, FuncGetTicksCallback cb)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 sel_get_detail_entertime(CtxHandler cHandle, const char* stdCode, const char* openTag)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 sel_get_first_entertime(CtxHandler cHandle, const char* stdCode)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 sel_get_last_entertime(CtxHandler cHandle, const char* stdCode)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt64 sel_get_last_exittime(CtxHandler cHandle, const char* stdCode)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTString sel_get_last_entertag(CtxHandler cHandle, const char* stdCode)
{
	SelMocker* ctx = getRunner().sel_mocker();
	if (ctx == NULL)
		return 0;
...
ZTUInt32 hft_get_date()
{
	return getRunner().replayer().get_date();
}

ZTUInt32 hft_get_time()
{
	return getRunner().replayer().get_raw_time();
}

ZTUInt32 hft_get_secs()
{
	return getRunner().replayer().get_secs();
}

ZTUInt32 hft_get_bars(CtxHandler cHandle, const char* stdCode, const char* period, ZTUInt32 barCnt, FuncGetBarsCallback cb)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return 0;
	...
	try
	{
		ZTSKlineSlice* kData = mocker->stra_get_bars(stdCode, period, barCnt);
		if (kData)
		{
			ZTUInt32 reaCnt = (ZTUInt32)kData->size();
...
ZTUInt32 hft_get_ticks(CtxHandler cHandle, const char* stdCode, ZTUInt32 tickCnt, FuncGetTicksCallback cb)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return 0;
	try
	{
		ZTSTickSlice* tData = mocker->stra_get_ticks(stdCode, tickCnt);
		if (tData)
		{
			uint32_t thisCnt = min(tickCnt, (ZTUInt32)tData->size());
			if(thisCnt != 0)
...
ZTUInt32 hft_get_ordque(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetOrdQueCallback cb)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return 0;
	try
	{
		ZTSOrdQueSlice* dataSlice = mocker->stra_get_order_queue(stdCode, itemCnt);
		if (dataSlice)
		{
			uint32_t thisCnt = min(itemCnt, (ZTUInt32)dataSlice->size());
			cb(cHandle, stdCode, (ZTSOrdQueStruct*)dataSlice->at(0), thisCnt, true);
			dataSlice->release();
			return thisCnt;
		}
		else
...
ZTUInt32 hft_get_orddtl(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetOrdDtlCallback cb)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return 0;
	try
	{
		ZTSOrdDtlSlice* dataSlice = mocker->stra_get_order_detail(stdCode, itemCnt);
		if (dataSlice)
		{
			uint32_t thisCnt = min(itemCnt, (ZTUInt32)dataSlice->size());
			cb(cHandle, stdCode, (ZTSOrdDtlStruct*)dataSlice->at(0), thisCnt, true);
			dataSlice->release();
			return thisCnt;
		}
		else
...
ZTUInt32 hft_get_trans(CtxHandler cHandle, const char* stdCode, ZTUInt32 itemCnt, FuncGetTransCallback cb)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return 0;
	try
	{
		ZTSTransSlice* dataSlice = mocker->stra_get_transaction(stdCode, itemCnt);
		if (dataSlice)
		{
			uint32_t thisCnt = min(itemCnt, (ZTUInt32)dataSlice->size());
			cb(cHandle, stdCode, (ZTSTransStruct*)dataSlice->at(0), thisCnt, true);
			dataSlice->release();
			return thisCnt;
		}
		else
...
void hft_log_text(CtxHandler cHandle, ZTUInt32 level, const char* message)
{
...
bool hft_cancel(CtxHandler cHandle, ZTUInt32 localid)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return false;
...
ZTString hft_cancel_all(CtxHandler cHandle, const char* stdCode, bool isBuy)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return "";
...
ZTString hft_buy(CtxHandler cHandle, const char* stdCode, double price, double qty, const char* userTag, int flag)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return "";
...
ZTString hft_sell(CtxHandler cHandle, const char* stdCode, double price, double qty, const char* userTag, int flag)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return "";
...
ZTString hft_load_userdata(CtxHandler cHandle, const char* key, const char* defVal)
{
	HftMocker* mocker = getRunner().hft_mocker();
	if (mocker == NULL)
		return defVal;
