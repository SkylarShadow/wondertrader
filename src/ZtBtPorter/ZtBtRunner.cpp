...
#include "../ZtBtCore/ExecMocker.h"
#include "../ZtBtCore/ZtHelper.h"
...
bool ZtBtRunner::loadRawHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb)
{
...
bool ZtBtRunner::loadFinalHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool ZtBtRunner::loadAllAdjFactors(void* obj, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool ZtBtRunner::loadAdjFactors(void* obj, const char* stdCode, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
	if (_ext_adj_fct_loader == NULL)
		return false;
...
bool ZtBtRunner::loadRawHisTicks(void* obj, const char* stdCode, uint32_t uDate, FuncReadTicks cb)
{
	StdUniqueLock lock(_feed_mtx);
...
void ZtBtRunner::feedRawBars(ZTSBarStruct* bars, uint32_t count)
{
...
void ZtBtRunner::feedAdjFactors(const char* stdCode, uint32_t* dates, double* factors, uint32_t count)
{
	if(_ext_adj_fct_loader == NULL)
	{
		ZTSLogger::error("Cannot feed adjusting factors because of no extented adjusting factor loader registered.");
		return;
	}
...
void ZtBtRunner::feedRawTicks(ZTSTickStruct* ticks, uint32_t count)
{
	if (_ext_tick_loader == NULL)
	{
		ZTSLogger::error("Cannot feed ticks because of no extented tick loader registered.");
		return;
	}
...
void ZtBtRunner::registerCtaCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, 
...
void ZtBtRunner::registerSelCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, 
...
void ZtBtRunner::registerHftCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraBarCallback cbBar,
...
uint32_t ZtBtRunner::initCtaMocker(const char* name, int32_t slippage /* = 0 */, bool hook /* = false */, 
...
uint32_t ZtBtRunner::initHftMocker(const char* name, bool hook/* = false*/)
{
	if (_hft_mocker)
	{
		delete _hft_mocker;
...
uint32_t ZtBtRunner::initSelMocker(const char* name, uint32_t date, uint32_t time, const char* period, 
...
void ZtBtRunner::ctx_on_bar(uint32_t id, const char* stdCode, const char* period, ZTSBarStruct* newBar, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_calc_done(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_init(uint32_t id, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_cond_triggered(uint32_t id, const char* stdCode, double target, double price, const char* usertag, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate, bool isBegin /* = true */, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void ZtBtRunner::ctx_on_tick(uint32_t id, const char* stdCode, ZTSTickData* newTick, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void ZtBtRunner::hft_on_order_queue(uint32_t id, const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_cb_hft_ordque)
		_cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void ZtBtRunner::hft_on_order_detail(uint32_t id, const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_cb_hft_orddtl)
		_cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void ZtBtRunner::hft_on_transaction(uint32_t id, const char* stdCode, ZTSTransData* newTrans)
{
	if (_cb_hft_trans)
		_cb_hft_trans(id, stdCode, &newTrans->getTransStruct());
}

void ZtBtRunner::hft_on_channel_ready(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, 1000/*CHNL_EVENT_READY*/);
}

void ZtBtRunner::hft_on_entrust(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool bSuccess, const char* message, const char* userTag)
{
	if (_cb_hft_entrust)
		_cb_hft_entrust(cHandle, localid, stdCode, bSuccess, message, userTag);
}

void ZtBtRunner::hft_on_order(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	if (_cb_hft_ord)
		_cb_hft_ord(cHandle, localid, stdCode, isBuy, totalQty, leftQty, price, isCanceled, userTag);
}

void ZtBtRunner::hft_on_trade(uint32_t cHandle, ZTUInt32 localid, const char* stdCode, bool isBuy, double vol, double price, const char* userTag)
{
	if (_cb_hft_trd)
		_cb_hft_trd(cHandle, localid, stdCode, isBuy, vol, price, userTag);
}

void ZtBtRunner::init(const char* logProfile /* = "" */, bool isFile /* = true */, const char* outDir/* = "./outputs_bt"*/)
{
#ifdef _MSC_VER
	CMiniDumper::Enable(getModuleName(), true, ZtHelper::getCWD().c_str());
#endif

	ZTSLogger::init(logProfile, isFile);

	ZtHelper::setInstDir(getBinDir());
	ZtHelper::setOutputDir(outDir);
}

void ZtBtRunner::config(const char* cfgFile, bool isFile /* = true */)
{
	if(_inited)
	{
		ZTSLogger::error("ZtBtEngine has already been inited");
		return;
	}

	_cfg = isFile ? ZTSCfgLoader::load_from_file(cfgFile) : ZTSCfgLoader::load_from_content(cfgFile, false);
	if(_cfg == NULL)
	{
	...
	ZTSVariant* cfgEnv = _cfg->get("env");
	const char* mode = cfgEnv->getCString("mocker");
	ZTSVariant* cfgMode = _cfg->get(mode);
	...
	else if (strcmp(mode, "sel") == 0 && cfgMode)
	{
		...
		ZTSVariant* cfgTask = cfgMode->get("task");
		if(cfgTask)
...
void ZtBtRunner::run(bool bNeedDump /* = false */, bool bAsync /* = false */)
{
	if (_running)
		return;
	...
	ZTSLogger::info("Backtesting will run in {} mode", _async ? "async" : "sync");
	...
	else
	{
		_worker.reset(new StdThread([this, bNeedDump]() {
			...
			{
				ZTSLogger::error("Exception raised while worker running");
				//print_stack_trace([](const char* message) {
				//	ZTSLogger::error(message);
				//});
			}
			ZTSLogger::debug("Worker thread of backtest finished");
			_running = false;
...
void ZtBtRunner::stop()
{
	...
	ZTSLogger::debug("Notify to finish last round");
	...
	ZTSLogger::debug("Last round ended");
	...
	ZTSLogger::freeAllDynLoggers();

	ZTSLogger::debug("Backtest stopped");
}

void ZtBtRunner::release()
{
	ZTSLogger::stop();
}

void ZtBtRunner::set_time_range(ZTUInt64 stime, ZTUInt64 etime)
{
	_replayer.set_time_range(stime, etime);

	ZTSLogger::info("Backtest time range is set to be [{},{}] mannually", stime, etime);
}

void ZtBtRunner::enable_tick(bool bEnabled /* = true */)
{
	_replayer.enable_tick(bEnabled);

	ZTSLogger::info("Tick data replaying is {}", bEnabled ? "enabled" : "disabled");
}

void ZtBtRunner::clear_cache()
{
	_replayer.clear_cache();
}

const char* ZtBtRunner::get_raw_stdcode(const char* stdCode)
{
...
bool ZtBtRunner::initEvtNotifier(ZTSVariant* cfg)
{
