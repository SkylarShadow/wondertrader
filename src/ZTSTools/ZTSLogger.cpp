...
ILogHandler*		ZTSLogger::m_logHandler	= NULL;
ZTSLogLevel			ZTSLogger::m_logLevel	= LL_NONE;
bool				ZTSLogger::m_bStopped = false;
bool				ZTSLogger::m_bInited = false;
bool				ZTSLogger::m_bTpInited = false;
SpdLoggerPtr		ZTSLogger::m_rootLogger = NULL;
ZTSLogger::LogPatterns*	ZTSLogger::m_mapPatterns = NULL;
thread_local char	ZTSLogger::m_buffer[];
std::set<std::string>	ZTSLogger::m_setDynLoggers;
...
inline ZTSLogLevel str_to_ll(const char* slvl)
{
	if (vvt_stricmp(slvl, "debug") == 0)
	{
		return LL_DEBUG;
...
void ZTSLogger::print_message(const char* buffer)
{
	print_timetag(true);
	fmt::print(buffer);
	fmt::print("\r\n");
...
void ZTSLogger::initLogger(const char* catName, ZTSVariant* cfgLogger)
{
	bool bAsync = cfgLogger->getBoolean("async");
	const char* level = cfgLogger->getCString("level");

	ZTSVariant* cfgSinks = cfgLogger->get("sinks");
	std::vector<spdlog::sink_ptr> sinks;
	for (uint32_t idx = 0; idx < cfgSinks->size(); idx++)
	{
		ZTSVariant* cfgSink = cfgSinks->get(idx);
...
void ZTSLogger::init(const char* propFile /* = "logcfg.json" */, bool isFile /* = true */, ILogHandler* handler /* = NULL */)
{
	if (m_bInited)
		return;
	...
	ZTSVariant* cfg = isFile ? ZTSCfgLoader::load_from_file(propFile) : ZTSCfgLoader::load_from_content(propFile, false);
	if (cfg == NULL)
		return;
	...
	for (std::string& key : keys)
	{
		...
		if (key == DYN_PATTERN)
		{
			...
			{
				ZTSVariant* cfgPattern = cfgItem->get(pkey.c_str());
...
void ZTSLogger::registerHandler(ILogHandler* handler /* = NULL */)
{
	m_logHandler = handler;
}
...
void ZTSLogger::debug_imp(SpdLoggerPtr logger, const char* message)
{
	if (logger)
...
void ZTSLogger::info_imp(SpdLoggerPtr logger, const char* message)
{
	if (logger)
		logger->info(message);
...
void ZTSLogger::warn_imp(SpdLoggerPtr logger, const char* message)
{
	if (logger)
		logger->warn(message);
...
void ZTSLogger::error_imp(SpdLoggerPtr logger, const char* message)
{
	if (logger)
...
void ZTSLogger::fatal_imp(SpdLoggerPtr logger, const char* message)
{
	if (logger)
...
void ZTSLogger::log_raw(ZTSLogLevel ll, const char* message)
{
...
void ZTSLogger::log_raw_by_cat(const char* catName, ZTSLogLevel ll, const char* message)
{
	if (m_logLevel > ll || m_bStopped)
		return;
...
void ZTSLogger::log_dyn_raw(const char* patttern, const char* catName, ZTSLogLevel ll, const char* message)
{
	if (m_logLevel > ll || m_bStopped)
		return;
...
SpdLoggerPtr ZTSLogger::getLogger(const char* logger, const char* pattern /* = "" */)
{
	...
	if (ret == NULL && strlen(pattern) > 0)
	{
		...
		ZTSVariant* cfg = (ZTSVariant*)m_mapPatterns->get(pattern);
		if (cfg == NULL)
			return SpdLoggerPtr();
...
void ZTSLogger::freeAllDynLoggers()
{
