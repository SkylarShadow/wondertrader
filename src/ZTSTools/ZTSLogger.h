...
private:
	...
	static void initLogger(const char* catName, ZTSVariant* cfgLogger);
...
public:
	 ...
	 *	直接输出
	 */
	static void log_raw(ZTSLogLevel ll, const char* message);

	/*
	 *	分类输出
	 */
	static void log_raw_by_cat(const char* catName, ZTSLogLevel ll, const char* message);

	/*
	 *	动态分类输出
	 */
	static void log_dyn_raw(const char* patttern, const char* catName, ZTSLogLevel ll, const char* message);
...
public:
	...
	template<typename... Args>
	static void log(ZTSLogLevel ll, const char* format, const Args& ...args)
	{
	...
	template<typename... Args>
	static void log_by_cat(const char* catName, ZTSLogLevel ll, const char* format, const Args& ...args)
	{
		if (m_logLevel > ll || m_bStopped)
			return;
	...
	template<typename... Args>
	static void log_by_cat_prefix(const char* catName, ZTSLogLevel ll, const char* format, const Args& ...args)
	{
		if (m_logLevel > ll || m_bStopped)
			return;
	...
	template<typename... Args>
	static void log_dyn(const char* patttern, const char* catName, ZTSLogLevel ll, const char* format, const Args& ...args)
	{
		if (m_logLevel > ll || m_bStopped)
			return;
	...
	template<typename... Args>
	static void log_dyn_prefix(const char* patttern, const char* catName, ZTSLogLevel ll, const char* format, const Args& ...args)
	{
		if (m_logLevel > ll || m_bStopped)
			return;
...
private:
	...
	static ZTSLogLevel			m_logLevel;

	static SpdLoggerPtr			m_rootLogger;

	typedef ZTSHashMap<std::string>	LogPatterns;
