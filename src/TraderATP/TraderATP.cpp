...
inline ZTSDirectionType wrapATPSide(const ATPSideType side)
{
...
inline ZTSDirectionType wrapDirectionType(ATPSideType side, ATPPositionEffectType pe)
{
...
inline ATPSideType wrapDirectionType(ZTSDirectionType dirType, ZTSOffsetType offsetType)
{
	if (WDT_LONG == dirType)
...
inline ZTSOffsetType wrapOffsetType(const ATPPositionEffectType dirType)
{
...
inline ATPPositionEffectType wrapOffsetType(ZTSOffsetType offType)
{
	if (WOT_OPEN == offType)
...
inline ATPOrdTypeType wrapOrdType(ZTSPriceType priceType, ZTSOrderFlag flag)
{
...
inline ZTSPriceType wrapOrdType(ATPOrdTypeType ordType)
{
...
inline ZTSOrderState wrapOrdStatus(ATPOrdStatusType orderState)
{
	switch (orderState)
	{
...
ZTSEntrust* TraderATP::makeEntrust(const ATPRspOrderStatusAckMsg* order_info)
{
...
ZTSOrderInfo* TraderATP::makeOrderInfo(const APIOrderUnit* order_info)
{
	const std::string code(order_info->security_id);
	const std::string exchg = (order_info->market_id == ATPMarketIDConst::kShangHai) ? "SSE" : "SZSE";
...
ZTSOrderInfo* TraderATP::makeOrderInfo(const ATPRspOrderStatusAckMsg *order_status_ack)
{
...
ZTSTradeInfo* TraderATP::makeTradeInfo(const APITradeOrderUnit* trade_info)
{
	...
	ZTSTradeInfo *pRet = ZTSTradeInfo::create(code.c_str(), exchg.c_str());
	...
	ZTSDirectionType dType = wrapATPSide(trade_info->side);
...
ZTSTradeInfo* TraderATP::makeTradeRecord(const ATPRspCashAuctionTradeERMsg *cash_auction_trade_er)
{
	...
	ZTSDirectionType dType = wrapATPSide(cash_auction_trade_er->side);
	pRet->setDirection(dType);
...
{
	if (_sink)
		_sink->handleEvent(ZTE_Connect, 0);
...
{
	if (_sink)
		_sink->handleEvent(ZTE_Logout, 0);
...
{
	if (_sink)
		_sink->handleEvent(ZTE_Connect, -1);
...
void TraderATP::OnRspOrderStatusInternalAck(const ATPRspOrderStatusAckMsg& order_status_ack)
{
	...
	{
		ZTSEntrust* entrust = makeEntrust(&order_status_ack);
		...
		if (order_status_ack.orig_cl_ord_no != 0)  // 可以根据orig_cl_ord_no是否为0来判断是否为撤单委托，需要注意撤单传入的orig_cl_ord_no不能为0
		{
			...
			ZTSError* error = ZTSError::create(WEC_ORDERCANCEL, order_status_ack.ord_rej_reason);
		...
		else
		{
			...
			ZTSError* error = ZTSError::create(WEC_ORDERINSERT, order_status_ack.ord_rej_reason);
			_sink->onRspEntrust(entrust, error);
			error->release();
	...
	else
	{
		...
		ZTSOrderInfo *orderInfo = makeOrderInfo(&order_status_ack);
		if (orderInfo)
		{
			//if (_sink)
...
void TraderATP::OnRspCashAuctionTradeER(const ATPRspCashAuctionTradeERMsg& cash_auction_trade_er)
{
	...
	ZTSTradeInfo *tRecord = makeTradeRecord(&cash_auction_trade_er);
	if (tRecord)
	{
...
void TraderATP::OnRspOrderQueryResult(const ATPRspOrderQueryResultMsg &msg)
{
	...
	if (ayOrders == NULL)
		ayOrders = ZTSArray::create();

	for (const auto& unit : msg.order_array)
	{
		ZTSOrderInfo* ordInfo = makeOrderInfo(&unit);
		if (ordInfo == NULL)
			continue;
...
void TraderATP::OnRspTradeOrderQueryResult(const ATPRspTradeOrderQueryResultMsg &msg)
{
	...
	for (const auto& unit : msg.order_array)
	{
		ZTSTradeInfo* trdInfo = makeTradeInfo(&unit);
		if (trdInfo == NULL)
			continue;
...
void TraderATP::OnRspShareQueryResult(const ATPRspShareQueryResultMsg &msg)
{
	...
	for (auto &unit : msg.order_array) {
		...
		ZTSContractInfo* contract = _bd_mgr->getContract(unit.security_id, exchg.c_str());
		if (contract)
		{
			ZTSCommodityInfo* commInfo = contract->getCommInfo();
			std::string key = fmt::format("{}-{}", unit.security_id, exchg.c_str());
			ZTSPositionItem* pos = (ZTSPositionItem*)_positions->get(key);
			if (pos == NULL)
			{
				pos = ZTSPositionItem::create(unit.security_id, commInfo->getCurrency(), commInfo->getExchg());
...
bool TraderATP::init(ZTSVariant *params)
{
...
int TraderATP::orderInsert(ZTSEntrust* entrust)
{
...
int TraderATP::orderAction(ZTSEntrustAction* action)
{
	if (_api == NULL || _state != TS_ALLREADY)
	{
		return -1;
