...
void VvtDtRunner::start(bool bAsync /* = false */, bool bAlldayMode /* = false */)
{
	_parsers.run();
...
void VvtDtRunner::initialize(const char* cfgFile, const char* logCfg, const char* modDir /* = "" */, bool bCfgFile /* = true */, bool bLogCfgFile /* = true */)
{
...
void VvtDtRunner::initDataMgr(VvTSVariant* config, bool bAlldayMode /* = false */)
{
...
void VvtDtRunner::registerParserPorter(FuncParserEvtCallback cbEvt, FuncParserSubCallback cbSub)
{
...
void VvtDtRunner::parser_init(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_INIT, id);
}

void VvtDtRunner::parser_connect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_CONNECT, id);
}

void VvtDtRunner::parser_disconnect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_DISCONNECT, id);
}

void VvtDtRunner::parser_release(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_RELEASE, id);
}

void VvtDtRunner::parser_subscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, true);
}

void VvtDtRunner::parser_unsubscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, false);
}

void VvtDtRunner::on_ext_parser_quote(const char* id, VvTSTickStruct* curTick, uint32_t uProcFlag)
{
...
bool VvtDtRunner::createExtParser(const char* id)
{
	ParserAdapterPtr adapter(new ParserAdapter(&_bd_mgr, &_data_mgr, &_idx_factory));
...
bool VvtDtRunner::createExtDumper(const char* id)
{
...
void VvtDtRunner::registerExtDumper(FuncDumpBars barDumper, FuncDumpTicks tickDumper)
{
	_dumper_for_bars = barDumper;
	_dumper_for_ticks = tickDumper;
}

void VvtDtRunner::registerExtHftDataDumper(FuncDumpOrdQue ordQueDumper, FuncDumpOrdDtl ordDtlDumper, FuncDumpTrans transDumper)
{
...
bool VvtDtRunner::dumpHisTicks(const char* id, const char* stdCode, uint32_t uDate, VvTSTickStruct* ticks, uint32_t count)
{
...
bool VvtDtRunner::dumpHisBars(const char* id, const char* stdCode, const char* period, VvTSBarStruct* bars, uint32_t count)
{
...
bool VvtDtRunner::dumpHisOrdDtl(const char* id, const char* stdCode, uint32_t uDate, VvTSOrdDtlStruct* items, uint32_t count)
{
...
bool VvtDtRunner::dumpHisOrdQue(const char* id, const char* stdCode, uint32_t uDate, VvTSOrdQueStruct* items, uint32_t count)
{
...
bool VvtDtRunner::dumpHisTrans(const char* id, const char* stdCode, uint32_t uDate, VvTSTransStruct* items, uint32_t count)
{
