...
bool ParserUDP::init( ZTSVariant* config )
{
...
void ParserUDP::extract_buffer(uint32_t length, bool isBroad /* = true */)
{
	...
	else if (header->_type == UDP_MSG_PUSHORDDTL)
	{
		...
		if (it != _set_subs.end())
		{
			ZTSOrdDtlData* curData = ZTSOrdDtlData::create(packet->_data);
			if (_sink)
	...
	else if (header->_type == UDP_MSG_PUSHORDQUE)
	{
		...
		if (it != _set_subs.end())
		{
			ZTSOrdQueData* curData = ZTSOrdQueData::create(packet->_data);
			if (_sink)
	...
	else if (header->_type == UDP_MSG_PUSHTRANS)
	{
		...
		if (it != _set_subs.end())
		{
			ZTSTransData* curData = ZTSTransData::create(packet->_data);
			if (_sink)
