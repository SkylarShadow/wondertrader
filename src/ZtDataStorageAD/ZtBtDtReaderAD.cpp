...
inline void pipe_btreader_log(IBtDtReaderSink* sink, ZTSLogLevel ll, const char* format, const Args&... args)
{
	if (sink == NULL)
		return;
...
extern "C"
{
	...
	{
		IBtDtReader* ret = new ZtBtDtReaderAD();
		return ret;
	}
...
void ZtBtDtReaderAD::init(ZTSVariant* cfg, IBtDtReaderSink* sink)
{
	_sink = sink;
	...
	pipe_btreader_log(_sink, LL_INFO, "ZtBtDtReaderAD initialized, root data dir is {}", _base_dir);
}

bool ZtBtDtReaderAD::read_raw_bars(const char* exchg, const char* code, ZTSKlinePeriod period, std::string& buffer)
{
	//直接从LMDB读取
	...
	int cnt = query.get_range(std::string((const char*)&lKey, sizeof(lKey)), std::string((const char*)&rKey, sizeof(rKey)),
		...
		auto szUnit = sizeof(ZTSBarStruct);
...
bool ZtBtDtReaderAD::read_raw_ticks(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	//直接从LMDB读取
	ZtLMDBPtr db = get_t_db(exchg, code);
	if (db == NULL)
		return false;
	...
	int cnt = query.get_range(std::string((const char*)&lKey, sizeof(lKey)), std::string((const char*)&rKey, sizeof(rKey)),
		...
		std::size_t cnt = ayVals.size();
		auto szUnit = sizeof(ZTSTickStruct);
		buffer.resize(szUnit*cnt);
		char* cursor = (char*)buffer.data();
		for (const std::string& item : ayVals)
		{
...
ZtBtDtReaderAD::ZtLMDBPtr ZtBtDtReaderAD::get_k_db(const char* exchg, ZTSKlinePeriod period)
{
...
ZtBtDtReaderAD::ZtLMDBPtr ZtBtDtReaderAD::get_t_db(const char* exchg, const char* code)
{
