...
extern "C"
{
	...
	{
		IDataWriter* ret = new ZtDataWriterAD();
		return ret;
	}
...
bool ZtDataWriterAD::init(ZTSVariant* params, IDataWriterSink* sink)
{
...
void* ZtDataWriterAD::resizeRTBlock(BoostMFPtr& mfPtr, uint32_t nCount)
{
	if (mfPtr == NULL)
		return NULL;
...
bool ZtDataWriterAD::writeTick(ZTSTickData* curTick, uint32_t procFlag)
{
	if (curTick == NULL)
		return false;
...
void ZtDataWriterAD::pushTask(TaskInfo task)
{
	if(_async_task)
	{
...
void ZtDataWriterAD::pipeToTicks(ZTSContractInfo* ct, ZTSTickData* curTick)
{
	//直接落地
	ZtLMDBPtr db = get_t_db(ct->getExchg(), ct->getCode());
	if (db)
	{
		...
		if (!query.put_and_commit((void*)&key, sizeof(key), &curTick->getTickStruct(), sizeof(ZTSTickStruct)))
		{
...
void ZtDataWriterAD::pipeToDayBars(ZTSContractInfo* ct, const ZTSBarStruct& bar)
{
	//直接落地
	ZtLMDBPtr db = get_k_db(ct->getExchg(), KP_DAY);
	if (db)
	{
		LMDBBarKey key(ct->getExchg(), ct->getCode(), bar.date);
		ZtLMDBQuery query(*db);
		if (!query.put_and_commit((void*)&key, sizeof(key), (void*)&bar, sizeof(ZTSBarStruct)))
		{
	...
	for (auto& item : _dumpers)
	{
		...
		bool bSucc = dumper->dumpHisBars(ct->getFullCode(), "d1", (ZTSBarStruct*)&bar, 1);
		if (!bSucc)
		{
...
void ZtDataWriterAD::pipeToM1Bars(ZTSContractInfo* ct, const ZTSBarStruct& bar)
{
	//直接落地
	ZtLMDBPtr db = get_k_db(ct->getExchg(), KP_Minute1);
	if(db)
	{
	...
	for (auto& item : _dumpers)
	{
		...
		bool bSucc = dumper->dumpHisBars(ct->getFullCode(), "m1", (ZTSBarStruct*)&bar, 1);
		if (!bSucc)
		{
...
void ZtDataWriterAD::pipeToM5Bars(ZTSContractInfo* ct, const ZTSBarStruct& bar)
{
	//直接落地
	ZtLMDBPtr db = get_k_db(ct->getExchg(), KP_Minute5);
	if (db)
	{
		LMDBBarKey key(ct->getExchg(), ct->getCode(), (uint32_t)bar.time);
		ZtLMDBQuery query(*db);
	...
	for (auto& item : _dumpers)
	{
		...
		bool bSucc = dumper->dumpHisBars(ct->getFullCode(), "m5", (ZTSBarStruct*)&bar, 1);
		if (!bSucc)
		{
...
void ZtDataWriterAD::updateBarCache(ZTSContractInfo* ct, ZTSTickData* curTick)
{
	uint32_t uDate = curTick->actiondate();
	ZTSSessionInfo* sInfo = _bd_mgr->getSessionByCode(curTick->code(), curTick->exchg());
	...
	if (!_disable_day && _d1_cache._cache_block)
	{
		...
		ZTSBarStruct* lastBar = &item._bar;
...
ZTSTickData* ZtDataWriterAD::getCurTick(const char* code, const char* exchg/* = ""*/)
{
	if (strlen(code) == 0)
		return NULL;
...
bool ZtDataWriterAD::updateTickCache(ZTSContractInfo* ct, ZTSTickData* curTick, uint32_t procFlag)
{
...
ZtDataWriterAD::ZtLMDBPtr ZtDataWriterAD::get_k_db(const char* exchg, ZTSKlinePeriod period)
{
	...
	ZtLMDBPtr dbPtr(new ZtLMDB(false));
...
ZtDataWriterAD::ZtLMDBPtr ZtDataWriterAD::get_t_db(const char* exchg, const char* code)
{
