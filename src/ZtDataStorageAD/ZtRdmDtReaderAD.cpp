...
extern "C"
{
	...
	{
		IRdmDtReader* ret = new ZtRdmDtReaderAD();
		return ret;
	}
...
ZtRdmDtReaderAD::ZtRdmDtReaderAD()
	: _base_data_mgr(NULL)
	, _hot_mgr(NULL)
{
}
...
void ZtRdmDtReaderAD::init(ZTSVariant* cfg, IRdmDtReaderSink* sink)
{
	...
	pipe_rdmreader_log(sink, LL_INFO, "ZtRdmDtReaderAD initialized, root data folder is {}", _base_dir);
}

ZTSTickSlice* ZtRdmDtReaderAD::readTickSliceByCount(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	//TODO: 以后再来实现吧
	return NULL;
}

ZTSTickSlice* ZtRdmDtReaderAD::readTickSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
	...
	if (isEmpty)
	{
		...
		if (cnt > 0)
		{
			ZTSTickStruct& curTs = tickList._ticks.front();
	...
	else
	{
		if (bNeedOlder)
		{
			...
			int cnt = query.get_range(std::string((const char*)&lKey, sizeof(lKey)), std::string((const char*)&rKey, sizeof(rKey)),
				...
				memcpy(&ayTicks[idx], tickList._ticks.data(), sizeof(ZTSTickStruct)*tickList._ticks.size());
				tickList._ticks.swap(ayTicks);
			});

			if(cnt > 0)
			{
				const ZTSTickStruct& curTs = tickList._ticks.front();
		...
		if (bNeedNewer)
		{
			...
			if (cnt > 0)
			{
				const ZTSTickStruct& curTs = tickList._ticks.back();
	...
	//全部读取完成以后，再生成切片
	{
		...
		ZTSTickStruct* pTick = std::lower_bound(&tickList._ticks[0], &tickList._ticks[0] + (tickList._ticks.size() - 1), eTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
		...
		{
			pTick = std::lower_bound(&tickList._ticks[0], &tickList._ticks[0] + (tickList._ticks.size() - 1), sTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
		...
		ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, pTick, cnt);
		return slice;
	}
}

ZTSKlineSlice* ZtRdmDtReaderAD::readKlineSliceByCount(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	//TODO: 以后再来实现吧
	return NULL;
}

ZTSTickSlice* ZtRdmDtReaderAD::readTickSliceByDate(const char* stdCode, uint32_t uDate )
{
	//TODO: 以后再来实现吧
	return NULL;
}

ZTSKlineSlice* ZtRdmDtReaderAD::readKlineSliceByRange(const char* stdCode, ZTSKlinePeriod period, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
	...
	//
	{
		...
		ZTSBarStruct* pHead = NULL;
		std::size_t cnt = 0;

		ZTSBarStruct* pBar = std::lower_bound(&barsList._bars[0], &barsList._bars[0] + (barsList._bars.size() - 1), eBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
			if (isDay)
		...
		{
			pBar = std::lower_bound(&barsList._bars[0], &barsList._bars[0] + (barsList._bars.size() - 1), sBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
				if (isDay)
					return a.date < b.date;
				else
					return a.time < b.time;
		...
		ZTSKlineSlice* slice = ZTSKlineSlice::create(stdCode, period, 1, pHead, cnt);
		return slice;
	}
}

ZtRdmDtReaderAD::ZtLMDBPtr ZtRdmDtReaderAD::get_k_db(const char* exchg, ZTSKlinePeriod period)
{
...
ZtRdmDtReaderAD::ZtLMDBPtr ZtRdmDtReaderAD::get_t_db(const char* exchg, const char* code)
{
