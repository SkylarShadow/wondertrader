...
extern "C"
{
	...
	{
		IDataReader* ret = new ZtDataReaderAD();
		return ret;
	}
...
void ZtDataReaderAD::init(ZTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader /* = NULL */)
{
	...
	pipe_reader_log(sink, LL_INFO, "ZtDataReaderAD initialized, root data folder is {}", _base_dir);
}

ZTSTickSlice* ZtDataReaderAD::readTickSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	//这里要改成从lmdb读取
	ZtLMDBPtr db = get_t_db(cInfo._exchg, cInfo._code);
	if (db == NULL)
		return NULL;
	...
	if(reload_flag == 1)
	{
		...
		int cnt = query.get_range(std::string((const char*)&lKey, sizeof(lKey)), 
			...
			{
				ZTSTickStruct* curTick = (ZTSTickStruct*)item.data();
	...
	if(cnt_2 >= count)
	{
		...
		return ZTSTickSlice::create(stdCode, &tickList._ticks[ayTwo.second - count], count);
	}
	else
	{
		...
		auto ret = ZTSTickSlice::create(stdCode, &tickList._ticks[ayOne.second - diff], diff);
		if(cnt_2 > 0)
...
std::string ZtDataReaderAD::read_bars_to_buffer(const char* exchg, const char* code, ZTSKlinePeriod period)
{
	//直接从LMDB读取
	ZtLMDBPtr db = get_k_db(exchg, period);
	if (db == NULL)
		return "";
	...
	query.get_range(std::string((const char*)&lKey, sizeof(lKey)),
		...
		buffer.resize(sizeof(ZTSBarStruct)*ayVals.size());
		memcpy((void*)buffer.data(), ayVals.data(), sizeof(ZTSBarStruct)*ayVals.size());
	});
	return std::move(buffer);
}

bool ZtDataReaderAD::cacheBarsFromStorage(const std::string& key, const char* stdCode, ZTSKlinePeriod period, uint32_t count)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
	...
	//直接从LMDB读取
	ZtLMDBPtr db = get_k_db(cInfo._exchg, period);
	if (db == NULL)
		return false;
	...
	int cnt = query.get_lowers(std::string((const char*)&lKey, sizeof(lKey)), std::string((const char*)&rKey, sizeof(rKey)),
		...
		for (std::size_t i = 0; i < cnt; i++)
		{
			...
			barList._bars.push_back(*(ZTSBarStruct*)ayVals[i].data());
		}
	});
...
void ZtDataReaderAD::update_cache_from_lmdb(BarsList& barsList, const char* exchg, const char* code, ZTSKlinePeriod period, uint32_t& lastBarTime)
{
...
ZTSBarStruct* ZtDataReaderAD::get_rt_cache_bar(const char* exchg, const char* code, ZTSKlinePeriod period)
{
...
ZTSKlineSlice* ZtDataReaderAD::readKlineSlice(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
	...
	if(barsList._last_req_time < etime)
	{
		...
		ZTSBarStruct& lastBar = barsList._bars.back();
		...
		if(lastBarTime < etime)
		{
			ZTSBarStruct* rtBar = get_rt_cache_bar(cInfo._exchg, curCode.c_str(), period);
			if(rtBar != NULL)
			{
	...
	{
		//如果array_two条数足够，则直接返回数据块
		return ZTSKlineSlice::create(stdCode, period, 1, &barsList._bars[ayTwo.second - count], count);
	}
	else
	{
		//如果array_two条数不够，需要再从array_one提取
		...
		auto diff = count - cnt_2;
		auto ret = ZTSKlineSlice::create(stdCode, period, 1, &barsList._bars[ayOne.second - diff], diff);
		if (cnt_2 > 0)
			ret->appendBlock(ayTwo.first, cnt_2);
		return ret;
	}
...
void ZtDataReaderAD::onMinuteEnd(uint32_t uDate, uint32_t uTime, uint32_t endTDate /* = 0 */)
{
	//这里应该触发检查
	...
	for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++)
	{
		...
		if (barsList._period != KP_DAY)
		{
			...
			{
				ZTSBarStruct* rtBar = get_rt_cache_bar(cInfo._exchg, cInfo._code, barsList._period);
...
ZtDataReaderAD::ZtLMDBPtr ZtDataReaderAD::get_k_db(const char* exchg, ZTSKlinePeriod period)
{
	ZtLMDBMap* the_map = NULL;
	std::string subdir;
	...
	else if (period == KP_DAY)
	{
		...
		return std::move(ZtLMDBPtr());

	auto it = the_map->find(exchg);
	if (it != the_map->end())
		return std::move(it->second);

	ZtLMDBPtr dbPtr(new ZtLMDB(true));
...
ZtDataReaderAD::ZtLMDBPtr ZtDataReaderAD::get_t_db(const char* exchg, const char* code)
{
