...
#include "../VvtBtCore/ExecMocker.h"
#include "../VvtBtCore/VvtHelper.h"
...
bool VvtBtRunner::loadRawHisBars(void* obj, const char* stdCode, VvTSKlinePeriod period, FuncReadBars cb)
{
...
bool VvtBtRunner::loadFinalHisBars(void* obj, const char* stdCode, VvTSKlinePeriod period, FuncReadBars cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool VvtBtRunner::loadAllAdjFactors(void* obj, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool VvtBtRunner::loadAdjFactors(void* obj, const char* stdCode, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
	if (_ext_adj_fct_loader == NULL)
		return false;
...
bool VvtBtRunner::loadRawHisTicks(void* obj, const char* stdCode, uint32_t uDate, FuncReadTicks cb)
{
	StdUniqueLock lock(_feed_mtx);
...
void VvtBtRunner::feedRawBars(VvTSBarStruct* bars, uint32_t count)
{
...
void VvtBtRunner::feedAdjFactors(const char* stdCode, uint32_t* dates, double* factors, uint32_t count)
{
	if(_ext_adj_fct_loader == NULL)
	{
		VvTSLogger::error("Cannot feed adjusting factors because of no extented adjusting factor loader registered.");
		return;
	}
...
void VvtBtRunner::feedRawTicks(VvTSTickStruct* ticks, uint32_t count)
{
	if (_ext_tick_loader == NULL)
	{
		VvTSLogger::error("Cannot feed ticks because of no extented tick loader registered.");
		return;
	}
...
void VvtBtRunner::registerCtaCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, 
...
void VvtBtRunner::registerSelCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, 
...
void VvtBtRunner::registerHftCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraBarCallback cbBar,
...
uint32_t VvtBtRunner::initCtaMocker(const char* name, int32_t slippage /* = 0 */, bool hook /* = false */, 
...
uint32_t VvtBtRunner::initHftMocker(const char* name, bool hook/* = false*/)
{
	if (_hft_mocker)
	{
		delete _hft_mocker;
...
uint32_t VvtBtRunner::initSelMocker(const char* name, uint32_t date, uint32_t time, const char* period, 
...
void VvtBtRunner::ctx_on_bar(uint32_t id, const char* stdCode, const char* period, VvTSBarStruct* newBar, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_calc_done(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_init(uint32_t id, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_cond_triggered(uint32_t id, const char* stdCode, double target, double price, const char* usertag, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate, bool isBegin /* = true */, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtBtRunner::ctx_on_tick(uint32_t id, const char* stdCode, VvTSTickData* newTick, EngineType eType/*= ET_CTA*/)
{
	switch (eType)
	{
...
void VvtBtRunner::hft_on_order_queue(uint32_t id, const char* stdCode, VvTSOrdQueData* newOrdQue)
{
	if (_cb_hft_ordque)
		_cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void VvtBtRunner::hft_on_order_detail(uint32_t id, const char* stdCode, VvTSOrdDtlData* newOrdDtl)
{
	if (_cb_hft_orddtl)
		_cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void VvtBtRunner::hft_on_transaction(uint32_t id, const char* stdCode, VvTSTransData* newTrans)
{
	if (_cb_hft_trans)
		_cb_hft_trans(id, stdCode, &newTrans->getTransStruct());
}

void VvtBtRunner::hft_on_channel_ready(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, 1000/*CHNL_EVENT_READY*/);
}

void VvtBtRunner::hft_on_entrust(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool bSuccess, const char* message, const char* userTag)
{
	if (_cb_hft_entrust)
		_cb_hft_entrust(cHandle, localid, stdCode, bSuccess, message, userTag);
}

void VvtBtRunner::hft_on_order(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	if (_cb_hft_ord)
		_cb_hft_ord(cHandle, localid, stdCode, isBuy, totalQty, leftQty, price, isCanceled, userTag);
}

void VvtBtRunner::hft_on_trade(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool isBuy, double vol, double price, const char* userTag)
{
	if (_cb_hft_trd)
		_cb_hft_trd(cHandle, localid, stdCode, isBuy, vol, price, userTag);
}

void VvtBtRunner::init(const char* logProfile /* = "" */, bool isFile /* = true */, const char* outDir/* = "./outputs_bt"*/)
{
#ifdef _MSC_VER
	CMiniDumper::Enable(getModuleName(), true, VvtHelper::getCWD().c_str());
#endif

	VvTSLogger::init(logProfile, isFile);

	VvtHelper::setInstDir(getBinDir());
	VvtHelper::setOutputDir(outDir);
}

void VvtBtRunner::config(const char* cfgFile, bool isFile /* = true */)
{
	if(_inited)
	{
		VvTSLogger::error("VvtBtEngine has already been inited");
		return;
	}
	...
	if(_cfg == NULL)
	{
		VvTSLogger::error("Loading config failed");
		return;
	}
...
void VvtBtRunner::run(bool bNeedDump /* = false */, bool bAsync /* = false */)
{
	if (_running)
		return;
	...
	VvTSLogger::info("Backtesting will run in {} mode", _async ? "async" : "sync");
	...
	else
	{
		_worker.reset(new StdThread([this, bNeedDump]() {
			...
			{
				VvTSLogger::error("Exception raised while worker running");
				//print_stack_trace([](const char* message) {
				//	VvTSLogger::error(message);
				//});
			}
			VvTSLogger::debug("Worker thread of backtest finished");
			_running = false;
...
void VvtBtRunner::stop()
{
	...
	VvTSLogger::debug("Notify to finish last round");
	...
	VvTSLogger::debug("Last round ended");
	...
	VvTSLogger::freeAllDynLoggers();

	VvTSLogger::debug("Backtest stopped");
}

void VvtBtRunner::release()
{
	VvTSLogger::stop();
}

void VvtBtRunner::set_time_range(VvTUInt64 stime, VvTUInt64 etime)
{
	_replayer.set_time_range(stime, etime);

	VvTSLogger::info("Backtest time range is set to be [{},{}] mannually", stime, etime);
}

void VvtBtRunner::enable_tick(bool bEnabled /* = true */)
{
	_replayer.enable_tick(bEnabled);

	VvTSLogger::info("Tick data replaying is {}", bEnabled ? "enabled" : "disabled");
}

void VvtBtRunner::clear_cache()
{
	_replayer.clear_cache();
}

const char* VvtBtRunner::get_raw_stdcode(const char* stdCode)
{
...
bool VvtBtRunner::initEvtNotifier(VvTSVariant* cfg)
{
