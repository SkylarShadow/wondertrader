...
bool ParserCTPOpt::init(VvTSVariant* config)
{
...
{	
	...
	VvTSContractInfo* cInfo = m_pBaseDataMgr->getContract(pDepthMarketData->InstrumentID, pDepthMarketData->ExchangeID);
	if (cInfo == NULL)
		return;

	VvTSCommodityInfo* pCommInfo = cInfo->getCommInfo();
