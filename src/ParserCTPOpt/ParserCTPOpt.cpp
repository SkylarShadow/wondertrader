...
bool ParserCTPOpt::init(ZTSVariant* config)
{
...
{	
	...
	ZTSContractInfo* cInfo = m_pBaseDataMgr->getContract(pDepthMarketData->InstrumentID, pDepthMarketData->ExchangeID);
	if (cInfo == NULL)
		return;

	ZTSCommodityInfo* pCommInfo = cInfo->getCommInfo();
