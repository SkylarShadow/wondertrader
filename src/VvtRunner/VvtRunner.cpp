...
void VvtRunner::init(const std::string& filename)
{
...
bool VvtRunner::config(const std::string& filename)
{
...
bool VvtRunner::initCtaStrategies()
{
...
bool VvtRunner::initHftStrategies()
{
	VvTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != VvTSVariant::VT_Object)
		return false;
...
bool VvtRunner::initActionPolicy()
{
	return _act_policy.init(_config->getCString("bspolicy"));
}

bool VvtRunner::initDataMgr()
{
...
bool VvtRunner::initParsers(VvTSVariant* cfgParser)
{
	if (cfgParser == NULL)
		return false;
...
bool VvtRunner::initExecuters(VvTSVariant* cfgExecuter)
{
...
bool VvtRunner::initTraders(VvTSVariant* cfgTrader)
{
...
void VvtRunner::run(bool bAsync /* = false */)
{
	try
	{
		_parsers.run();
...
void VvtRunner::handleLogAppend(VvTSLogLevel ll, const char* msg)
{
	_notifier.notify_log(LOG_TAGS[ll - 100], msg);
}

bool VvtRunner::initEvtNotifier()
{
