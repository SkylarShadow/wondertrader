...
void ZtRunner::init(const std::string& filename)
{
	...
	{
		ZTSLogger::warn("logging configure {} not exists", filename);
	}
}

bool ZtRunner::config(const std::string& filename)
{
	_config = ZTSCfgLoader::load_from_file(filename);
	if(_config == NULL)
	{
...
bool ZtRunner::initCtaStrategies()
{
	...
	std::string path = ZtHelper::getCWD() + "cta/";
...
bool ZtRunner::initHftStrategies()
{
	ZTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != ZTSVariant::VT_Object)
		return false;
	...
	std::string path = ZtHelper::getCWD() + "hft/";
...
bool ZtRunner::initEngine()
{
	...
	if (_is_hft)
	{
		ZTSLogger::info("Trading enviroment initialzied with engine: HFT");
		_hft_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_hft_engine;
	}
	else if (_is_sel)
	{
		ZTSLogger::info("Trading enviroment initialzied with engine: SEL");
		_sel_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_sel_engine;
	}
	else
	{
		ZTSLogger::info("Trading enviroment initialzied with engine: CTA");
...
bool ZtRunner::initActionPolicy()
{
	return _act_policy.init(_config->getCString("bspolicy"));
}

bool ZtRunner::initDataMgr()
{
...
bool ZtRunner::initParsers(ZTSVariant* cfgParser)
{
	if (cfgParser == NULL)
		return false;
...
bool ZtRunner::initExecuters(ZTSVariant* cfgExecuter)
{
	if (cfgExecuter == NULL || cfgExecuter->type() != ZTSVariant::VT_Array)
		return false;

	std::string path = ZtHelper::getCWD() + "executer/";
...
bool ZtRunner::initTraders(ZTSVariant* cfgTrader)
{
...
void ZtRunner::run(bool bAsync /* = false */)
{
	try
	{
		_parsers.run();
...
void ZtRunner::handleLogAppend(ZTSLogLevel ll, const char* msg)
{
	_notifier.notify_log(LOG_TAGS[ll - 100], msg);
}

bool ZtRunner::initEvtNotifier()
{
