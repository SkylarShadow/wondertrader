...
VvtUftRtTicker::VvtUftRtTicker(VvtUftEngine* engine)
	: _engine(engine)
	, _stopped(false)
	, _date(0)
	, _time(UINT_MAX)
...
VvtUftRtTicker::~VvtUftRtTicker()
{
}

void VvtUftRtTicker::init(const char* sessionID)
{
...
void VvtUftRtTicker::on_tick(VvTSTickData* curTick)
{
	if (_thrd == NULL)
	{
		if (_engine)
