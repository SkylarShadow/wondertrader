...
bool UftStrategyMgr::loadFactories(const char* path)
{
	...
	{
		VvTSLogger::error("Directory {} of UFT strategy factory not exists", path);
		return false;
	}
	...
	for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++)
	{
		...
		if (pFact != NULL)
		{
			...
			fInfo._fact = pFact;
			VvTSLogger::info("UFT strategy factory[{}] loaded", pFact->getName());
	...
	VvTSLogger::info("{} UFT strategy factories in directory[{}] loaded", count, path);
