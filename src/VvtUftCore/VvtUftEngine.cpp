...
void VvtUftEngine::set_date_time(uint32_t curDate, uint32_t curTime, uint32_t curSecs /* = 0 */, uint32_t rawTime /* = 0 */)
{
	_cur_date = curDate;
	_cur_time = curTime;
	_cur_secs = curSecs;
...
void VvtUftEngine::set_trading_date(uint32_t curTDate)
{
	_cur_tdate = curTDate;
...
VvTSCommodityInfo* VvtUftEngine::get_commodity_info(const char* stdCode)
{
...
VvTSContractInfo* VvtUftEngine::get_contract_info(const char* stdCode)
{
	const StringVector& ay = StrUtil::split(stdCode, ".");
	return _base_data_mgr->getContract(ay[1].c_str(), ay[0].c_str());
}

VvTSSessionInfo* VvtUftEngine::get_session_info(const char* sid, bool isCode /* = false */)
{
	if (!isCode)
...
VvTSTickSlice* VvtUftEngine::get_tick_slice(uint32_t sid, const char* code, uint32_t count)
{
	return NULL;
	return _data_mgr->get_tick_slice(code, count);
}

VvTSTickData* VvtUftEngine::get_last_tick(uint32_t sid, const char* stdCode)
{
	return _data_mgr->grab_last_tick(stdCode);
}

VvTSKlineSlice* VvtUftEngine::get_kline_slice(uint32_t sid, const char* stdCode, const char* period, uint32_t count, uint32_t times /* = 1 */, uint64_t etime /* = 0 */)
{
	return NULL;
...
void VvtUftEngine::sub_tick(uint32_t sid, const char* stdCode)
{
	SubList& sids = _tick_sub_map[stdCode];
	sids.insert(sid);
}

double VvtUftEngine::get_cur_price(const char* stdCode)
{
...
void VvtUftEngine::notify_params_update(const char* name)
{
	for(auto& v : _ctx_map)
	{
...
void VvtUftEngine::init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtUftDtMgr* dataMgr, EventNotifier* notifier)
{
...
void VvtUftEngine::run()
{
	...
	_tm_ticker = new VvtUftRtTicker(this);
...
void VvtUftEngine::handle_push_quote(VvTSTickData* newTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(newTick);
}

void VvtUftEngine::handle_push_order_detail(VvTSOrdDtlData* curOrdDtl)
{
...
void VvtUftEngine::handle_push_order_queue(VvTSOrdQueData* curOrdQue)
{
...
void VvtUftEngine::handle_push_transaction(VvTSTransData* curTrans)
{
...
void VvtUftEngine::sub_order_detail(uint32_t sid, const char* stdCode)
{
	SubList& sids = _orddtl_sub_map[stdCode];
	sids.insert(sid);
}

void VvtUftEngine::sub_order_queue(uint32_t sid, const char* stdCode)
{
	SubList& sids = _ordque_sub_map[stdCode];
	sids.insert(sid);
}

void VvtUftEngine::sub_transaction(uint32_t sid, const char* stdCode)
{
	SubList& sids = _trans_sub_map[stdCode];
	sids.insert(sid);
}

void VvtUftEngine::on_session_begin()
{
...
void VvtUftEngine::on_session_end()
{
	for (auto it = _ctx_map.begin(); it != _ctx_map.end(); it++)
	{
		UftContextPtr& ctx = (UftContextPtr&)it->second;
...
void VvtUftEngine::on_tick(const char* stdCode, VvTSTickData* curTick)
{
	if(_data_mgr)
...
void VvtUftEngine::on_bar(const char* stdCode, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
	std::string key = fmt::format("{}-{}-{}", stdCode, period, times);
...
void VvtUftEngine::on_minute_end(uint32_t curDate, uint32_t curTime)
{

}

void VvtUftEngine::addContext(UftContextPtr ctx)
{
	uint32_t sid = ctx->id();
	_ctx_map[sid] = ctx;
}

UftContextPtr VvtUftEngine::getContext(uint32_t id)
{
...
VvTSOrdQueSlice* VvtUftEngine::get_order_queue_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_queue_slice(code, count);
}

VvTSOrdDtlSlice* VvtUftEngine::get_order_detail_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_detail_slice(code, count);
}

VvTSTransSlice* VvtUftEngine::get_transaction_slice(uint32_t sid, const char* code, uint32_t count)
{
