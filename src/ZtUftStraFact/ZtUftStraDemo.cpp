...
ZtUftStraDemo::ZtUftStraDemo(const char* id)
	: UftStrategy(id)
	, _last_tick(NULL)
...
ZtUftStraDemo::~ZtUftStraDemo()
{
	if (_last_tick)
		_last_tick->release();
}

const char* ZtUftStraDemo::getName()
{
	return "UftDemoStrategy";
}

const char* ZtUftStraDemo::getFactName()
{
	return FACT_NAME;
}

bool ZtUftStraDemo::init(ZTSVariant* cfg)
{
	//这里演示一下外部传入参数的获取
...
void ZtUftStraDemo::on_entrust(uint32_t localid, bool bSuccess, const char* message)
{
	if(!bSuccess)
	{
...
void ZtUftStraDemo::on_init(IUftStraCtx* ctx)
{
...
void ZtUftStraDemo::on_tick(IUftStraCtx* ctx, const char* code, ZTSTickData* newTick)
{	
...
void ZtUftStraDemo::check_orders()
{
...
void ZtUftStraDemo::on_bar(IUftStraCtx* ctx, const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	
}

void ZtUftStraDemo::on_trade(IUftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double qty, double price)
{
	
}

void ZtUftStraDemo::on_position(IUftStraCtx* ctx, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	if (_code != stdCode)
		return;
...
void ZtUftStraDemo::on_order(IUftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double totalQty, double leftQty, double price, bool isCanceled)
{
	//如果不是我发出去的订单,我就不管了
	auto it = _orders.find(localid);
	if (it == _orders.end())
...
void ZtUftStraDemo::on_channel_ready(IUftStraCtx* ctx)
{
...
void ZtUftStraDemo::on_channel_lost(IUftStraCtx* ctx)
{
	_channel_ready = false;
}

void ZtUftStraDemo::on_params_updated()
{
	//ctx->watch_param("second", _secs);
	//ctx->watch_param("freq", _freq);
	//ctx->watch_param("offset", _offset);
