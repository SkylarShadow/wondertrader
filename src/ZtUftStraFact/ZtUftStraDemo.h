...
class ZtUftStraDemo : public UftStrategy
{
public:
	ZtUftStraDemo(const char* id);
	~ZtUftStraDemo();
...
public:
	...
	virtual void on_tick(IUftStraCtx* ctx, const char* code, ZTSTickData* newTick) override;

	virtual void on_bar(IUftStraCtx* ctx, const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar) override;
