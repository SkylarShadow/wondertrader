...
namespace hft
{
	...
	public:
		void	run(uint32_t times)
		{
			...
			for (uint32_t i = 0; i < times; i++)
			{
				...
				ZTSContractInfo* contract = _bd_mgr->getContract("rb2205", "SHFE");
				if (contract == NULL)
					return;
				...
				ZTSTickData* tick = ZTSTickData::create("rb2205");
			...
			ZTSLogger::warn("{} ticks simulated in {:.0f} ns, HftEngine Innner Latency: {:.3f} ns", times, total*1.0, t2t);
		}
	...
	bool HftLatencyTool::init()
	{
		...
		ZTSVariant* _config = ZTSCfgLoader::load_from_file("config.yaml");
		if (_config == NULL)
		{
			ZTSLogger::log_raw(LL_ERROR, "Loading config file config.yaml failed");
			return false;
		}
		...
		if (cfgBF->get("hot"))
		{
			...
			ZTSLogger::log_raw(LL_INFO, "Hot rules loades");
		}

		_act_mgr.init("actpolicy.yaml");

		_times = _config->getUInt32("times");
		ZTSLogger::warn("{} ticks will be simulated", _times);

		_core = _config->getUInt32("core");
		ZTSLogger::warn("Testing thread will be bind to core {}", _core);
	...
	bool HftLatencyTool::initEngine(ZTSVariant* cfg)
	{
		ZTSLogger::warn("Trading enviroment initialzied with engine: HFT");
	...
	void HftLatencyTool::run()
	{
		if (_core != 0)
		{
			...
			{
				ZTSLogger::error("Binding to core {} failed", _core);
			}
		}
