...
VvtStraDualThrust::VvtStraDualThrust(const char* id)
	: CtaStrategy(id)
{
}


VvtStraDualThrust::~VvtStraDualThrust()
{
}

const char* VvtStraDualThrust::getFactName()
{
	return FACT_NAME;
}

const char* VvtStraDualThrust::getName()
{
	return "DualThrust";
}

bool VvtStraDualThrust::init(VvTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
void VvtStraDualThrust::on_session_begin(ICtaStraCtx* ctx, uint32_t uTDate)
{
...
void VvtStraDualThrust::on_schedule(ICtaStraCtx* ctx, uint32_t curDate, uint32_t curTime)
{
...
void VvtStraDualThrust::on_init(ICtaStraCtx* ctx)
{
	std::string code = _code;
...
void VvtStraDualThrust::on_tick(ICtaStraCtx* ctx, const char* stdCode, VvTSTickData* newTick)
{
	//没有什么要处理
}
