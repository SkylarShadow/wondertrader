...
bool StateMonitor::initialize(const char* filename, ZTSBaseDataMgr* bdMgr, DataManager* dtMgr)
{
	_bd_mgr = bdMgr;
	_dt_mgr = dtMgr;
	...
	{
		ZTSLogger::error("State config file {} not exists", filename);
		return false;
	}

	ZTSVariant* config = ZTSCfgLoader::load_from_file(filename);
	if (config == NULL)
	{
		ZTSLogger::error("Loading state config failed");
		return false;
	}
	...
	{
		ZTSVariant* jItem = config->get(sid.c_str());

		ZTSSessionInfo* ssInfo = _bd_mgr->getSession(sid.c_str());
		if (ssInfo == NULL)
		{
			ZTSLogger::error("Trading session template [{}] not exists,state control rule skipped", sid);
			continue;
		}
		...
		if (pCommSet)
		{
			...
			for (auto it = pCommSet->begin(); it != pCommSet->end(); it++)
			{
				...
				{
					ZTSLogger::info("Instrument {} is in holiday", pid);
				}
			}
		}
	}
		...
		_thrd.reset(new StdThread([this](){
			...
			while (!_stopped)
			{
				...
				for (; it != _map.end(); it++)
				{
					...
					ZTSSessionInfo* sInfo = stateInfo->_sInfo;
					...
					case SS_ORIGINAL:
						{
							...
							{
								ZTSLogger::info("No corresponding instrument of trading session {}[{}], changed into holiday state", sInfo->name(), stateInfo->_session);
								stateInfo->_state = SS_Holiday;
							}

							if(isAllHoliday)
							{
								ZTSLogger::info("All instruments of trading session {}[{}] are in holiday, changed into holiday state", sInfo->name(), stateInfo->_session);
								stateInfo->_state = SS_Holiday;
							}
							else if (offTime >= offCloseTime)
							{
								stateInfo->_state = SS_CLOSED;
								ZTSLogger::info("Trading session {}[{}] stopped receiving data", sInfo->name(), stateInfo->_session);
							}
							else if (aucStartTime != -1 && offTime >= aucStartTime)
							{
								if (stateInfo->isInSections(offTime))
								{
									...
									ZTSLogger::info("Trading session {}[{}] started receiving data", sInfo->name(), stateInfo->_session);
								}
								else
								{
									...
									if(offTime < sInfo->getCloseTime(true))
									{
										...
										ZTSLogger::info("Trading session {}[{}] paused receiving data", sInfo->name(), stateInfo->_session);
									}
									else
							...
							else if (offTime >= offInitTime)
							{
								...
								ZTSLogger::info("Trading session {}[{}] initialized", sInfo->name(), stateInfo->_session);
							}
					...
					case SS_PAUSED:
						{
							...
							if (!isAllHoliday)
							{
								...
								{
									stateInfo->_state = SS_RECEIVING;
									ZTSLogger::info("Trading session {}[{}] continued to receive data", sInfo->name(), stateInfo->_session);
								}
							}
							else
							{
					...
					case SS_CLOSED:
						{
							...
							if (offTime >= offProcTime)
							{
								if(!_dt_mgr->isSessionProceeded(stateInfo->_session))
								{
									...
									ZTSLogger::info("Trading session {}[{}] started processing closing task", sInfo->name(), stateInfo->_session);
					...
					case SS_Holiday:
						{
							...
							if (offTime >= 0 && offTime < offInitTime)
							{
								...
								if(!isAllHoliday)
								{
									...
									ZTSLogger::info("Trading session {}[{}] state reset", sInfo->name(), stateInfo->_session);
								}
							}
						}
						break;
