...
bool IndexWorker::init(ZTSVariant* config)
{
	if (config == NULL)
		return false;
	...
	ZTSVariant* cfgComms = config->get("commodities");
	ZTSVariant* cfgCodes = config->get("codes");
	if (cfgComms != NULL && cfgComms->size() > 0)
	{
		...
		{
			ZTSVariant* cfgItem = cfgComms->get(i);
			std::string fullPid;
			double weight = 1.0;
			...
			//通过品种代码找到对应的合约列表，并订阅
			ZTSCommodityInfo* commInfo = bdMgr->getCommodity(fullPid.c_str());
			if (commInfo == NULL)
				continue;
			...
			for (const auto& c : codes)
			{
				...
				//订阅的时候读取最后的快照，作为基础数据
				ZTSTickData* lastTick = _factor->sub_ticks(fullCode.c_str());
				if (lastTick)
				{
					memcpy(&wFactor._tick, &lastTick->getTickStruct(), sizeof(ZTSTickStruct));
					lastTick->release();
				}

				ZTSLogger::info("Consist {} of block index {}.{} subscribed", fullCode, _exchg, _code);
			}
		}
	}
	else if (cfgCodes != NULL && cfgCodes->size() > 0)
	{
		...
		for (std::size_t i = 0; i < cnt; i++)
		{
			ZTSVariant* cfgItem = cfgCodes->get(i);
			std::string fullCode;
			double weight = 1.0;
			...
			else
			{
				...
				if (strlen(cInfo._ruletag) > 0)
				{
					...
					ZTSLogger::info("{} contract confirmed: {} -> {}.{}", cInfo._ruletag, fullCode, cInfo._exchg, code);
				}
				else
				{
					code = cInfo._code;
			...
			ZTSContractInfo* cInfo = _factor->get_bd_mgr()->getContract(code.c_str(), exchg.c_str());
			if(cInfo == NULL)
			{
				ZTSLogger::error("Consist {} of block index {}.{} not exists", fullCode, _exchg, _code);
				continue;
			}
	...
	ZTSLogger::info("Block index {}.{} initialized，weight algorithm: {}, trigger: {}, timeout: {}", _exchg, _code, WEIGHT_ALGS[_weight_alg], _trigger, _timeout);

	return true;
}

void IndexWorker::handle_quote(ZTSTickData* newTick)
{
...

	{
		...
		memcpy(&wFactor._tick, &newTick->getTickStruct(), sizeof(ZTSTickStruct));
	}
...
void IndexWorker::generate_tick()
{
	...
	ZTSTickData *newTick = ZTSTickData::create(_cache);
	newTick->setContractInfo(_cInfo);
	_factor->push_tick(newTick);
	ZTSLogger::debug("{}.{} - {}.{} - {}", _cache.exchg, _cache.code, _cache.action_date, _cache.action_time, _cache.price);
	newTick->release();
}