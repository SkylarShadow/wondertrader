...
//////////////////////////////////////////////////////////////////////////
//ParserAdapter
ParserAdapter::ParserAdapter(ZTSBaseDataMgr * bgMgr, DataManager* dtMgr, IndexFactory *idxFactory)
	: _parser_api(NULL)
	, _remover(NULL)
	, _stopped(false)
	, _bd_mgr(bgMgr)
...
bool ParserAdapter::init(const char* id, ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...

	{
		...
		if (hInst == NULL)
		{
			ZTSLogger::log_dyn("parser", _id.c_str(), LL_ERROR, "[{}] Parser module {} loading failed", _id.c_str(), module.c_str());
			return false;
		}
		else
		{
			ZTSLogger::log_dyn("parser", _id.c_str(), LL_INFO, "[{}] Parser module {} loaded", _id.c_str(), module.c_str());
		}
		...
		if (_parser_api->init(cfg))
		{
			...
			else if (!_exchg_filter.empty())
			{
				...
				for (; it != _exchg_filter.end(); it++)
				{
					...
					ZTSArray* ayContract = _bd_mgr->getContracts(exchg);
					...
					ZTSLogger::log_dyn("parser", _id.c_str(), LL_INFO, "[{}] {} contracts of {} added to sublist...", _id.c_str(), cnt, exchg);
				}
			}
			else
			{
...
void ParserAdapter::handleSymbolList( const ZTSArray* aySymbols )
{
...
void ParserAdapter::handleTransaction(ZTSTransData* transData)
{
	...
	ZTSContractInfo* contract = _bd_mgr->getContract(transData->code(), transData->exchg());
	if (contract == NULL)
		return;
...
void ParserAdapter::handleOrderDetail(ZTSOrdDtlData* ordDetailData)
{
	if (_stopped)
		return;
	...
	ZTSContractInfo* contract = _bd_mgr->getContract(ordDetailData->code(), ordDetailData->exchg());
	if (contract == NULL)
		return;
...
void ParserAdapter::handleOrderQueue(ZTSOrdQueData* ordQueData)
{
	...
	ZTSContractInfo* contract = _bd_mgr->getContract(ordQueData->code(), ordQueData->exchg());
	if (contract == NULL)
		return;
...
void ParserAdapter::handleQuote( ZTSTickData *quote, uint32_t procFlag )
{
	...
	ZTSContractInfo* contract = quote->getContractInfo();
	if (contract == NULL)
	{
...
void ParserAdapter::handleParserLog( ZTSLogLevel ll, const char* message)
{
	...
	ZTSLogger::log_raw_by_cat("parser", ll, message);
}
...
bool ParserAdapterMgr::addAdapter(const char* id, ParserAdapterPtr& adapter)
{
	...
	if (it != _adapters.end())
	{
		ZTSLogger::error(" Same name of parsers: %s", id);
		return false;
	}
