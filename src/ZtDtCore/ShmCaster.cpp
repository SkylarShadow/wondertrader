...
bool ShmCaster::init(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	_inited = true;
	ZTSLogger::info("ShmCaste initialized @ {}", _path.c_str());

	return true;
}

void ShmCaster::broadcast(ZTSTickData* curTick)
{
	...
	memcpy(&_queue->_items[realIdx]._tick, &curTick->getTickStruct(), sizeof(ZTSTickStruct));
	_queue->_readable = wIdx;
}

void ShmCaster::broadcast(ZTSOrdQueData* curOrdQue)
{
	...
	memcpy(&_queue->_items[realIdx]._queue, &curOrdQue->getOrdQueStruct(), sizeof(ZTSOrdQueStruct));
	_queue->_readable = wIdx;
}

void ShmCaster::broadcast(ZTSOrdDtlData* curOrdDtl)
{
	...
	memcpy(&_queue->_items[realIdx]._order, &curOrdDtl->getOrdDtlStruct(), sizeof(ZTSOrdDtlStruct));
	_queue->_readable = wIdx;
}

void ShmCaster::broadcast(ZTSTransData* curTrans)
{
	...
	memcpy(&_queue->_items[realIdx]._trans, &curTrans->getTransStruct(), sizeof(ZTSTransStruct));
	_queue->_readable = wIdx;
}