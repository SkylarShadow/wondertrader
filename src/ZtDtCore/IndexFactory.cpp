...
bool IndexFactory::init(ZTSVariant* config, IHotMgr* hotMgr, IBaseDataMgr* bdMgr, DataManager* dataMgr)
{
	_hot_mgr = hotMgr;
	_bd_mgr = bdMgr;
	_data_mgr = dataMgr;
	...
	ZTSVariant* cfgIdx = config->get("indice");
	...
	{
		ZTSVariant* cfgItem = cfgIdx->get(i);
...
void IndexFactory::handle_quote(ZTSTickData* newTick)
{
	if (newTick == NULL)
		return;
...
void IndexFactory::push_tick(ZTSTickData* newTick)
{
	_data_mgr->writeTick(newTick, 1);
}

ZTSTickData* IndexFactory::sub_ticks(const char* fullCode)
{
