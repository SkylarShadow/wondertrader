...
typedef UDPDataPacket<ZTSOrdQueStruct>	UDPOrdQuePacket;
typedef UDPDataPacket<ZTSOrdDtlStruct>	UDPOrdDtlPacket;
typedef UDPDataPacket<ZTSTransStruct>	UDPTransPacket;
...
bool UDPCaster::init(ZTSVariant* cfg, ZTSBaseDataMgr* bdMgr, DataManager* dtMgr)
{
	m_bdMgr = bdMgr;
	m_dtMgr = dtMgr;
...
void UDPCaster::start(int sport)
{
	...
	{
		ZTSLogger::error("Exception raised while start subscribing service @ port {}", sport);
	}
		...
		if (bytes_recvd == sizeof(UDPReqPacket))
		{
			...
			if (req->_type == UDP_MSG_SUBSCRIBE)
			{
				...
				for(const std::string& fullcode : ay)
				{
					...
					ZTSContractInfo* ct = m_bdMgr->getContract(code.c_str(), exchg.c_str());
					if (ct == NULL)
						continue;

					ZTSTickData* curTick = m_dtMgr->getCurTick(code.c_str(), exchg.c_str());
					if(curTick == NULL)
						continue;
					...
					pkt->_type = req->_type;
					memcpy(&pkt->_data, &curTick->getTickStruct(), sizeof(ZTSTickStruct));
					curTick->release();
						...
						[this, data](const boost::system::error_code& ec, std::size_t /*bytes_sent*/)
					{
						...
						if (ec)
						{
							ZTSLogger::error("Sending data on UDP failed: {}", ec.message().c_str());
						}
					});
				}
			}			
...
void UDPCaster::broadcast(ZTSTickData* curTick)
{
	do_broadcast(curTick, UDP_MSG_PUSHTICK);
}

void UDPCaster::broadcast(ZTSOrdDtlData* curOrdDtl)
{
	do_broadcast(curOrdDtl, UDP_MSG_PUSHORDDTL);
}

void UDPCaster::broadcast(ZTSOrdQueData* curOrdQue)
{
	do_broadcast(curOrdQue, UDP_MSG_PUSHORDQUE);
}

void UDPCaster::broadcast(ZTSTransData* curTrans)
{
	do_broadcast(curTrans, UDP_MSG_PUSHTRANS);
}

void UDPCaster::do_broadcast(ZTSObject* data, uint32_t dataType)
{
				...
				while(!tmpQue.empty())
				{
					...
					if (!m_listRawGroup.empty() || !m_listRawRecver.empty())
					{
						...
						if (castData._datatype == UDP_MSG_PUSHTICK)
						{
							...
							ZTSTickData* curObj = (ZTSTickData*)castData._data;
							memcpy(&pack->_data, &curObj->getTickStruct(), sizeof(ZTSTickStruct));
						}
						else if (castData._datatype == UDP_MSG_PUSHORDDTL)
						{
							...
							pack->_type = castData._datatype;
							ZTSOrdDtlData* curObj = (ZTSOrdDtlData*)castData._data;
							memcpy(&pack->_data, &curObj->getOrdDtlStruct(), sizeof(ZTSOrdDtlStruct));
						}
						else if (castData._datatype == UDP_MSG_PUSHORDQUE)
						{
							...
							pack->_type = castData._datatype;
							ZTSOrdQueData* curObj = (ZTSOrdQueData*)castData._data;
							memcpy(&pack->_data, &curObj->getOrdQueStruct(), sizeof(ZTSOrdQueStruct));
						}
						else if (castData._datatype == UDP_MSG_PUSHTRANS)
						{
							...
							pack->_type = castData._datatype;
							ZTSTransData* curObj = (ZTSTransData*)castData._data;
							memcpy(&pack->_data, &curObj->getTransStruct(), sizeof(ZTSTransStruct));
						}
						else
						{
							break;
						...
						for (auto it = m_listRawRecver.begin(); it != m_listRawRecver.end(); it++)
						{
							...
							if (ec)
							{
								ZTSLogger::error("Error occured while sending to ({}:{}): {}({})", 
...
void UDPCaster::handle_send_broad(const EndPoint& ep, const boost::system::error_code& error, std::size_t bytes_transferred)
{
	if(error)
	{
		ZTSLogger::error("Broadcasting of market data failed, remote addr: {}, error message: {}", ep.address().to_string().c_str(), error.message().c_str());
	}
}
...
void UDPCaster::handle_send_multi(const EndPoint& ep, const boost::system::error_code& error, std::size_t bytes_transferred)
{
	if(error)
	{
		ZTSLogger::error("Multicasting of market data failed, remote addr: {}, error message: {}", ep.address().to_string().c_str(), error.message().c_str());
	}
}
