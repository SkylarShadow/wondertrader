...
{
#ifdef _MSC_VER
    CMiniDumper::Enable("ZtBtRunner.exe", true, ZtHelper::getCWD().c_str());
#endif
	...
	ZTSVariant* cfg = ZTSCfgLoader::load_from_file(filename.c_str());
	if (cfg == NULL)
	{
		ZTSLogger::info("Loading configuration file {} failed", filename);
		return -1;
	}
