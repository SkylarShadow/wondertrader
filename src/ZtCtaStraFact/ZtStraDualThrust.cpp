...
ZtStraDualThrust::ZtStraDualThrust(const char* id)
	: CtaStrategy(id)
{
}


ZtStraDualThrust::~ZtStraDualThrust()
{
}

const char* ZtStraDualThrust::getFactName()
{
	return FACT_NAME;
}

const char* ZtStraDualThrust::getName()
{
	return "DualThrust";
}

bool ZtStraDualThrust::init(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
void ZtStraDualThrust::on_session_begin(ICtaStraCtx* ctx, uint32_t uTDate)
{
...
void ZtStraDualThrust::on_schedule(ICtaStraCtx* ctx, uint32_t curDate, uint32_t curTime)
{
	std::string code = _code;

	ZTSKlineSlice *kline = ctx->stra_get_bars(code.c_str(), _period.c_str(), _count, true);
	if(kline == NULL)
	{
		//这里可以输出一些日志
		return;
	...
	ZTSValueArray* closes = kline->extractData(KFT_CLOSE);
	...
	ZTSCommodityInfo* commInfo = ctx->stra_get_comminfo(_code.c_str());
...
void ZtStraDualThrust::on_init(ICtaStraCtx* ctx)
{
	std::string code = _code;
...
void ZtStraDualThrust::on_tick(ICtaStraCtx* ctx, const char* stdCode, ZTSTickData* newTick)
{
	//没有什么要处理
}
