...
inline void pipe_rdmreader_log(IRdmDtReaderSink* sink, ZTSLogLevel ll, const char* format, const Args&... args)
{
	if (sink == NULL)
		return;
...
extern "C"
{
	...
	{
		IRdmDtReader* ret = new ZtRdmDtReader();
		return ret;
	}
...
void ZtRdmDtReader::init(ZTSVariant* cfg, IRdmDtReaderSink* sink)
{
	_sink = sink;
...
bool ZtRdmDtReader::loadStkAdjFactorsFromFile(const char* adjfile)
{
...
ZTSTickSlice* ZtRdmDtReader::readTickSliceByDate(const char* stdCode, uint32_t uDate )
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	...
	if (uDate <= curTDate)
	{
		...
		while (bHasHisTick)
		{
			...
			uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisTickBlock)) / sizeof(ZTSTickStruct);
			if (tcnt <= 0)
				break;

			ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, tBlock->_ticks, tcnt);
			return slice;
	...
	while(isToday)
	{
		...
		ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, tBlock->_ticks, tBlock->_size);
		return slice;
	}
...
ZTSTickSlice* ZtRdmDtReader::readTickSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, NULL, 0);
	...
	while(nowTDate < curTDate)
	{
		...
		while(bHasHisTick)
		{
			...
			ZTSTickStruct* pTick = std::lower_bound(tBlock->_ticks, tBlock->_ticks + (tcnt - 1), eTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
			...
			if (beginTDate != nowTDate)
			{
				...
				//ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, tBlock->_ticks, eIdx + 1);
			...
			{
				//如果交易日相同，则查找起始的位置
				pTick = std::lower_bound(tBlock->_ticks, tBlock->_ticks + eIdx, sTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
					if (a.action_date != b.action_date)
						return a.action_date < b.action_date;
					else
						return a.action_time < b.action_time;
				...
				//ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, tBlock->_ticks + sIdx, eIdx - sIdx + 1);
				//ayTicks->append(slice, false);
	...
	while(hasToday)
	{
		...
		ZTSTickStruct* pTick = std::lower_bound(tBlock->_ticks, tBlock->_ticks + (tBlock->_size - 1), eTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
...
ZTSOrdQueSlice* ZtRdmDtReader::readOrdQueSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	//比较时间的对象
	ZTSOrdQueStruct eTick;
	eTick.action_date = rDate;
	eTick.action_time = rTime * 100000 + rSecs;

	ZTSOrdQueStruct sTick;
	sTick.action_date = lDate;
	sTick.action_time = lTime * 100000 + lSecs;
	...
	if (isToday)
	{
		...
		ZTSOrdQueStruct* pItem = std::lower_bound(rtBlock->_queues, rtBlock->_queues + (rtBlock->_size - 1), eTick, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, rtBlock->_queues, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(rtBlock->_queues, rtBlock->_queues + eIdx, sTick, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, rtBlock->_queues + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
	else
	{
		...
		if (it == _his_ordque_map.end())
		{
			...
			//需要解压
			std::string buf = ZTSCmpHelper::uncompress_data(tBlockV2->_data, (std::size_t)tBlockV2->_size);
		...
		uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisOrdQueBlock)) / sizeof(ZTSOrdQueStruct);
		if (tcnt <= 0)
			return NULL;

		ZTSOrdQueStruct* pItem = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, tBlock->_items, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(tBlock->_items, tBlock->_items + eIdx, sTick, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, tBlock->_items + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
}

ZTSOrdDtlSlice* ZtRdmDtReader::readOrdDtlSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	//比较时间的对象
	ZTSOrdDtlStruct eTick;
	eTick.action_date = rDate;
	eTick.action_time = rTime * 100000 + rSecs;

	ZTSOrdDtlStruct sTick;
	sTick.action_date = lDate;
	sTick.action_time = lTime * 100000 + lSecs;
	...
	if (isToday)
	{
		...
		ZTSOrdDtlStruct* pItem = std::lower_bound(rtBlock->_details, rtBlock->_details + (rtBlock->_size - 1), eTick, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, rtBlock->_details, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(rtBlock->_details, rtBlock->_details + eIdx, sTick, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, rtBlock->_details + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
	else
	{
		...
		uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisOrdDtlBlock)) / sizeof(ZTSOrdDtlStruct);
		if (tcnt <= 0)
			return NULL;

		ZTSOrdDtlStruct* pItem = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, tBlock->_items, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(tBlock->_items, tBlock->_items + eIdx, sTick, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			std::size_t sIdx = pItem - tBlock->_items;
			ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, tBlock->_items + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
}

ZTSTransSlice* ZtRdmDtReader::readTransSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	//比较时间的对象
	ZTSTransStruct eTick;
	eTick.action_date = rDate;
	eTick.action_time = rTime * 100000 + rSecs;

	ZTSTransStruct sTick;
	sTick.action_date = lDate;
	sTick.action_time = lTime * 100000 + lSecs;
	...
	if (isToday)
	{
		...
		ZTSTransStruct* pItem = std::lower_bound(rtBlock->_trans, rtBlock->_trans + (rtBlock->_size - 1), eTick, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, rtBlock->_trans, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(rtBlock->_trans, rtBlock->_trans + eIdx, sTick, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, rtBlock->_trans + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
	else
	{
		...
		uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisTransBlock)) / sizeof(ZTSTransStruct);
		if (tcnt <= 0)
			return NULL;

		ZTSTransStruct* pItem = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		{
			//如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
			ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, tBlock->_items, eIdx + 1);
			return slice;
		}
		else
		{
			//如果交易日相同，则查找起始的位置
			pItem = std::lower_bound(tBlock->_items, tBlock->_items + eIdx, sTick, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
				if (a.action_date != b.action_date)
					return a.action_date < b.action_date;
				else
					return a.action_time < b.action_time;
			...
			std::size_t sIdx = pItem - tBlock->_items;
			ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, tBlock->_items + sIdx, eIdx - sIdx + 1);
			return slice;
		}
	}
}

bool ZtRdmDtReader::cacheHisBarsFromFile(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period)
{
	...
	if (strlen(ruleTag) > 0)//如果是读取期货主力连续数据
	{
		...
		for (auto it = secs.rbegin(); it != secs.rend(); it++)
		{
...

			{
				...
				uint32_t barcnt = content.size() / sizeof(ZTSBarStruct);
				ZTSBarStruct* firstBar = (ZTSBarStruct*)content.data();
	...
	else if(cInfo->isExright() && commInfo->isStock())//如果是读取股票复权数据
	{
		...
		do
		{
			...
			if (kBlock->_version == BLOCK_VERSION_CMP)
			{
				...
				buffer = ZTSCmpHelper::uncompress_data(kBlockV2->_data, (std::size_t)kBlockV2->_size);
			}
			else
			{
			...
			barcnt = buffer.size() / sizeof(ZTSBarStruct);
	...
	if (realCnt > 0)
	{
		...
		for (auto it = barsSections.rbegin(); it != barsSections.rend(); it++)
		{
			...
			memcpy(barList._bars.data() + curIdx, tempAy->data(), tempAy->size()*sizeof(ZTSBarStruct));
			curIdx += tempAy->size();
			delete tempAy;
		}
		barsSections.clear();
...
ZTSBarStruct* ZtRdmDtReader::indexBarFromCacheByRange(const std::string& key, uint64_t stime, uint64_t etime, uint32_t& count, bool isDay /* = false */)
{
	...
	std::size_t eIdx,sIdx;
	{
		...
		auto eit = std::lower_bound(barsList._bars.begin(), barsList._bars.end(), eBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b){
			if (isDay)
				return a.date < b.date;
			else
				return a.time < b.time;
		...
		auto sit = std::lower_bound(barsList._bars.begin(), eit, sBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
			if (isDay)
				return a.date < b.date;
			else
				return a.time < b.time;
...
ZTSBarStruct* ZtRdmDtReader::indexBarFromCacheByCount(const std::string& key, uint64_t etime, uint32_t& count, bool isDay /* = false */)
{
...
uint32_t ZtRdmDtReader::readBarsFromCacheByRange(const std::string& key, uint64_t stime, uint64_t etime, std::vector<ZTSBarStruct>& ayBars, bool isDay /* = false */)
{
	uint32_t rDate, rTime, lDate, lTime;
	rDate = (uint32_t)(etime / 10000);
	rTime = (uint32_t)(etime % 10000);
	...
	if(curCnt > 0)
	{
		...
		memcpy(ayBars.data(), &barsList._bars[sIdx], sizeof(ZTSBarStruct)*curCnt);
	}
	return curCnt;
}

ZTSKlineSlice* ZtRdmDtReader::readKlineSliceByRange(const char* stdCode, ZTSKlinePeriod period, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	ZTSBarStruct* hisHead = NULL;
	ZTSBarStruct* rtHead = NULL;
	uint32_t hisCnt = 0;
	uint32_t rtCnt = 0;
	...
	if (bHasToday)
	{
		...
		if(cInfo._exright != 2)
		{
			...
			if (kPair != NULL)
			{
				...
				//读取当日的数据
				ZTSBarStruct* pBar = std::lower_bound(kPair->_block->_bars, kPair->_block->_bars + (kPair->_block->_size - 1), eBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
					if (isDay)
						return a.date < b.date;
					else
						return a.time < b.time;
				...
				{
					pBar = std::lower_bound(kPair->_block->_bars, kPair->_block->_bars + idx, sBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
						if (isDay)
							return a.date < b.date;
						else
							return a.time < b.time;
		...
		else
		{
			...
			if (kPair != NULL)
			{
				...
				if (newSize > oldSize)
				{
					...
					memcpy(&barsList._rt_bars[idx], &kPair->_block->_bars[idx], sizeof(ZTSBarStruct)*(newSize - oldSize + 1));

					//最后做复权处理
					double factor = barsList._factor;
					for (; idx < newSize; idx++)
					{
						ZTSBarStruct* pBar = &barsList._rt_bars[idx];
						pBar->open *= factor;
						pBar->high *= factor;
						pBar->low *= factor;
						pBar->close *= factor;
				...
				//最后做一个定位
				auto it = std::lower_bound(barsList._rt_bars.begin(), barsList._rt_bars.end(), eBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
					if (isDay)
						return a.date < b.date;
					else
						return a.time < b.time;
				...
				{
					it = std::lower_bound(barsList._rt_bars.begin(), barsList._rt_bars.begin() + idx, sBar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
						if (isDay)
							return a.date < b.date;
						else
							return a.time < b.time;
	...
	if (hisCnt + rtCnt > 0)
	{
		ZTSKlineSlice* slice = ZTSKlineSlice::create(stdCode, period, 1, hisHead, hisCnt);
		if (rtCnt > 0)
...
ZtRdmDtReader::TickBlockPair* ZtRdmDtReader::getRTTickBlock(const char* exchg, const char* code)
{
...
ZtRdmDtReader::OrdDtlBlockPair* ZtRdmDtReader::getRTOrdDtlBlock(const char* exchg, const char* code)
{
	std::string key = fmt::format("{}.{}", exchg, code);
...
ZtRdmDtReader::OrdQueBlockPair* ZtRdmDtReader::getRTOrdQueBlock(const char* exchg, const char* code)
{
	std::string key = fmt::format("{}.{}", exchg, code);
...
ZtRdmDtReader::TransBlockPair* ZtRdmDtReader::getRTTransBlock(const char* exchg, const char* code)
{
	std::string key = fmt::format("{}.{}", exchg, code);
...
ZtRdmDtReader::RTKlineBlockPair* ZtRdmDtReader::getRTKilneBlock(const char* exchg, const char* code, ZTSKlinePeriod period)
{
...
ZTSKlineSlice* ZtRdmDtReader::readKlineSliceByCount(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
		...
		else
		{
			...
			if (kPair != NULL)
			{
				...
				if(newSize > oldSize)
				{
					...
					//因为每次拷贝，最后一条K线都有可能是未闭合的，所以需要把最后一条K线覆盖
					memcpy(&barsList._rt_bars[idx], &kPair->_block->_bars[idx], sizeof(ZTSBarStruct)*(newSize - idx));
...
ZTSTickSlice* ZtRdmDtReader::readTickSliceByCount(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();

	ZTSSessionInfo* sInfo = _base_data_mgr->getSession(_base_data_mgr->getCommodity(cInfo._exchg, cInfo._code)->getSession());
	...
	ZTSTickSlice* slice = ZTSTickSlice::create(stdCode);
...
double ZtRdmDtReader::getAdjFactorByDate(const char* stdCode, uint32_t date /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	if (!commInfo->isStock())
