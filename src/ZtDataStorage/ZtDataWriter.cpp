...
inline void pipe_writer_log(IDataWriterSink* sink, ZTSLogLevel ll, const char* format, const Args&... args)
{
	if (sink == NULL)
		return;
...
extern "C"
{
	...
	{
		IDataWriter* ret = new ZtDataWriter();
		return ret;
	}
...
ZtDataWriter::_TaskInfo::_TaskInfo(ZTSObject* data, uint64_t dtype, uint32_t flag/* = 0*/)
...
ZtDataWriter::_TaskInfo::_TaskInfo(const _TaskInfo& rhs)
...
ZtDataWriter::_TaskInfo::~_TaskInfo() 
{ 
	_obj->release(); 
}


ZtDataWriter::ZtDataWriter()
	: _terminated(false)
...
bool ZtDataWriter::isSessionProceeded(const char* sid)
{
...
bool ZtDataWriter::init(ZTSVariant* params, IDataWriterSink* sink)
{
	...
	_proc_chk.reset(new StdThread(boost::bind(&ZtDataWriter::check_loop, this)));

	pipe_writer_log(sink, LL_INFO, "ZtDataWriter initialized, root dir: {}, save_csv_tick: {}, async_mode: {}, log_group_size: {}, disable_history: {}, "
...
void DataManager::preloadRtCaches(const char* exchg)
{
	...
	uint32_t cnt = 0;
	uint32_t codecnt = 0;
	ZTSArray* ayCts = _bd_mgr->getContracts(exchg);
	if (ayCts != NULL && ayCts->size() > 0)
	{
		for (auto it = ayCts->begin(); it != ayCts->end(); it++)
		{
			ZTSContractInfo* ct = (ZTSContractInfo*)(*it);
			if (ct == NULL)
				continue;
			ZTSCommodityInfo* commInfo = _bd_mgr->getCommodity(ct);
			if(commInfo == NULL)
				continue;
	...
	if (ayCts != NULL)
		ayCts->release();
	pipe_writer_log(_sink, LL_INFO, "预加载%个品种的实时数据缓存文件{}个,耗时{}微秒", codecnt, cnt, ZTSLogger::fmtInt64(ticker.micro_seconds()));
	_preloaded = true;
}
*/

void ZtDataWriter::loadCache()
{
...
void* ZtDataWriter::resizeRTBlock(BoostMFPtr& mfPtr, uint32_t nCount)
{
	if (mfPtr == NULL)
		return NULL;
...
bool ZtDataWriter::writeTick(ZTSTickData* curTick, uint32_t procFlag)
{
	if (curTick == NULL)
		return false;
...
void ZtDataWriter::procTick(ZTSTickData* curTick, uint32_t procFlag)
{
	do
	{
		ZTSContractInfo* ct = curTick->getContractInfo();
		if (ct == NULL)
			break;
...
bool ZtDataWriter::writeOrderQueue(ZTSOrdQueData* curOrdQue)
{
...
void ZtDataWriter::procQueue(ZTSOrdQueData* curOrdQue)
{
	do
	{
		ZTSContractInfo* ct = curOrdQue->getContractInfo();
		ZTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		if (blk->_size >= blk->_capacity)
		{
			...
			pBlockPair->_block = (RTOrdQueBlock*)resizeRTBlock<RTDayBlockHeader, ZTSOrdQueStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_queues[blk->_size], &curOrdQue->getOrdQueStruct(), sizeof(ZTSOrdQueStruct));
		blk->_size += 1;
...
bool ZtDataWriter::writeOrderDetail(ZTSOrdDtlData* curOrdDtl)
{
...
void ZtDataWriter::procOrder(ZTSOrdDtlData* curOrdDtl)
{
	do
	{
		ZTSContractInfo* ct = curOrdDtl->getContractInfo();
		ZTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		{
			pBlockPair->_file->sync();
			pBlockPair->_block = (RTOrdDtlBlock*)resizeRTBlock<RTDayBlockHeader, ZTSOrdDtlStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_details[blk->_size], &curOrdDtl->getOrdDtlStruct(), sizeof(ZTSOrdDtlStruct));
		blk->_size += 1;
...
bool ZtDataWriter::writeTransaction(ZTSTransData* curTrans)
{
...
void ZtDataWriter::procTrans(ZTSTransData* curTrans)
{
	do
	{
		ZTSContractInfo* ct = curTrans->getContractInfo();
		ZTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		{
			pBlockPair->_file->sync();
			pBlockPair->_block = (RTTransBlock*)resizeRTBlock<RTDayBlockHeader, ZTSTransStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_trans[blk->_size], &curTrans->getTransStruct(), sizeof(ZTSTransStruct));
		blk->_size += 1;
...
void ZtDataWriter::pushTask(const TaskInfo& task)
{
	if (!_async_proc)
		return;
		...
		_task_thrd.reset(new StdThread([this]() {
			while (!_terminated)
			{
				...
				while (!tempQueue.empty())
				{
					...
					{
					case 0: procTick((ZTSTickData*)curTask._obj, curTask._flag); break;
					case 1: procQueue((ZTSOrdQueData*)curTask._obj); break;
					case 2: procOrder((ZTSOrdDtlData*)curTask._obj); break;
					case 3: procTrans((ZTSTransData*)curTask._obj); break;
					default:
						break;
					}
					tempQueue.pop();
...
void ZtDataWriter::pipeToTicks(ZTSContractInfo* ct, ZTSTickData* curTick)
{
	...
	{
		pBlockPair->_file->sync();
		pBlockPair->_block = (RTTickBlock*)resizeRTBlock<RTDayBlockHeader, ZTSTickStruct>(pBlockPair->_file, blk->_capacity * 2);
		blk = pBlockPair->_block;
	...
	memcpy(&blk->_ticks[blk->_size], &curTick->getTickStruct(), sizeof(ZTSTickStruct));
	blk->_size += 1;
...
ZtDataWriter::OrdQueBlockPair* ZtDataWriter::getOrdQueBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSOrdQueStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_queues, 0, sizeof(ZTSOrdQueStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			do
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSOrdQueStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(ZTSOrdQueStruct));
...
ZtDataWriter::OrdDtlBlockPair* ZtDataWriter::getOrdDtlBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSOrdDtlStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_details, 0, sizeof(ZTSOrdDtlStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			for (;;)
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSOrdDtlStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(ZTSOrdDtlStruct));
					//文件大小不匹配,一般是因为capacity改了,但是实际没扩容
					//这是做一次扩容即可
					pBlock->_block->_capacity = oldCnt;
					pBlock->_block->_size = oldCnt;
...
ZtDataWriter::TransBlockPair* ZtDataWriter::getTransBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSTransStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_trans, 0, sizeof(ZTSTransStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			for (;;)
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(ZTSTransStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(ZTSTransStruct));
					//文件大小不匹配,一般是因为capacity改了,但是实际没扩容
					//这是做一次扩容即可
					pBlock->_block->_capacity = oldCnt;
					pBlock->_block->_size = oldCnt;
...
ZtDataWriter::TickBlockPair* ZtDataWriter::getTickBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if(pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTTickBlock) + sizeof(ZTSTickStruct) * HFT_SIZE_STEP;
			BoostFile bf;
			bf.create_new_file(path.c_str());
			bf.truncate_file((uint32_t)uSize);
			bf.close_file();
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_ticks, 0, sizeof(ZTSTickStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			do
			{
				uint64_t uSize = sizeof(RTTickBlock) + sizeof(ZTSTickStruct) * pBlock->_block->_capacity;
				uint64_t realSz = pBlock->_file->size();
				if (realSz != uSize)
				{
					uint32_t realCap = (uint32_t)((realSz - sizeof(RTTickBlock)) / sizeof(ZTSTickStruct));
...
void ZtDataWriter::pipeToKlines(ZTSContractInfo* ct, ZTSTickData* curTick)
{
	...
	ZTSSessionInfo* sInfo = ct->getCommInfo()->getSessionInfo();
	...
	if (!_disable_min1)
	{
		...
		if (pBlockPair && pBlockPair->_block)
		{
			...
			{
				pBlockPair->_file->sync();
				pBlockPair->_block = (RTKlineBlock*)resizeRTBlock<RTKlineBlock, ZTSBarStruct>(pBlockPair->_file, blk->_capacity * 2);
				blk = pBlockPair->_block;
			}
...
template<typename T>
void ZtDataWriter::releaseBlock(T* block)
{
...
ZtDataWriter::KBlockPair* ZtDataWriter::getKlineBlock(ZTSContractInfo* ct, ZTSKlinePeriod period, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path))
		{
			...
			uint64_t uSize = sizeof(RTKlineBlock) + sizeof(ZTSBarStruct) * totalMins;	//预分配按照K线条数分配
			BoostFile bf;
		...
		//	memset(&pBlock->_block->_bars, 0, sizeof(ZTSBarStruct)*pBlock->_block->_capacity);
		//}
...
ZTSTickData* ZtDataWriter::getCurTick(const char* code, const char* exchg/* = ""*/)
{
	if (strlen(code) == 0)
		return NULL;
	...
	return ZTSTickData::create(item._tick);
}

bool ZtDataWriter::updateCache(ZTSContractInfo* ct, ZTSTickData* curTick, uint32_t procFlag)
{
	...
	ZTSTickStruct& newTick = curTick->getTickStruct();

	if (curTick->tradingdate() > item._date)
	{
		//新数据交易日大于老数据,则认为是新一天的数据
		item._date = curTick->tradingdate();
		memcpy(&item._tick, &newTick, sizeof(ZTSTickStruct));
		if (procFlag==1)
		{
...
void ZtDataWriter::transHisData(const char* sid)
{
	...
	if (strcmp(sid, CMD_CLEAR_CACHE) != 0)
	{
		...
		for (auto it = pCommSet->begin(); it != pCommSet->end(); it++)
		{
			...
			ZTSCommodityInfo* pCommInfo = _bd_mgr->getCommodity(exchg, pid);
			if (pCommInfo == NULL)
				continue;
			...
			for (auto code : codes)
			{
				ZTSContractInfo* ct = _bd_mgr->getContract(code.c_str(), exchg);
				if(ct)
	...
	if (_proc_thrd == NULL)
	{
		_proc_thrd.reset(new StdThread(boost::bind(&ZtDataWriter::proc_loop, this)));
	}
	else
	{
		_proc_cond.notify_all();
...
uint32_t ZtDataWriter::dump_bars_via_dumper(ZTSContractInfo* ct)
{
	...
	if (it != _tick_cache_idx.end())
	{
		...
		const ZTSTickStruct& ts = tci._tick;
...
bool ZtDataWriter::proc_block_data(const char* tag, std::string& content, bool isBar, bool bKeepHead /* = true */)
{
	...
	if (bCmped)
	{
		...
		//将文件头后面的数据进行解压
		buffer = ZTSCmpHelper::uncompress_data(content.data() + BLOCK_HEADERV2_SIZE, (std::size_t)blkV2->_size);
	}
	else
	{
		if (!bOldVer)
...
bool ZtDataWriter::dump_day_data(ZTSContractInfo* ct, ZTSBarStruct* newBar)
{
	std::stringstream ss;
	...
	if (f.create_or_open_file(filename.c_str()))
	{
		...
		if (bNew)
		{
			...
			f.write_file(newBar, sizeof(ZTSBarStruct));
		}
		else
		{
			//日线必须要检查一下
			...
			if (barcnt != 0)
			{
				ZTSBarStruct& oldBS = ((ZTSBarStruct*)content.data())[barcnt - 1];

				if (oldBS.date == newBar->date && memcmp(&oldBS, newBar, sizeof(ZTSBarStruct)) != 0)
				{
				...
				{
					content.append((char*)newBar, sizeof(ZTSBarStruct));
					barcnt++;
				}
			}
			...
			if (bNeedCompress)
			{
				std::string cmpData = ZTSCmpHelper::compress_data(content.data(), content.size());
...
uint32_t ZtDataWriter::dump_bars_to_file(ZTSContractInfo* ct)
{
	if (ct == NULL)
		return 0;
	...
	if (!_disable_min1)
	{
		...
		if (kBlkPair != NULL && kBlkPair->_block->_size > 0)
		{
			...
			if (f.create_or_open_file(filename.c_str()))
			{
				...
				//追加新的数据
				buffer.append((const char*)kBlkPair->_block->_bars, sizeof(ZTSBarStruct)*size);

				std::string cmpData = ZTSCmpHelper::compress_data(buffer.data(), buffer.size());
				...
				//最后将缓存清空
				//memset(kBlkPair->_block->_bars, 0, sizeof(ZTSBarStruct)*kBlkPair->_block->_size);
				kBlkPair->_block->_size = 0;
			}
			else
			{
	...
	while (!_terminated)
	{
		...
		if (fullcode.compare(CMD_CLEAR_CACHE) == 0)
		{
			...
			for (auto it = _tick_cache_idx.begin(); it != _tick_cache_idx.end(); it++)
			{
				const char* key = it->first.c_str();
				const StringVector& ay = StrUtil::split(key, ".");
				ZTSContractInfo* ct = _bd_mgr->getContract(ay[1].c_str(), ay[0].c_str());
				if (ct != NULL)
				{
					setCodes.insert(key);
				...
				if (tBlkPair != NULL)
				{
					...
					if (tBlkPair->_block->_size > 0)
					{
						...
						{//////////////////////////////////////////////////////////////////////////
							...
							{
								//先压缩数据
								std::string cmp_data = ZTSCmpHelper::compress_data(tBlkPair->_block->_ticks, sizeof(ZTSTickStruct)*tBlkPair->_block->_size);
								...
								//最后将缓存清空
								//memset(tBlkPair->_block->_ticks, 0, sizeof(ZTSTickStruct)*tBlkPair->_block->_size);
			...
			if (!_disable_trans)
			{
				...
				if (tBlkPair != NULL && tBlkPair->_block->_size > 0)
				{
...

					{
						...
						{
							//先压缩数据
							std::string cmp_data = ZTSCmpHelper::compress_data(tBlkPair->_block->_trans, sizeof(ZTSTransStruct)*tBlkPair->_block->_size);
			...
			if (!_disable_orddtl)
			{
				...
				if (tBlkPair != NULL && tBlkPair->_block->_size > 0)
				{
...

					{
						...
						{
							//先压缩数据
							std::string cmp_data = ZTSCmpHelper::compress_data(tBlkPair->_block->_details, sizeof(ZTSOrdDtlStruct)*tBlkPair->_block->_size);
			...
			if (!_disable_ordque)
			{
				...
				if (tBlkPair != NULL && tBlkPair->_block->_size > 0)
				{
...

					{
						...
						{
							//先压缩数据
							std::string cmp_data = ZTSCmpHelper::compress_data(tBlkPair->_block->_queues, sizeof(ZTSOrdQueStruct)*tBlkPair->_block->_size);
