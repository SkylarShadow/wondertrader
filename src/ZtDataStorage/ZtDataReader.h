...
class ZtDataReader : public IDataReader
{
public:
	ZtDataReader();
	virtual ~ZtDataReader();
...
private:
	...
	bool	cacheIntegratedBars(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period);
	bool	cacheAdjustedStkBars(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period);

	/*
	 *	将历史数据放入缓存
	 */
	bool	cacheHisBarsFromFile(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period);
	bool	cacheFinalBarsFromLoader(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period);
...
public:
	virtual void init(ZTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader = NULL) override;

	virtual void onMinuteEnd(uint32_t uDate, uint32_t uTime, uint32_t endTDate = 0) override;

	virtual ZTSTickSlice*	readTickSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) override;
	virtual ZTSOrdDtlSlice*	readOrdDtlSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) override;
	virtual ZTSOrdQueSlice*	readOrdQueSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) override;
	virtual ZTSTransSlice*	readTransSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) override;
	virtual ZTSKlineSlice*	readKlineSlice(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime = 0) override;
