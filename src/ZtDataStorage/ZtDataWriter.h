...
class ZtDataWriter : public IDataWriter
{
public:
	ZtDataWriter();
	~ZtDataWriter();	
...
private:
	...
	uint32_t  dump_bars_to_file(ZTSContractInfo* ct);

	uint32_t  dump_bars_via_dumper(ZTSContractInfo* ct);

private:
	bool	dump_day_data(ZTSContractInfo* ct, ZTSBarStruct* newBar);

	bool	proc_block_data(const char* tag, std::string& content, bool isBar, bool bKeepHead = true);

	void	procTick(ZTSTickData* curTick, uint32_t procFlag);
	void	procQueue(ZTSOrdQueData* curOrdQue);
	void	procOrder(ZTSOrdDtlData* curOrdDetail);
	void	procTrans(ZTSTransData* curTrans);

public:
	virtual bool init(ZTSVariant* params, IDataWriterSink* sink) override;
	virtual void release() override;

	virtual bool writeTick(ZTSTickData* curTick, uint32_t procFlag) override;

	virtual bool writeOrderQueue(ZTSOrdQueData* curOrdQue) override;

	virtual bool writeOrderDetail(ZTSOrdDtlData* curOrdDetail) override;

	virtual bool writeTransaction(ZTSTransData* curTrans) override;

	virtual void transHisData(const char* sid) override;
	
	virtual bool isSessionProceeded(const char* sid) override;

	virtual ZTSTickData* getCurTick(const char* code, const char* exchg = "") override;
...
private:
	...
	typedef struct alignas(64) _TaskInfo
	{
		...
		_TaskInfo(ZTSObject* data, uint64_t dtype, uint32_t flag = 0);
...
private:
	...
	bool updateCache(ZTSContractInfo* ct, ZTSTickData* curTick, uint32_t procFlag);

	void pipeToTicks(ZTSContractInfo* ct, ZTSTickData* curTick);

	void pipeToKlines(ZTSContractInfo* ct, ZTSTickData* curTick);

	KBlockPair* getKlineBlock(ZTSContractInfo* ct, ZTSKlinePeriod period, bool bAutoCreate = true);

	TickBlockPair* getTickBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate = true);
	TransBlockPair* getTransBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate = true);
	OrdDtlBlockPair* getOrdDtlBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate = true);
	OrdQueBlockPair* getOrdQueBlock(ZTSContractInfo* ct, uint32_t curDate, bool bAutoCreate = true);
