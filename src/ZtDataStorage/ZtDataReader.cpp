...
inline void pipe_reader_log(IDataReaderSink* sink, ZTSLogLevel ll, const char* format, const Args&... args)
{
	if (sink == NULL)
		return;
...
extern "C"
{
	...
	{
		IDataReader* ret = new ZtDataReader();
		return ret;
	}
...
void ZtDataReader::init(ZTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader /* = NULL */)
{
	...
	pipe_reader_log(sink, LL_INFO, "ZtDataReader initialized, rt dir is {}, hist dir is {}, adjust_flag is {}", _rt_dir, _his_dir, _adjust_flag);
...
bool ZtDataReader::loadStkAdjFactorsFromLoader()
{
	if (NULL == _loader)
		return false;
...
bool ZtDataReader::loadStkAdjFactorsFromFile(const char* adjfile)
{
...
ZTSTickSlice* ZtDataReader::readTickSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	if (isToday)
	{
		...
		ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, tBlock->_ticks + sIdx, cnt);
		return slice;
	}
	else
	{
...
ZTSOrdQueSlice* ZtDataReader::readOrdQueSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, rtBlock->_queues + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		ZTSOrdQueStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSOrdQueSlice* slice = ZTSOrdQueSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}

ZTSOrdDtlSlice* ZtDataReader::readOrdDtlSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, rtBlock->_details + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		ZTSOrdDtlStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSOrdDtlSlice* slice = ZTSOrdDtlSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}

ZTSTransSlice* ZtDataReader::readTransSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, rtBlock->_trans + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		ZTSTransStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		ZTSTransSlice* slice = ZTSTransSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}


bool ZtDataReader::cacheFinalBarsFromLoader(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period)
{
	if (NULL == _loader)
		return false;
	...
	bool ret = _loader->loadFinalHisBars(&barList, stdCode, period, [](void* obj, ZTSBarStruct* firstBar, uint32_t count) {
		BarsList* bars = (BarsList*)obj;
		bars->_factor = 1.0;
		bars->_bars.resize(count);
		memcpy(bars->_bars.data(), firstBar, sizeof(ZTSBarStruct)*count);
	});
...
bool ZtDataReader::cacheIntegratedBars(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	...
	for (auto it = secs.rbegin(); it != secs.rend(); it++)
	{
		...
		if (NULL != _loader)
		{
			...
			bLoaded = _loader->loadRawHisBars(&buffer, wCode.c_str(), period, [](void* obj, ZTSBarStruct* bars, uint32_t count) {
...
bool ZtDataReader::cacheAdjustedStkBars(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	...
	std::vector<ZTSBarStruct>* ayAdjusted = NULL;
	...
	do
	{
		...
		ayAdjusted = new std::vector<ZTSBarStruct>();
	...
	do
	{
		...
		if (NULL != _loader)
		{
			bLoaded = _loader->loadRawHisBars(&buffer, rawCode.c_str(), period, [](void* obj, ZTSBarStruct* bars, uint32_t count) {
				std::string* buff = (std::string*)obj;
				buff->resize(sizeof(ZTSBarStruct)*count);
				memcpy((void*)buff->c_str(), bars, sizeof(ZTSBarStruct)*count);
			});
		...
		if (pBar != NULL)
		{
			...
			std::vector<ZTSBarStruct>* ayRaw = new std::vector<ZTSBarStruct>();
			ayRaw->resize(curCnt);
			memcpy(ayRaw->data(), &firstBar[sIdx], sizeof(ZTSBarStruct)*curCnt);
			realCnt += curCnt;
...
bool ZtDataReader::cacheHisBarsFromFile(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo->_exchg, cInfo->_product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (NULL != _loader)
	{
		bLoaded = _loader->loadRawHisBars(&buffer, stdCode, period, [](void* obj, ZTSBarStruct* bars, uint32_t count) {
			std::string* buff = (std::string*)obj;
			buff->resize(sizeof(ZTSBarStruct)*count);
			memcpy((void*)buff->c_str(), bars, sizeof(ZTSBarStruct)*count);
		});
...
ZTSKlineSlice* ZtDataReader::readKlineSlice(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	...
	ZTSKlineSlice* slice = ZTSKlineSlice::create(stdCode, period, 1, NULL, 0);;
	ZTSBarStruct* head = NULL;
	uint32_t hisCnt = 0;
	uint32_t rtCnt = 0;
	uint32_t totalCnt = 0;
	std::string pname;
	...
	*/
	...
	if (bHasToday)
	{
		...
		if (kPair != NULL && kPair->_block && kPair->_block->_size>0)
		{
			//读取当日的数据
			ZTSBarStruct* pBar = NULL;
			pBar = std::lower_bound(kPair->_block->_bars, kPair->_block->_bars + (kPair->_block->_size - 1), bar, [period](const ZTSBarStruct& a, const ZTSBarStruct& b) {
				if (period == KP_DAY)
					return a.date < b.date;
				else
					return a.time < b.time;
			...
			if(cInfo._exright == 2/* && commInfo->isStock()*/)
			{
				...
				if (barsList._rt_cursor == UINT_MAX || idx > barsList._rt_cursor)
				{
					...
					memcpy(&barsList._bars[oldSize], &kPair->_block->_bars[sIdx], sizeof(ZTSBarStruct)* curCnt);
					for(uint32_t thisIdx = oldSize; thisIdx < newSize; thisIdx++)
					{
						ZTSBarStruct* pBar = &barsList._bars[thisIdx];
						pBar->open *= factor;
						pBar->high *= factor;
						pBar->low *= factor;
						pBar->close *= factor;
...
ZtDataReader::TickBlockPair* ZtDataReader::getRTTickBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
...
ZtDataReader::OrdDtlBlockPair* ZtDataReader::getRTOrdDtlBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
ZtDataReader::OrdQueBlockPair* ZtDataReader::getRTOrdQueBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
ZtDataReader::TransBlockPair* ZtDataReader::getRTTransBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
ZtDataReader::RTKlineBlockPair* ZtDataReader::getRTKilneBlock(const char* exchg, const char* code, ZTSKlinePeriod period)
{
...
void ZtDataReader::onMinuteEnd(uint32_t uDate, uint32_t uTime, uint32_t endTDate /* = 0 */)
{
	//这里应该触发检查
	...
	for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++)
	{
		...
		if (barsList._period != KP_DAY)
		{
			if (!barsList._raw_code.empty())
			{
				...
				for (;;)
				{
					...
					ZTSBarStruct& nextBar = kBlk->_block->_bars[preCnt];
		...
		//		for (;;)
		//		{
		//			ZTSBarStruct& nextBar = barsList._bars[barsList._his_cursor + 1];
...
double ZtDataReader::getAdjFactorByDate(const char* stdCode, uint32_t date /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	if (!commInfo->isStock())
...
const ZtDataReader::AdjFactorList& ZtDataReader::getAdjFactors(const char* code, const char* exchg, const char* pid)
{
	...
	if (it == _adj_factors.end())
	{
		...
		if (_loader)
		{
			...
			_loader->loadAdjFactors(this, key, [](void* obj, const char* stdCode, uint32_t* dates, double* factors, uint32_t count) {
				ZtDataReader* self = (ZtDataReader*)obj;
