#include "VvtSelEngine.h"
#include "VvtDtMgr.h"
#include "VvtSelTicker.h"
...
void VvtSelEngine::on_session_end()
{
	if (_evt_listener)
		_evt_listener->on_session_event(_cur_tdate, false);
}

void VvtSelEngine::on_session_begin()
{
	if (_evt_listener)
...
void VvtSelEngine::on_init()
{
	if (_evt_listener)
		_evt_listener->on_initialize_event();
}

void VvtSelEngine::handle_push_quote(VvTSTickData* curTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(curTick);
}

void VvtSelEngine::on_bar(const char* stdCode, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
...
void VvtSelEngine::on_tick(const char* stdCode, VvTSTickData* curTick)
{
...
void VvtSelEngine::on_minute_end(uint32_t curDate, uint32_t curTime)
{
	//要比较下一分钟的时间
...
void VvtSelEngine::run()
{
	VvTSVariant* cfgProd = _cfg->get("product");
	_tm_ticker = new VvtSelRtTicker(this);
...
void VvtSelEngine::init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier /* = NULL */)
{
...
void VvtSelEngine::addContext(SelContextPtr ctx, uint32_t date, uint32_t time, TaskPeriodType period, bool bStrict /* = true */, const char* trdtpl /* = "CHINA" */, const char* sessionID/* ="TRADING" */)
{
	if (ctx == NULL)
		return;
...
SelContextPtr VvtSelEngine::getContext(uint32_t id)
{
...
void VvtSelEngine::handle_pos_change(const char* straName, const char* stdCode, double diffQty)
{
...
VvTSCommodityInfo* VvtSelEngine::get_comm_info(const char* stdCode)
{
...
VvTSSessionInfo* VvtSelEngine::get_sess_info(const char* stdCode)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
...
uint64_t VvtSelEngine::get_real_time()
{
