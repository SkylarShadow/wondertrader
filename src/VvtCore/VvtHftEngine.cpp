...
#include "VvtHftEngine.h"
#include "VvtHftTicker.h"
#include "VvtDtMgr.h"
...
void VvtHftEngine::init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier /* = NULL */)
{
...
void VvtHftEngine::run()
{
	...
	_tm_ticker = new VvtHftRtTicker(this);
...
void VvtHftEngine::handle_push_quote(VvTSTickData* newTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(newTick);
}

void VvtHftEngine::handle_push_order_detail(VvTSOrdDtlData* curOrdDtl)
{
...
void VvtHftEngine::handle_push_order_queue(VvTSOrdQueData* curOrdQue)
{
...
void VvtHftEngine::handle_push_transaction(VvTSTransData* curTrans)
{
...
void VvtHftEngine::sub_order_detail(uint32_t sid, const char* stdCode)
{
...
void VvtHftEngine::sub_order_queue(uint32_t sid, const char* stdCode)
{
	std::size_t length = strlen(stdCode);
	if (stdCode[length - 1] == SUFFIX_QFQ || stdCode[length - 1] == SUFFIX_HFQ)
		length--;
...
void VvtHftEngine::sub_transaction(uint32_t sid, const char* stdCode)
{
	std::size_t length = strlen(stdCode);
	if (stdCode[length - 1] == SUFFIX_QFQ || stdCode[length - 1] == SUFFIX_HFQ)
		length--;
...
void VvtHftEngine::on_tick(const char* stdCode, VvTSTickData* curTick)
{
...
void VvtHftEngine::on_bar(const char* stdCode, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
...
void VvtHftEngine::on_session_begin()
{
	VvTSLogger::info("Trading day {} begun", _cur_tdate);
	VvtEngine::on_session_begin();
...
void VvtHftEngine::on_session_end()
{
...
void VvtHftEngine::on_minute_end(uint32_t curDate, uint32_t curTime)
{
	//已去掉高频策略的on_schedule
...
void VvtHftEngine::addContext(HftContextPtr ctx)
{
	uint32_t sid = ctx->id();
	_ctx_map[sid] = ctx;
}

HftContextPtr VvtHftEngine::getContext(uint32_t id)
{
...
VvTSOrdQueSlice* VvtHftEngine::get_order_queue_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_queue_slice(code, count);
}

VvTSOrdDtlSlice* VvtHftEngine::get_order_detail_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_detail_slice(code, count);
}

VvTSTransSlice* VvtHftEngine::get_transaction_slice(uint32_t sid, const char* code, uint32_t count)
{
