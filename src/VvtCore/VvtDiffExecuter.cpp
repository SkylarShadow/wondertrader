...
VvtDiffExecuter::VvtDiffExecuter(VvtExecuterFactory* factory, const char* name, IDataManager* dataMgr, IBaseDataMgr* bdMgr)
	: IExecCommand(name)
	, _factory(factory)
	, _data_mgr(dataMgr)
...
VvtDiffExecuter::~VvtDiffExecuter()
{
	if (_pool)
		_pool->wait();
}

void VvtDiffExecuter::setTrader(TraderAdapter* adapter)
{
	_trader = adapter;
	//设置的时候读取一下trader的状态
	if(_trader)
...
bool VvtDiffExecuter::init(VvTSVariant* params)
{
	if (params == NULL)
		return false;
...
void VvtDiffExecuter::load_data()
{
	//读取执行器的理论部位，以及待执行的差量
...
void VvtDiffExecuter::save_data()
{
	std::string filename = VvtHelper::getExecDataDir();
	filename += _name + ".json";
...
ExecuteUnitPtr VvtDiffExecuter::getUnit(const char* stdCode, bool bAutoCreate /* = true */)
{
...
VvTSTickSlice* VvtDiffExecuter::getTicks(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_data_mgr == NULL)
		return NULL;
...
VvTSTickData* VvtDiffExecuter::grabLastTick(const char* stdCode)
{
	if (_data_mgr == NULL)
		return NULL;
...
double VvtDiffExecuter::getPosition(const char* stdCode, bool validOnly /* = true */, int32_t flag /* = 3 */)
{
	if (NULL == _trader)
		return 0.0;
...
double VvtDiffExecuter::getUndoneQty(const char* stdCode)
{
	if (NULL == _trader)
		return 0.0;
...
OrderMap* VvtDiffExecuter::getOrders(const char* stdCode)
{
	if (NULL == _trader)
		return NULL;
...
OrderIDs VvtDiffExecuter::buy(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
OrderIDs VvtDiffExecuter::sell(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
bool VvtDiffExecuter::cancel(uint32_t localid)
{
	if (!_channel_ready)
		return false;
...
OrderIDs VvtDiffExecuter::cancel(const char* stdCode, bool isBuy, double qty)
{
	if (!_channel_ready)
		return OrderIDs();
...
void VvtDiffExecuter::writeLog(const char* message)
{
...
VvTSCommodityInfo* VvtDiffExecuter::getCommodityInfo(const char* stdCode)
{
	return _stub->get_comm_info(stdCode);
}

VvTSSessionInfo* VvtDiffExecuter::getSessionInfo(const char* stdCode)
{
	return _stub->get_sess_info(stdCode);
}

uint64_t VvtDiffExecuter::getCurTime()
{
...
#pragma region 外部接口
void VvtDiffExecuter::on_position_changed(const char* stdCode, double diffPos)
{
...
void VvtDiffExecuter::set_position(const vvt_hashmap<std::string, double>& targets)
{
...
void VvtDiffExecuter::on_tick(const char* stdCode, VvTSTickData* newTick)
{
...
void VvtDiffExecuter::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void VvtDiffExecuter::on_order(uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled /* = false */)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void VvtDiffExecuter::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void VvtDiffExecuter::on_channel_ready()
{
	_channel_ready = true;
	//SpinLock lock(_mtx_units);
...
void VvtDiffExecuter::on_channel_lost()
{
	_channel_ready = false;
	//SpinLock lock(_mtx_units);
	for (auto it = _unit_map.begin(); it != _unit_map.end(); it++)
...
void VvtDiffExecuter::on_account(const char* currency, double prebalance, double balance, double dynbalance,
...
void VvtDiffExecuter::on_position(const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail, uint32_t tradingday)
{
