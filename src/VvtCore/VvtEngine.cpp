...
void VvtEngine::set_date_time(uint32_t curDate, uint32_t curTime, uint32_t curSecs /* = 0 */, uint32_t rawTime /* = 0 */)
{
	_cur_date = curDate;
	_cur_time = curTime;
	_cur_secs = curSecs;
...
void VvtEngine::set_trading_date(uint32_t curTDate)
{
	_cur_tdate = curTDate; 
...
VvTSCommodityInfo* VvtEngine::get_commodity_info(const char* stdCode)
{
...
VvTSContractInfo* VvtEngine::get_contract_info(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	return _base_data_mgr->getContract(cInfo._code, cInfo._exchg);
}

std::string VvtEngine::get_rawcode(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	if (cInfo.hasRule())
	{
...
VvTSSessionInfo* VvtEngine::get_session_info(const char* sid, bool isCode /* = false */)
{
	if (!isCode)
...
void VvtEngine::on_tick(const char* stdCode, VvTSTickData* curTick)
{
...
void VvtEngine::update_fund_dynprofit()
{
...
void VvtEngine::writeRiskLog(const char* message)
{
...
uint32_t VvtEngine::getCurDate()
{
	return _cur_date;
}

uint32_t VvtEngine::getCurTime()
{
	return _cur_time;
}

uint32_t VvtEngine::getTradingDate()
{
	return _cur_tdate;
}

bool VvtEngine::isInTrading()
{
	return false;
}

void VvtEngine::setVolScale(double scale)
{
...
VvTSPortFundInfo* VvtEngine::getFundInfo()
{
	update_fund_dynprofit();
	save_datas();
...
void VvtEngine::init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier)
{
...
void VvtEngine::on_session_begin()
{

}

void VvtEngine::save_datas()
{
...
VvTSTickSlice* VvtEngine::get_tick_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_tick_slice(code, count);
}

VvTSTickData* VvtEngine::get_last_tick(uint32_t sid, const char* stdCode)
{
	return _data_mgr->grab_last_tick(stdCode);
}

VvTSKlineSlice* VvtEngine::get_kline_slice(uint32_t sid, const char* stdCode, const char* period, uint32_t count, uint32_t times /* = 1 */, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* cInfo = _base_data_mgr->getCommodity(codeInfo._exchg, codeInfo._product);
	if (cInfo == NULL)
...
void VvtEngine::handle_push_quote(VvTSTickData* curTick)
{
...
double VvtEngine::get_cur_price(const char* stdCode)
{
...
double VvtEngine::get_day_price(const char* stdCode, int flag /* = 0 */)
{
	auto len = strlen(stdCode);
	char lastChar = stdCode[len - 1];
	//前复权直接读取标准合约代码
...
double VvtEngine::get_exright_factor(const char* stdCode, VvTSCommodityInfo* commInfo /* = NULL */)
{
	if (commInfo == NULL)
...
uint32_t VvtEngine::get_adjusting_flag()
{
	if (_data_mgr)
...
void VvtEngine::sub_tick(uint32_t sid, const char* stdCode)
{
...
void VvtEngine::load_fees(const char* filename)
{
...
double VvtEngine::calc_fee(const char* stdCode, double price, double qty, uint32_t offset)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
...
void VvtEngine::append_signal(const char* stdCode, double qty, bool bStandBy /* = true */)
{
	/*
...
void VvtEngine::do_set_position(const char* stdCode, double qty, double curPx /* = -1 */)
{
	PosInfoPtr& pInfo = _pos_map[stdCode];
	if (pInfo == NULL)
		pInfo.reset(new PosInfo);
...
void VvtEngine::push_task(TaskItem task)
{
	{
...
bool VvtEngine::init_riskmon(VvTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	_risk_mon.reset(new VvtRiskMonWrapper(_risk_fact._fact->createRiskMonotor(name), _risk_fact._fact));
	_risk_mon->self()->init(this, cfg);

	return true;
}

void VvtEngine::init_outputs()
{
...
void VvtEngine::log_trade(const char* stdCode, bool isLong, bool isOpen, uint64_t curTime, double price, double qty, double fee /* = 0.0 */)
{
	if (_trade_logs)
	{
		std::stringstream ss;
...
void VvtEngine::log_close(const char* stdCode, bool isLong, uint64_t openTime, double openpx, uint64_t closeTime, double closepx, double qty, double profit, double totalprofit /* = 0 */)
{
	if (_close_logs)
	{
		std::stringstream ss;
