...
#include "VvtCtaEngine.h"
#include "VvtDtMgr.h"
#include "VvtCtaTicker.h"
#include "VvtHelper.h"
...
void VvtCtaEngine::run()
{
	_tm_ticker = new VvtCtaRtTicker(this);
...
void VvtCtaEngine::init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier /* = NULL */)
{
	VvtEngine::init(cfg, bdMgr, dataMgr, hotMgr, notifier);
...
void VvtCtaEngine::addContext(CtaContextPtr ctx)
{
	uint32_t sid = ctx->id();
	_ctx_map[sid] = ctx;
}

CtaContextPtr VvtCtaEngine::getContext(uint32_t id)
{
...
void VvtCtaEngine::on_session_begin()
{
...
void VvtCtaEngine::on_session_end()
{
	VvtEngine::on_session_end();
...
void VvtCtaEngine::on_schedule(uint32_t curDate, uint32_t curTime)
{
	//去检查一下过滤器
...
void VvtCtaEngine::handle_push_quote(VvTSTickData* newTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(newTick);
}

void VvtCtaEngine::handle_pos_change(const char* straName, const char* stdCode, double diffPos)
{
...
void VvtCtaEngine::on_tick(const char* stdCode, VvTSTickData* curTick)
{
	VvtEngine::on_tick(stdCode, curTick);
...
void VvtCtaEngine::on_bar(const char* stdCode, const char* period, uint32_t times, VvTSBarStruct* newBar)
{
...
bool VvtCtaEngine::isInTrading()
{
	return _tm_ticker->is_in_trading();
}

uint32_t VvtCtaEngine::transTimeToMin(uint32_t uTime)
{
	return _tm_ticker->time_to_mins(uTime);
}

VvTSCommodityInfo* VvtCtaEngine::get_comm_info(const char* stdCode)
{
...
VvTSSessionInfo* VvtCtaEngine::get_sess_info(const char* stdCode)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
...
uint64_t VvtCtaEngine::get_real_time()
{
	return TimeUtils::makeTime(_cur_date, _cur_raw_time * 100000 + _cur_secs);
}

void VvtCtaEngine::notify_chart_marker(uint64_t time, const char* straId, double price, const char* icon, const char* tag)
{
	if (_notifier)
		_notifier->notify_chart_marker(time, straId, price, icon, tag);
}

void VvtCtaEngine::notify_chart_index(uint64_t time, const char* straId, const char* idxName, const char* lineName, double val)
{
	if (_notifier)
		_notifier->notify_chart_index(time, straId, idxName, lineName, val);
}

void VvtCtaEngine::notify_trade(const char* straId, const char* stdCode, bool isLong, bool isOpen, uint64_t curTime, double price, const char* userTag)
{
	if (_notifier)
