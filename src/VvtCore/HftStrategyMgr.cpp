...
bool HftStrategyMgr::loadFactories(const char* path)
{
	...
	{
		VvTSLogger::error("Directory {} of HFT strategy factory not exists", path);
		return false;
	}
	...
	for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++)
	{
		...
		if (pFact != NULL)
		{
			...
			fInfo._fact = pFact;
			VvTSLogger::info("HFT strategy factory[{}] loaded", pFact->getName());
	...
	VvTSLogger::info("{} HFT strategy factories in directory[{}] loaded", count, path);
