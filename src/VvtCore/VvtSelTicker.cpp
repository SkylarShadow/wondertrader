...
VvtSelRtTicker::VvtSelRtTicker(VvtSelEngine* engine)
	: _engine(engine)
	, _stopped(false)
	, _date(0)
	, _time(UINT_MAX)
...
VvtSelRtTicker::~VvtSelRtTicker()
{
}

void VvtSelRtTicker::init(IDataReader* store, const char* sessionID)
{
	_store = store;
...
void VvtSelRtTicker::trigger_price(VvTSTickData* curTick, uint32_t hotFlag /* = 0 */)
{
	if (_engine)
	{
...
void VvtSelRtTicker::on_tick(VvTSTickData* curTick, uint32_t hotFlag /* = 0 */)
{
	if (_thrd == NULL)
	{
