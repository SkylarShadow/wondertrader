...
VvtHftRtTicker::VvtHftRtTicker(VvtHftEngine* engine)
	: _engine(engine)
	, _stopped(false)
	, _date(0)
	, _time(UINT_MAX)
...
VvtHftRtTicker::~VvtHftRtTicker()
{
}

void VvtHftRtTicker::init(IDataReader* store, const char* sessionID)
{
	_store = store;
...
void VvtHftRtTicker::trigger_price(VvTSTickData* curTick)
{
	if (_engine)
	{
...
void VvtHftRtTicker::on_tick(VvTSTickData* curTick)
{
	if (_thrd == NULL)
	{
