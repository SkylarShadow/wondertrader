...
bool VvtDtMgr::initStore(VvTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	if (module.empty())
		module = VvtHelper::getInstDir() + DLLHelper::wrap_module("VvtDataStorage");
	else
...
bool VvtDtMgr::init(VvTSVariant* cfg, VvtEngine* engine, bool bForceCache /* = false */)
{
	_engine = engine;
...
void VvtDtMgr::on_all_bar_updated(uint32_t updateTime)
{
...
IBaseDataMgr* VvtDtMgr::get_basedata_mgr()
{ 
	return _engine->get_basedata_mgr(); 
}

IHotMgr* VvtDtMgr::get_hot_mgr() 
{ 
	return _engine->get_hot_mgr(); 
}

uint32_t VvtDtMgr::get_date() 
{ 
	return _engine->get_date(); 
}

uint32_t VvtDtMgr::get_min_time()
{ 
	return _engine->get_min_time(); 
}

uint32_t VvtDtMgr::get_secs() 
{ 
	return _engine->get_secs(); 
}

void VvtDtMgr::reader_log(VvTSLogLevel ll, const char* message)
{
	VvTSLogger::log_raw(ll, message);
}

void VvtDtMgr::on_bar(const char* code, VvTSKlinePeriod period, VvTSBarStruct* newBar)
{
	...
	VvTSSessionInfo* sInfo = _engine->get_session_info(code, true);
...
void VvtDtMgr::handle_push_quote(const char* stdCode, VvTSTickData* newTick)
{
	if (newTick == NULL)
		return;
	...
	{
		VvTSHisTickData* tData = (VvTSHisTickData*)_ticks_adjusted->get(stdCode);
		if (tData == NULL)
			return;
...
VvTSTickData* VvtDtMgr::grab_last_tick(const char* code)
{
	if (_rt_tick_map == NULL)
		return NULL;
...
double VvtDtMgr::get_adjusting_factor(const char* stdCode, uint32_t uDate)
{
	if (_reader)
...
uint32_t VvtDtMgr::get_adjusting_flag()
{
...
VvTSTickSlice* VvtDtMgr::get_tick_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
	...
	if (it == _ticks_adjusted->end())
	{
		...
		VvTSTickSlice* slice = _reader->readTickSlice(pureStdCode.c_str(), 999999, etime);
		std::vector<VvTSTickStruct> ayTicks;
		ayTicks.resize(slice->size());
		std::size_t offset = 0;
		for (std::size_t bIdx = 0; bIdx < slice->get_block_counts(); bIdx++)
		{
			memcpy(&ayTicks[0] + offset, slice->get_block_addr(bIdx), slice->get_block_size(bIdx) * sizeof(VvTSTickStruct));
			offset += slice->get_block_size(bIdx);
		}

		//缓存的数据做一个复权处理
		for (VvTSTickStruct& tick : ayTicks)
		{
			tick.price *= factor;
			tick.open *= factor;
			tick.high *= factor;
		...
		//添加到缓存中
		VvTSHisTickData* hisTick = VvTSHisTickData::create(stdCode, false, factor);
		hisTick->getDataRef().swap(ayTicks);
		_ticks_adjusted->add(pureStdCode, hisTick, false);
	}

	VvTSHisTickData* hisTick = (VvTSHisTickData*)_ticks_adjusted->get(pureStdCode);
	...
	VvTSTickStruct* pTick = std::lower_bound(&ticks.front(), &ticks.back(), eTick, [](const VvTSTickStruct& a, const VvTSTickStruct& b) {
	...
	VvTSTickSlice* slice = VvTSTickSlice::create(stdCode, &ticks.front() + sIdx, cnt);
	return slice;
}

VvTSOrdQueSlice* VvtDtMgr::get_order_queue_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
VvTSOrdDtlSlice* VvtDtMgr::get_order_detail_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
VvTSTransSlice* VvtDtMgr::get_transaction_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
VvTSKlineSlice* VvtDtMgr::get_kline_slice(const char* stdCode, VvTSKlinePeriod period, uint32_t times, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
	...
	if (kData == NULL || kData->size() < count)
	{
		...
		if (rawData != NULL && rawData->size() > 0)
		{
			...
			{
				kData = VvTSKlineData::create(stdCode, rawData->size());
				kData->setPeriod(period, 1);
				kData->setClosed(true);
				VvTSBarStruct* pBar = kData->getDataRef().data();
				for(uint32_t bIdx = 0; bIdx < rawData->get_block_counts(); bIdx++ )
				{
					memcpy(pBar, rawData->get_block_addr(bIdx), sizeof(VvTSBarStruct)*rawData->get_block_size(bIdx));
