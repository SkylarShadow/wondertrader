...
//////////////////////////////////////////////////////////////////////////
//WtTimeTicker
void VvtCtaRtTicker::init(IDataReader* store, const char* sessionID)
{
	_store = store;
...
void VvtCtaRtTicker::trigger_price(VvTSTickData* curTick)
{
	if (_engine )
	{
...
void VvtCtaRtTicker::on_tick(VvTSTickData* curTick)
{
	if (_thrd == NULL)
	{
...
bool VvtCtaRtTicker::is_in_trading() const 
{
	if (_s_info == NULL)
		return false;
...
uint32_t VvtCtaRtTicker::time_to_mins(uint32_t uTime) const
{
	if (_s_info == NULL)
		return uTime;
