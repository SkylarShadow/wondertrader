...
class VvtRiskMonWrapper
{
public:
	VvtRiskMonWrapper(VvtRiskMonitor* mon, IRiskMonitorFact* fact) :_mon(mon), _fact(fact){}
	~VvtRiskMonWrapper()
	{
		if (_mon)
		{
...
typedef std::shared_ptr<VvtRiskMonWrapper>	VvtRiskMonPtr;
...
class VvtEngine : public VvtPortContext, public IParserStub
{
public:
	VvtEngine();
	...
	inline void setRiskMonitor(VvtRiskMonPtr& monitor)
	{
		_risk_mon = monitor;
	}
...
public:
	virtual void init(VvTSVariant* cfg, IBaseDataMgr* bdMgr, VvtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier);
