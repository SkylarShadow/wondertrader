...
bool SelMocker::init_sel_factory(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	if (cfgStra)
	{
		...
		if (_strategy)
		{
			ZTSLogger::info("Strategy {}.{} created,strategy ID: {}", _factory._fact->getName(), _strategy->getName(), _strategy->id());
		}
...
void SelMocker::handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void SelMocker::handle_tick(const char* stdCode, ZTSTickData* newTick, uint32_t pxType)
{
...
//////////////////////////////////////////////////////////////////////////
//回调函数
void SelMocker::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void SelMocker::on_init()
{
	...
	ZTSLogger::info("SEL Strategy initialized with {} slippage: {}", _ratio_slippage ? "ratio" : "absolute", _slippage);
}
...
void SelMocker::on_tick(const char* stdCode, ZTSTickData* newTick, bool bEmitStrategy /* = true */)
{
	//By Wesley @ 2022.04.19
	//这个逻辑迁移到handle_tick去了
}

void SelMocker::on_bar_close(const char* code, const char* period, ZTSBarStruct* newBar)
{
	if (_strategy)
		_strategy->on_bar(this, code, period, newBar);
}

void SelMocker::on_tick_updated(const char* code, ZTSTickData* newTick)
{
...
void SelMocker::stra_set_position(const char* stdCode, double qty, const char* userTag /* = "" */)
{
	...
	if (commInfo->isT1())
	{
		...
		{
			ZTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR, 
...
ZTSKlineSlice* SelMocker::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
	...
	ZTSKlineSlice* kline = _replayer->get_kline_slice(stdCode, basePeriod, count, times, false);
...
ZTSTickSlice* SelMocker::stra_get_ticks(const char* stdCode, uint32_t count)
{
	return _replayer->get_tick_slice(stdCode, count);
}

ZTSTickData* SelMocker::stra_get_last_tick(const char* stdCode)
{
...
ZTSCommodityInfo* SelMocker::stra_get_comminfo(const char* stdCode)
{
...
ZTSSessionInfo* SelMocker::stra_get_sessinfo(const char* stdCode)
{
