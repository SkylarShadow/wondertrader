...
bool proc_block_data(const char* tag, std::string& content, bool isBar, bool bKeepHead = true)
{
	...
	if (bCmped)
	{
		...
		{
			ZTSLogger::error("Size check failed while processing {} data of {}", isBar ? "bar" : "tick", tag);
			return false;
		}
	...
	if (bOldVer)
	{
		if (isBar)
		{
			...
			ZTSLogger::debug("{} bars of {} transferd to new version...", barcnt, tag);
		}
		else
		{
			...
			ZTSLogger::debug("{} ticks of {} transferd to new version...", tick_cnt, tag);
		}
	}
...
bool HisDataReplayer::init(ZTSVariant* cfg, EventNotifier* notifier /* = NULL */, IBtDataLoader* dataLoader /* = NULL */)
{
	_notifier = notifier;
	...
	if(_mode == "storage" || _mode == "bin" || _mode == "vvtp")
	{
		...
		{
			ZTSVariant* item = ZTSVariant::createObject();
	...
	ZTSLogger::info("Backtest time range is set to be [{},{}] via config", _begin_time, _end_time);

	_cache_clear_days = cfg->getUInt32("cache_clear_days");
	ZTSLogger::info("Unused cache data will be cleard in {} days", _cache_clear_days);
	

	_tick_enabled = cfg->getBoolean("tick");
	ZTSLogger::info("Tick data replaying is {}", _tick_enabled ? "enabled" : "disabled");

	_adjust_flag = cfg->getUInt32("adjust_flag");
	ZTSLogger::info("adjust_flag is {}", _adjust_flag);
	...
	ZTSLogger::info("nosim_if_notrade is {}", _nosim_if_notrade);

	//基础数据文件
	ZTSVariant* cfgBF = cfg->get("basefiles");
...
bool HisDataReplayer::loadStkAdjFactorsFromLoader()
{
	...
	if (ret) ZTSLogger::info("Adjusting factors of {} contracts loaded via extended loader", _adj_factors.size());
	return ret;
}
...
bool HisDataReplayer::loadStkAdjFactorsFromFile(const char* adjfile)
{
	...
	{
		ZTSLogger::error("Adjust factor file {} not exists, skipped", adjfile);
		return false;
	}
	...
	ZTSVariant* doc = ZTSCfgLoader::load_from_file(adjfile);
	if (doc == NULL)
	{
		ZTSLogger::error("Parsing adjust factor file {} faield", adjfile);
		return false;
	}
	...
	{
		ZTSVariant* itemExchg = doc->get(exchg);
		for (const std::string& code : itemExchg->memberNames())
		{
			ZTSVariant* ayFacts = itemExchg->get(code);
			if (!ayFacts->isArray())
				continue;
			...
			{
				ZTSVariant* fItem = ayFacts->get(i);
				AdjFactor adjFact;
	...
	ZTSLogger::info("{} items of adjust factors for {} tickers loaded from {}", fct_cnt, stk_cnt, adjfile);
	doc->release();
	return true;
}
...
void HisDataReplayer::register_task(uint32_t taskid, uint32_t date, uint32_t time, const char* period, const char* trdtpl /* = "CHINA" */, const char* session /* = "TRADING" */)
{
	...
	ZTSLogger::info("Timed task registration succeed, frequency: {}", period);
}
...
void HisDataReplayer::clear_cache()
{
	...
	ZTSLogger::log_raw(LL_WARN, "All cached data cleared");
}
...
void HisDataReplayer::reset()
{
	...
	{
		HftDataList<ZTSTickStruct>& cacheItem = (HftDataList<ZTSTickStruct>&)m.second;
		cacheItem._cursor = UINT_MAX;
	}

	for (auto& m : _orddtl_cache)
	{
		HftDataList<ZTSOrdDtlStruct>& cacheItem = (HftDataList<ZTSOrdDtlStruct>&)m.second;
		cacheItem._cursor = UINT_MAX;
	}

	for (auto& m : _ordque_cache)
	{
		HftDataList<ZTSOrdQueStruct>& cacheItem = (HftDataList<ZTSOrdQueStruct>&)m.second;
		cacheItem._cursor = UINT_MAX;
	}

	for (auto& m : _trans_cache)
	{
		HftDataList<ZTSTransStruct>& cacheItem = (HftDataList<ZTSTransStruct>&)m.second;
		cacheItem._cursor = UINT_MAX;
	}
	...
	for (auto& m : _bars_cache)
	{
		...
		ZTSLogger::info("Reading flag of {} has been reset", m.first.c_str());
	}
...
void HisDataReplayer::dump_btstate(const char* stdCode, ZTSKlinePeriod period, uint32_t times, uint64_t stime, uint64_t etime, double progress, int64_t elapse)
{
	std::string output;
	{
	...
	std::string folder = ZtHelper::getOutputDir();
	folder += _stra_name;
	folder += "/";
...
void HisDataReplayer::notify_state(const char* stdCode, ZTSKlinePeriod period, uint32_t times, uint64_t stime, uint64_t etime, double progress)
{
	if (!_notifier)
		return;
...
uint32_t HisDataReplayer::locate_barindex(const std::string& key, uint64_t now, bool bUpperBound /* = false */)
{
	...
	auto it = std::lower_bound(barsList->_bars.begin(), barsList->_bars.end(), bar, [isDay](const ZTSBarStruct& a, const ZTSBarStruct& b) {
		if (isDay)
...
void HisDataReplayer::stop()
{
	if(!_running)
	{
		ZTSLogger::log_raw(LL_ERROR, "Backtesting is not running, no need to stop");
		return;
	}
	...
	_terminated = true;
	ZTSLogger::log_raw(LL_WARN, "Terminating flag reset to true, backtesting will quit at next round");
}
...
bool HisDataReplayer::prepare()
{
	if (_running)
	{
		ZTSLogger::log_raw(LL_ERROR, "Cannot run more than one backtesting task at the same time");
		return false;
	}
...
void HisDataReplayer::run(bool bNeedDump/* = false*/)
{
	if(_task == NULL)
	{
		...
		{
			ZTSKlinePeriod minPeriod = KP_DAY;
			uint32_t minTimes = 1;
			for(auto& m : _bars_cache)
			{
			...
			ZTSLogger::info("Main K bars automatic determined: {}", _main_key.c_str());
		}
		...
		{
			ZTSLogger::log_raw(LL_INFO, "Main K bars not subscribed and backtesting of tick data not available , replaying done");
...
void HisDataReplayer::run_by_ticks(bool bNeedDump /* = false */)
{
	...
	while (_cur_tdate <= end_tdate && !_terminated)
	{
		...
		{
			ZTSLogger::info("Start to replay tick data of {}...", _cur_tdate);
	...
	if (_terminated)
		ZTSLogger::debug("Replaying by ticks terminated forcely");

	ZTSLogger::log_raw(LL_INFO, "All back data replayed, replaying done");
	_listener->handle_replay_done();
	if (_notifier)
		_notifier->notifyEvent("BT_END");
}
...
void HisDataReplayer::run_by_bars(bool bNeedDump /* = false */)
{
	...
	ZTSSessionInfo* sInfo = get_session_info(barsList->_code.c_str(), true);
	...
	ZTSLogger::info("Start to replay back data from {}...", _begin_time);
	...
	for (; !_terminated;)
	{
		...
		if (barsList->_cursor != UINT_MAX)
		{
			...
			{
				ZTSLogger::info("{} is beyond ending time {},replaying done", nextBarTime, _end_time);
				break;
			}
			...
			if(isDay || (!isDay && sInfo->offsetTime(nextTime, false) != sInfo->getCloseTime(true)))
			{
				...
				if (_opened_tdate != nextTDate)
				{
					...
					{
						ZTSLogger::debug("Tradingday {} ends", _cur_tdate);
						_listener->handle_session_end(_cur_tdate);
					...
					ZTSLogger::debug("Tradingday {} begins", nextTDate);
		...
		{
			ZTSLogger::log_raw(LL_ERROR, "No back data initialized, replaying canceled");
			break;
		}
	}

	if (_terminated)
		ZTSLogger::debug("Replaying by bars terminated forcely");
...
void HisDataReplayer::run_by_tasks(bool bNeedDump /* = false */)
{
	...
	ZTSLogger::info("Start to backtest with task frequency from {}...", _begin_time);
	...
	if (_task->_period != TPT_Minute)
	{
		...
		for (; !_terminated;)
		{
			...
			{
				ZTSLogger::log_raw(LL_INFO, "Backtesting with task frequency is done");
				if (_listener)
				{
					_listener->handle_session_end(_cur_tdate);
					_listener->handle_replay_done();
	...
	for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++)
	{
		...
		if (barsList->_period != KP_DAY)
		{
			if (barsList->_bars.size() > barsList->_cursor)
			{
				for(;;)
				{
					ZTSBarStruct& nextBar = barsList->_bars[barsList->_cursor];
					...
					if (barTime == nowTime && bCanSim)
					{
						...
						{
							//开高低收
							ZTSTickStruct& curTS = _day_cache[barsList->_code];
							...
							ZTSTickData* curTick = ZTSTickData::create(curTS);
				...
				for (;;)
				{
					...
					if (nextBar.date == endTDate)
					{
						...
						if (ticker == it->first)
						{
							...
							ZTSCommodityInfo* commInfo = _bd_mgr.getCommodity(cInfo._exchg, cInfo._product);

							std::string realCode = barsList->_code;
							if (cInfo.isExright())
								realCode = realCode.substr(0, realCode.size() - 1);

							ZTSSessionInfo* sInfo = get_session_info(realCode.c_str(), true);
	...
	for (auto& v : _tick_sub_map)
	{
		...
		if (tickList._cursor == UINT_MAX)
		{
			...
			else
			{
				...
				ZTSTickStruct curTick;
				curTick.action_date = uDate;
				curTick.action_time = uTime * 100000;

				auto tit = std::lower_bound(tickList._items.begin(), tickList._items.end(), curTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
		...
		const ZTSTickStruct& nextTick = tickList._items[tickList._cursor - 1];
	...
	for (auto v : _trans_sub_map)
	{
		...
		if (itemList._cursor == UINT_MAX)
		{
			...
			else
			{
				...
				ZTSTransStruct curItem;
				curItem.action_date = uDate;
				curItem.action_time = uTime * 100000;

				auto tit = std::lower_bound(itemList._items.begin(), itemList._items.end(), curItem, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
					if (a.action_date != b.action_date)
						return a.action_date < b.action_date;
					else
						return a.action_time < b.action_time;
	...
	for (auto v : _orddtl_sub_map)
	{
		...
		if (itemList._cursor == UINT_MAX)
		{
			...
			else
			{
				...
				ZTSOrdDtlStruct curItem;
				curItem.action_date = uDate;
				curItem.action_time = uTime * 100000;

				auto tit = std::lower_bound(itemList._items.begin(), itemList._items.end(), curItem, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
					if (a.action_date != b.action_date)
						return a.action_date < b.action_date;
					else
						return a.action_time < b.action_time;
	...
	for (auto v : _ordque_sub_map)
	{
		...
		if (itemList._cursor == UINT_MAX)
		{
			...
			else
			{
				...
				ZTSOrdQueStruct curItem;
				curItem.action_date = uDate;
				curItem.action_time = uTime * 100000;

				auto tit = std::lower_bound(itemList._items.begin(), itemList._items.end(), curItem, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
					if (a.action_date != b.action_date)
						return a.action_date < b.action_date;
					else
						return a.action_time < b.action_time;
	...
	for (;!_terminated;)
	{
		...
		for (auto& v : _orddtl_sub_map)
		{
			...
			if (lastTime <= nextTime)
			{
				...
				ZTSOrdDtlData* newData = ZTSOrdDtlData::create(nextItem);
		...
		for (auto& v : _trans_sub_map)
		{
			...
			if (lastTime <= nextTime)
			{
				...
				ZTSTransData* newData = ZTSTransData::create(nextItem);
				newData->setCode(stdCode);
		...
		for (auto& v : _tick_sub_map)
		{
			...
			const char* stdCode = v.first.c_str();
			HftDataList<ZTSTickStruct>& tickList = _ticks_cache[stdCode];
			//By Wesley @ 2022.03.06 
			//这里加了一个数据的判断
			//如果数据为空，则不再进行回放
			...
			ZTSTickStruct& nextTick = tickList._items[tickList._cursor - 1];
			uint64_t lastTime = (uint64_t)nextTick.action_date * 1000000000 + nextTick.action_time;
			if (lastTime <= nextTime)
			{
				tickList._cursor++;
				...
				ZTSTickData* newTick = ZTSTickData::create(nextTick);
		...
		for (auto& v : _ordque_sub_map)
		{
			...
			if (lastTime <= nextTime)
			{
				...
				ZTSOrdQueData* newData = ZTSOrdQueData::create(nextItem);
				newData->setCode(stdCode);
...
{	
	ZTSLogger::log_raw(LL_DEBUG, "replaying hft data...");
	for (;;)
	{
		...
		for (auto& v : _tick_sub_map)
		{
			...
			if (lastTime <= nextTime)
			{
				...
				ZTSTickData* newData = ZTSTickData::create(nextItem);
				newData->setCode(stdCode);
...
ZTSKlineSlice* HisDataReplayer::get_kline_slice(const char* stdCode, const char* period, uint32_t count, uint32_t times /* = 1 */, bool isMain /* = false */)
{
	...
	if(sInfo == NULL)
	{
		ZTSLogger::error("Cannot find corresponding session of {}", stdCode);
		return NULL;
	}
	...
	{	
		...
		ZTSKlineSlice* rawKline = ZTSKlineSlice::create(stdCode, kp, realTimes, &rawBars->_bars[0], rawBars->_bars.size());
		rawKline->setCode(stdCode);

		static ZTSDataFactory dataFact;
		ZTSKlineData* kData = dataFact.extractKlineData(rawKline, kp, realTimes, sInfo, true, _align_by_section);
		rawKline->release();
		...
		if(kData)
		{
			...
			kData->release();
			ZTSLogger::info("{} resampled {}{} back kline of {} ready", barsList->_bars.size(), period, times, stdCode);
		}
		else
		{
			ZTSLogger::error("Resampling {}{} back kline of {} failed", period, times, stdCode);
			return NULL;
		}
	}
	...
	if (kBlkPair->_cursor == UINT_MAX)
	{
		...
		auto it = std::lower_bound(kBlkPair->_bars.begin(), kBlkPair->_bars.end(), bar, [isDay, isClosed](const ZTSBarStruct& a, const ZTSBarStruct& b){
			if (isDay)
				if (!isClosed)
					return a.date < b.date;
				else 
	...
	ZTSKlineSlice* kline = ZTSKlineSlice::create(stdCode, kp, 1, kBlkPair->_bars.data() + sIdx, realCnt);
	return kline;
}

ZTSTickSlice* HisDataReplayer::get_tick_slice(const char* stdCode, uint32_t count, uint64_t etime)
{
	if (!_tick_enabled)
		return NULL;
	...
	if (tickList._cursor == UINT_MAX)
	{
		...
		else
		{
			...
			std::size_t idx = tit - tickList._items.begin();
			const ZTSTickStruct& thisTick = *tit;
	...
	ZTSTickSlice* ticks = ZTSTickSlice::create(stdCode, tickList._items.data() + sIdx, realCnt);
	return ticks;
}

ZTSOrdDtlSlice* HisDataReplayer::get_order_detail_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	if (dataList._cursor == UINT_MAX)
	{
		...
		auto tit = std::lower_bound(dataList._items.begin(), dataList._items.end(), curItem, [](const ZTSOrdDtlStruct& a, const ZTSOrdDtlStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
	...
	ZTSOrdDtlSlice* ticks = ZTSOrdDtlSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
	return ticks;
}

ZTSOrdQueSlice* HisDataReplayer::get_order_queue_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	if (dataList._cursor == UINT_MAX)
	{
		...
		auto tit = std::lower_bound(dataList._items.begin(), dataList._items.end(), curItem, [](const ZTSOrdQueStruct& a, const ZTSOrdQueStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
	...
	ZTSOrdQueSlice* ticks = ZTSOrdQueSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
	return ticks;
}

ZTSTransSlice* HisDataReplayer::get_transaction_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	if (dataList._cursor == UINT_MAX)
	{
		...
		auto tit = std::lower_bound(dataList._items.begin(), dataList._items.end(), curItem, [](const ZTSTransStruct& a, const ZTSTransStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
	...
	ZTSTransSlice* ticks = ZTSTransSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
	return ticks;
}
...
bool HisDataReplayer::checkOrderDetails(const char* stdCode, uint32_t uDate)
{
	...
	if (bNeedCache)
	{
		...
		if (_mode == "csv")
		{
			ZTSLogger::error("Cannot use stock level2 data in csv mode!");
			return false;
		}
		else
		{
...
bool HisDataReplayer::checkTicks(const char* stdCode, uint32_t uDate)
{
	...
	{
		HftDataList<ZTSTickStruct>& tickList = (HftDataList<ZTSTickStruct>&)it->second;
...
ZTSTickData* HisDataReplayer::get_last_tick(const char* stdCode)
{
	if (!checkTicks(stdCode, _cur_tdate))
		return NULL;
	...
	return ZTSTickData::create(tickList._items[tickList._cursor - 1]);
}

ZTSCommodityInfo* HisDataReplayer::get_commodity_info(const char* stdCode)
{
...
ZTSSessionInfo* HisDataReplayer::get_session_info(const char* sid, bool isCode /* = false */)
{
	if (!isCode)
...
void HisDataReplayer::loadFees(const char* filename)
{
	...
	{
		ZTSLogger::error("Fees template file {} not exists", filename);
		return;
	}

	ZTSVariant* cfg = ZTSCfgLoader::load_from_file(filename);
	if (cfg == NULL)
	{
		ZTSLogger::error("Converting fees template file {} failed", filename);
		return;
	}
	...
	{
		ZTSVariant* cfgItem = cfg->get(key.c_str());
	...
	ZTSLogger::info("{} items of fees template loaded", _fee_map.size());
}
...
double HisDataReplayer::calc_fee(const char* stdCode, double price, double qty, uint32_t offset)
{
	...
	double ret = 0.0;
	ZTSCommodityInfo* commInfo = _bd_mgr.getCommodity(stdPID);
...
double HisDataReplayer::get_day_price(const char* stdCode, int flag /* = 0 */)
{
	if(_tick_enabled)
	{
		ZTSTickData* lastTick = get_last_tick(stdCode);
		if(lastTick != NULL)
		{
			const ZTSTickStruct& curTs = lastTick->getTickStruct();
	...
	const ZTSTickStruct& curTs = it->second;
	double price = 0.0;
	switch (flag)
	{
	case 0:
...
void HisDataReplayer::checkUnbars()
{
	for(const std::string& stdCode : _unsubbed_in_need)
	{
		...
		ZTSSessionInfo* sInfo = get_session_info(stdCode.c_str(), true);
		...
		auto it = std::lower_bound(kBlkPair->_bars.begin(), kBlkPair->_bars.end(), bar, [](const ZTSBarStruct& a, const ZTSBarStruct& b) {
			return a.time < b.time;
		});
...
bool HisDataReplayer::cacheRawTicksFromBin(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	if(!bHit)
	{
		ZTSLogger::warn("No ticks data of {} on {} found", stdCode, uDate);
		return false;
	}

	auto& ticksList = _ticks_cache[key];
	uint32_t tickcnt = 0;
	tickcnt = content.size() / sizeof(ZTSTickStruct);
...
bool HisDataReplayer::cacheRawOrdDtlFromBin(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	if (!bHit)
	{
		ZTSLogger::warn("No order detail data of {} on {} found", stdCode, uDate);
		return false;
	}

	auto& dataList = _orddtl_cache[key];
	uint32_t dataCnt = 0;
	dataCnt = content.size() / sizeof(ZTSOrdDtlStruct);
...
bool HisDataReplayer::cacheRawOrdQueFromBin(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	if (!bHit)
	{
		ZTSLogger::warn("No order queue data of {} on {} found", stdCode, uDate);
		return false;
	}

	auto& dataList = _ordque_cache[key];
	uint32_t dataCnt = 0;
	dataCnt = content.size() / sizeof(ZTSOrdQueStruct);
	dataList._items.resize(dataCnt);
	memcpy(dataList._items.data(), content.data(), content.size());
...
bool HisDataReplayer::cacheRawTransFromBin(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	if (!bHit)
	{
		ZTSLogger::warn("No transaction data of {} on {} found", stdCode, uDate);
		return false;
	}

	auto& dataList = _trans_cache[key];
	uint32_t dataCnt = 0;
	dataCnt = content.size() / sizeof(ZTSTransStruct);
	dataList._items.resize(dataCnt);
	memcpy(dataList._items.data(), content.data(), content.size());
...
bool HisDataReplayer::cacheRawTicksFromLoader(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	bool bSucc = _bt_loader->loadRawHisTicks(&dataList, stdCode, uDate, [](void* obj, ZTSTickStruct* firstItem, uint32_t count) {
		HftDataList<ZTSTickStruct>* ticks = (HftDataList<ZTSTickStruct>*)obj;
		ticks->_items.resize(count);
		ticks->_count = count;
		memcpy(ticks->_items.data(), firstItem, sizeof(ZTSTickStruct)*count);
	});
	...
	if (dataList._count > 0)
		ZTSLogger::info("{} items of back tick data of {} on {} loaded via extended loader", dataList._count, stdCode, uDate);
...
bool HisDataReplayer::cacheRawTicksFromCSV(const std::string& key, const char* stdCode, uint32_t uDate)
{
	...
	{
		//如果有格式化的历史数据文件, 则直接读取
		ZTSLogger::info("Reading data from {}...", filename);
		std::string content;
		StdFile::read_file_content(filename.c_str(), content);
		if (content.size() < sizeof(HisTickBlockV2))
		{
			ZTSLogger::error("Sizechecking of back tick data file {} failed", filename);
			return false;
		}

		ZTSLogger::info("Processing file content of {}...", filename);
		proc_block_data(filename.c_str(), content, false, false);
		uint32_t tickcnt = content.size() / sizeof(ZTSTickStruct);
		auto& ticksList = _ticks_cache[key];
		ticksList._items.resize(tickcnt);
		memcpy(ticksList._items.data(), content.data(), content.size());
		ticksList._cursor = UINT_MAX;
	...
	{
		ZTSLogger::error("Back tick data file {} not exists", filename.c_str());
		ZTSLogger::warn("If you want to use tick data in csv mode, you can use ZtPy.ZtDataHelper.store_ticks to generate dsb file", filename.c_str());
		return false;
		...
		//{
		//	ZTSLogger::error("Back tick data file {} not exists", csvfile.c_str());
		//	return false;
		//}
		...
		//ZTSLogger::info("Reading data from {}...", csvfile.c_str());
		...
		//	{
		//		ZTSLogger::info("{} items of data loaded", tickList._items.size());
		//	}
		//}
		//tickList._count = tickList._items.size();
		//ifs.close();
		//ZTSLogger::info("Data file {} all loaded, totally {} items", csvfile.c_str(), tickList._items.size());
		...
		//std::string cmpData = ZTSCmpHelper::compress_data(tickList._items.data(), sizeof(ZTSTickStruct)*tickList._count);
		//pBlk->_size = cmpData.size();
		//content.append(cmpData);

		//StdFile::write_file_content(filename.c_str(), content.c_str(), content.size());
		//ZTSLogger::info("Ticks transfered to file {}", filename.c_str());
	}

	return true;
}

bool HisDataReplayer::cacheFinalBarsFromLoader(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed /* = true */)
{
	if (NULL == _bt_loader)
		return false;
	...
	if(_bt_loader->isAutoTrans() && StdFile::exists(filename.c_str()))
	{
		...
		{
			ZTSLogger::error("Sizechecking of back kbar data file {} failed", filename.c_str());
		}
		else
		{
			HisKlineBlockV2* kBlock = (HisKlineBlockV2*)content.c_str();
			std::string rawData = ZTSCmpHelper::uncompress_data(kBlock->_data, kBlock->_size);
			uint32_t barcnt = rawData.size() / sizeof(ZTSBarStruct);
			...
			ZTSLogger::info("{} items of back {} data of {} directly loaded from dsb file, from {} to {}", barcnt, pname.c_str(), stdCode, stime, etime);
			bHit = true;
		}
	}
	...
	if(!bHit)
	{
		...
		ZTSLogger::log_raw(LL_INFO, "Reading data via extended loader...");
		...
		std::string buffer;
		bool bSucc = _bt_loader->loadFinalHisBars(barsList.get(), stdCode, period, [](void* obj, ZTSBarStruct* firstBar, uint32_t count) {
			BarsList* bars = (BarsList*)obj;
			bars->_count = count;
			bars->_bars.resize(count);
			memcpy((void*)bars->_bars.data(), firstBar, sizeof(ZTSBarStruct)*count);
		});
		...
		ZTSLogger::info("{} items of back {} data of {} loaded via extended loader, from {} to {}", barsList->_count, pname.c_str(), stdCode, stime, etime);
		...
		if(_bt_loader->isAutoTrans())
		{
			...
			std::string cmpData = ZTSCmpHelper::compress_data(barsList->_bars.data(), sizeof(ZTSBarStruct)*barsList->_count);
			kBlock->_size = cmpData.size();
			content.append(cmpData);

			StdFile::write_file_content(filename.c_str(), content.c_str(), content.size());
			ZTSLogger::info("Bars transfered to file {}", filename);
		}
	}
...
bool HisDataReplayer::cacheRawBarsFromCSV(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed/* = true*/)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);
	ZTSCommodityInfo* commInfo = _bd_mgr.getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
	...
	if (StdFile::exists(filename.c_str()))
	{
		...
		if (content.size() < sizeof(HisKlineBlockV2))
		{
			ZTSLogger::error("Sizechecking of back kbar data file {} failed", filename);
			return false;
		}
		...
		uint32_t barcnt = content.size() / sizeof(ZTSBarStruct);
		...
		ZTSLogger::info("{} items of back {} data of {} directly loaded from dsb file, from {} to {}", barcnt, p_suffix.c_str(), stdCode, stime, etime);
	}
	else
	{
		//如果没有格式化的历史数据文件, 则从csv加载
		...
		if (!StdFile::exists(csvfile.c_str()))
		{
			ZTSLogger::error("Back kbar data file {} not exists", csvfile);
			return false;
		}
		...
		ZTSLogger::info("Reading data from {}, with fields: {}...", csvfile, reader.fields());
		...
		while (reader.next_row())
		{
			...
			{
				ZTSLogger::info("{} lines of data loaded", barsList->_bars.size());
			}
		}
		...
		ZTSLogger::info("Data file {} all loaded, totally {} items, from {} to {}", csvfile.c_str(), barsList->_bars.size(), stime, etime);
...
bool HisDataReplayer::cacheIntegratedFutBarsFromBin(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed /* = true */)
{
	...
	std::vector<std::vector<ZTSBarStruct>*> barsSections;
	...
	std::vector<ZTSBarStruct>* hotAy = NULL;
	...
	do
	{
		...
		if(!bSucc)
		{
			ZTSLogger::warn("Loading {} bars of {} via HisDtMgr failed", PERIOD_NAME[period], stdCode);
			break;
		}
		...
		hotAy = new std::vector<ZTSBarStruct>();
		hotAy->resize(barcnt);
		...
		ZTSLogger::info("{} items of back {} data of hot contract {} directly loaded, from {} to {}", barcnt, pname.c_str(), stdCode, stime, etime);
	...
	for (auto it = secs.rbegin(); it != secs.rend(); it++)
	{
		...
		if (NULL != _bt_loader)
		{
			...
			bLoaded = _bt_loader->loadRawHisBars(&buffer, wCode.c_str(), period, [](void* obj, ZTSBarStruct* bars, uint32_t count) {
				std::string* buff = (std::string*)obj;
				buff->resize(sizeof(ZTSBarStruct)*count);
				memcpy((void*)buff->c_str(), bars, sizeof(ZTSBarStruct)*count);
			});
		...
		if (!bLoaded)
		{
			...
			if (!bLoaded)
			{
				ZTSLogger::warn("Loading {} bars of {} via HisDtMgr failed", PERIOD_NAME[period], curCode);
				break;
			}
		}
		...
		uint32_t barcnt = buffer.size() / sizeof(ZTSBarStruct);

		ZTSBarStruct* firstBar = (ZTSBarStruct*)buffer.data();

		ZTSBarStruct* pBar = std::lower_bound(firstBar, firstBar + (barcnt - 1), sBar, [period](const ZTSBarStruct& a, const ZTSBarStruct& b) {
			if (period == KP_DAY)
			{
				return a.date < b.date;
			}
		...
		pBar = std::lower_bound(firstBar + sIdx, firstBar + (barcnt - 1), eBar, [period](const ZTSBarStruct& a, const ZTSBarStruct& b) {
			if (period == KP_DAY)
			{
				return a.date < b.date;
			}
		...
		std::vector<ZTSBarStruct>* tempAy = new std::vector<ZTSBarStruct>();
		tempAy->resize(curCnt);
		memcpy(tempAy->data(), &firstBar[sIdx], sizeof(ZTSBarStruct)*curCnt);
		realCnt += curCnt;
	...
	if (realCnt > 0)
	{
		...
		{
			std::vector<ZTSBarStruct>* tempAy = *it;
			memcpy(barsList->_bars.data() + curIdx, tempAy->data(), tempAy->size() * sizeof(ZTSBarStruct));
			curIdx += tempAy->size();
			delete tempAy;
		}
		barsSections.clear();
	...
	ZTSLogger::info("{} items of back {} data of {} cached", realCnt, pname, stdCode);
...
const HisDataReplayer::AdjFactorList& HisDataReplayer::getAdjFactors(const char* code, const char* exchg, const char* pid /* = "" */)
{
	...
	if (it == _adj_factors.end())
	{
		...
		if (_bt_loader)
		{
            ZTSLogger::info("No adjusting factors of {} cached, searching via extented loader...", key);
			_bt_loader->loadAdjFactors(this, key, [](void* obj, const char* stdCode, uint32_t* dates, double* factors, uint32_t count) {
                ...
                ZTSLogger::info("{} items of adjusting factors of {} loaded via extended loader", count, stdCode);
			});
		}
	}
...
bool HisDataReplayer::cacheAdjustedStkBarsFromBin(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed /* = true */)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	const char* stdPID = cInfo->stdCommID();
	...
	std::vector<ZTSBarStruct>* adjustedBars = NULL;
	uint64_t lastQTime = 0;

	ZTSLogger::info("Loading adjusted bars of {}...", stdCode);
	do
	{
		...
		adjustedBars = new std::vector<ZTSBarStruct>();
		...
		ZTSLogger::info("{} items of adjusted back {} data of {} directly loaded", barcnt, PERIOD_NAME[period], stdCode);
	} while (false);

	ZTSLogger::info("Loading raw bars of {}...", stdCode);
	do
	{
		...
		if (NULL != _bt_loader)
		{
			...
			if (bLoaded)
				ZTSLogger::debug("Raw bars of {} loaded via extended loader", stdCode);
		}
		...
		std::size_t barcnt = buffer.size() / sizeof(ZTSBarStruct);
		...
		if (pBar != NULL)
		{
			...
			if (!ayFactors.empty())
			{
				ZTSLogger::info("Adjusting bars of {} with adjusting factors...", stdCode);
				//做复权处理
				...
				for (auto it = ayFactors.rbegin(); it != ayFactors.rend(); it++)
				{
					...
					ZTSBarStruct* pBar = NULL;
					pBar = std::lower_bound(firstBar, firstBar + lastIdx - 1, bar, [period](const ZTSBarStruct& a, const ZTSBarStruct& b) {
						return a.date < b.date;
					});
			...
			{
				ZTSLogger::info("No adjusting factors of {} found, ajusting task skipped...", stdCode);
			}
	...
	ZTSLogger::info("{} items of back {} data of {} cached", realCnt, PERIOD_NAME[period], stdCode);

	return true;
}


bool HisDataReplayer::cacheRawBarsFromBin(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed/* = true*/)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);
	ZTSCommodityInfo* commInfo = _bd_mgr.getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = cInfo.stdCommID();
	...
	if (NULL != _bt_loader)
	{
		bLoaded = _bt_loader->loadRawHisBars(&buffer, stdCode, period, [](void* obj, ZTSBarStruct* bars, uint32_t count) {
			std::string* buff = (std::string*)obj;
			buff->resize(sizeof(ZTSBarStruct)*count);
			memcpy((void*)buff->c_str(), bars, sizeof(ZTSBarStruct)*count);
		});
...
void HisDataReplayer::check_cache_days()
{
	...
	ZTSLogger::info("Cached bars of {} cleared due to outdated", codes);
}