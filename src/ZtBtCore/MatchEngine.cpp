...
void MatchEngine::init(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return;
...
void MatchEngine::match_orders(ZTSTickData* curTick, OrderIDs& to_erase)
{
	...
	for (auto& v : _orders)
	{
		...
		if (ordInfo._state == 9)//要撤单
		{
			...
			ZTSLogger::info("订单{}已撤销, 剩余数量: {}", localid, ordInfo._left*(ordInfo._buy ? 1 : -1));
			ordInfo._left = 0;
			continue;
		}
...
void MatchEngine::update_lob(ZTSTickData* curTick)
{
...
{
	ZTSTickData* lastTick = grab_last_tick(stdCode);
	if (lastTick == NULL)
		return OrderIDs();
...
void MatchEngine::handle_tick(const char* stdCode, ZTSTickData* curTick)
{
	if (NULL == curTick)
		return;

	if (NULL == _tick_cache)
		_tick_cache = ZTSTickCache::create();
...
ZTSTickData* MatchEngine::grab_last_tick(const char* stdCode)
{
	if (NULL == _tick_cache)
		return NULL;

	return (ZTSTickData*)_tick_cache->grab(stdCode);
}