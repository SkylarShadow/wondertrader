...
bool CtaMocker::init_cta_factory(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	ZTSVariant* cfgStra = cfg->get("strategy");
	if (cfgStra)
	{
		_strategy = _factory._fact->createStrategy(cfgStra->getCString("name"), cfgStra->getCString("id"));
		if(_strategy)
		{
			ZTSLogger::info("Strategy {}.{} is created,strategy ID: {}", _factory._fact->getName(), _strategy->getName(), _strategy->id());
		}
...
void CtaMocker::load_incremental_data(const char* incremental_backtest_base)
{
	...
	folder += "/";
	ZTSLogger::info("loading incremental data from: {}", folder);
	...
	{
		ZTSLogger::info("load incremental data json: {}", strategyDumpFilename);
		...
		if (d.HasMember("conditions") && d["conditions"].HasMember("items"))
		{
			...
			for (rj::Value::ConstMemberIterator itr = conditionItemsEntry.MemberBegin(); itr != conditionItemsEntry.MemberEnd(); ++itr)
			{
				...
				for (rj::SizeType i = 0; i < itr->value.Size(); i++)
				{
					...
					condEntrust._field = (ZTSCompareField)conditionItemStkCondEntry["field"].GetInt();
					condEntrust._alg = (ZTSCompareType)conditionItemStkCondEntry["alg"].GetInt();
	...
	{
		ZTSLogger::warn("fail load incremental data json: {}", strategyDumpFilename);
	}
}
...
void CtaMocker::handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void CtaMocker::handle_replay_done()
{
	...
	if(_emit_times > 0)
	{
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO, 
	...
	{
		ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_DEBUG, "Replay done, notify control thread");
		while(_wait_calc)
			_cond_calc.notify_all();
		ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_DEBUG, "Notify control thread the end done");
	}

	ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Notify strategy the end of backtest");
	this->on_bactest_end();
}
...
void CtaMocker::handle_tick(const char* stdCode, ZTSTickData* newTick, uint32_t pxType /* = 0 */)
{
...
//////////////////////////////////////////////////////////////////////////
//回调函数
void CtaMocker::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void CtaMocker::on_init()
{
	...
	ZTSLogger::info("CTA Strategy initialized with {} slippage: {}", _ratio_slippage?"ratio":"absolute", _slippage);
}
...
void CtaMocker::update_dyn_profit(const char* stdCode, double price)
{
	...
	if (it != _pos_map.end())
	{
		...
		{
			ZTSCommodityInfo* commInfo = _replayer->get_commodity_info(stdCode);
			double dynprofit = 0;
...
void CtaMocker::on_tick(const char* stdCode, ZTSTickData* newTick, bool bEmitStrategy /* = true */)
{
	//这个逻辑全部迁移到handle_tick里去了
}

void CtaMocker::on_bar_close(const char* code, const char* period, ZTSBarStruct* newBar)
{
	if (_strategy)
		_strategy->on_bar(this, code, period, newBar);
}

void CtaMocker::on_tick_updated(const char* code, ZTSTickData* newTick)
{
...
void CtaMocker::enable_hook(bool bEnabled /* = true */)
{
	...
	ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Calculating hook {}", bEnabled?"enabled":"disabled");
}
...
void CtaMocker::install_hook()
{
	...
	ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "CTA hook installed");
}
...
bool CtaMocker::step_calc()
{
	...
	if(bNotify)
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Notify calc thread, wait for calc done");
	...
	if(_in_backtest)
	{
		...
		_wait_calc = false;
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Calc done notified");
	...
	{
		_hook_valid = false;
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Backtest exit automatically");
		return false;
	}
}
...
bool CtaMocker::on_schedule(uint32_t curDate, uint32_t curTime)
{
	...
	for (auto it = _kline_tags.begin(); it != _kline_tags.end(); it++)
	{
		...
		ZTSSessionInfo* sInfo = _replayer->get_session_info(stdCode, true);
		...
		if (isMainUdt || _kline_tags.empty())
		{
			...
			if (offTime <= sInfo->getCloseTime(true))
			{
				...
				if(_has_hook && _hook_valid)
				{
					ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Waiting for resume notify");
					StdUniqueLock lock(_mtx_calc);
					_cond_calc.wait(_mtx_calc);
					ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Calc resumed");
					_cur_step = 1;
				}
				...
				if (_has_hook && _hook_valid)
				{
					ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Calc done, notify control thread");
					while (_cur_step==1)
						_cond_calc.notify_all();
			...
			{
				ZTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO, "{} is not trading time,strategy will not be scheduled", curTime);
			}
			break;
		}
	}
...
void CtaMocker::stra_set_position(const char* stdCode, double qty, const char* userTag /* = "" */, double limitprice /* = 0.0 */, double stopprice /* = 0.0 */)
{
	...
	if(commInfo->isT1())
	{
		...
		{
			ZTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR, "New position of {} cannot be set to {} due to {} being frozen", stdCode, qty, frozen);
			return;
		}
	}
...
ZTSKlineSlice* CtaMocker::stra_get_bars(const char* stdCode, const char* period, uint32_t count, bool isMain /* = false */)
{
	thread_local static char key[64] = { 0 };
	...
	if (isMain)
	{
		...
		{
			ZTSLogger::error("Main k bars can only be setup once");
			return NULL;
		}
	...
	ZTSKlineSlice* kline = _replayer->get_kline_slice(stdCode, basePeriod, count, times, isMain);
...
ZTSTickSlice* CtaMocker::stra_get_ticks(const char* stdCode, uint32_t count)
{
	return _replayer->get_tick_slice(stdCode, count);
}

ZTSTickData* CtaMocker::stra_get_last_tick(const char* stdCode)
{
	auto it = _ticks.find(stdCode);
	if (it != _ticks.end())
	{
		ZTSTickData* lastTick = ZTSTickData::create((ZTSTickStruct&)it->second);
		return lastTick;
	}
...
ZTSCommodityInfo* CtaMocker::stra_get_comminfo(const char* stdCode)
{
...
{
	ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_INFO, message);
}

void CtaMocker::stra_log_debug(const char* message)
{
	ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_DEBUG, message);
}

void CtaMocker::stra_log_warn(const char* message)
{
	ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_WARN, message);
}

void CtaMocker::stra_log_error(const char* message)
{
	ZTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_ERROR, message);
}
...
void CtaMocker::add_chart_mark(double price, const char* icon, const char* tag)
{
	if (!_is_in_schedule)
	{
		ZTSLogger::error("Marks can be added only during schedule");
		return;
	}
...
bool CtaMocker::register_index_line(const char* idxName, const char* lineName, uint32_t lineType)
{
	...
	{
		ZTSLogger::error("Index {} not registered", idxName);
		return false;
	}
...
bool CtaMocker::set_index_value(const char* idxName, const char* lineName, double val)
{
	...
	{
		ZTSLogger::error("Line {} of index {} not registered", lineName, idxName);
		return false;
	}
