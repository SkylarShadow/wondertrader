...
bool HftMocker::init_hft_factory(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
void HftMocker::handle_tick(const char* stdCode, ZTSTickData* curTick, uint32_t pxType)
{
	on_tick(stdCode, curTick);
}

void HftMocker::handle_order_detail(const char* stdCode, ZTSOrdDtlData* curOrdDtl)
{
	on_order_detail(stdCode, curOrdDtl);
}

void HftMocker::handle_order_queue(const char* stdCode, ZTSOrdQueData* curOrdQue)
{
	on_order_queue(stdCode, curOrdQue);
}

void HftMocker::handle_transaction(const char* stdCode, ZTSTransData* curTrans)
{
	on_transaction(stdCode, curTrans);
}

void HftMocker::handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void HftMocker::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (_strategy)
...
void HftMocker::install_hook()
{
	...
	ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "HFT hook installed");
}
...
void HftMocker::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void HftMocker::on_tick_updated(const char* stdCode, ZTSTickData* newTick)
{
...
void HftMocker::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	on_ordque_updated(stdCode, newOrdQue);
}

void HftMocker::on_ordque_updated(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_strategy)
		_strategy->on_order_queue(this, stdCode, newOrdQue);
}

void HftMocker::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	on_orddtl_updated(stdCode, newOrdDtl);
}

void HftMocker::on_orddtl_updated(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_strategy)
		_strategy->on_order_detail(this, stdCode, newOrdDtl);
}

void HftMocker::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
	on_trans_updated(stdCode, newTrans);
}

void HftMocker::on_trans_updated(const char* stdCode, ZTSTransData* newTrans)
{
	if (_strategy)
...
void HftMocker::update_dyn_profit(const char* stdCode, ZTSTickData* newTick)
{
...
bool HftMocker::procOrder(uint32_t localid)
{
	...
	ZTSTickData* curTick = stra_get_last_tick(ordInfo->_code);
	if (curTick == NULL)
		return false;
...
ZTSCommodityInfo* HftMocker::stra_get_comminfo(const char* stdCode)
{
...
ZTSKlineSlice* HftMocker::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
...
ZTSTickSlice* HftMocker::stra_get_ticks(const char* stdCode, uint32_t count)
{
	return _replayer->get_tick_slice(stdCode, count);
}

ZTSOrdQueSlice* HftMocker::stra_get_order_queue(const char* stdCode, uint32_t count)
{
	return _replayer->get_order_queue_slice(stdCode, count);
}

ZTSOrdDtlSlice* HftMocker::stra_get_order_detail(const char* stdCode, uint32_t count)
{
	return _replayer->get_order_detail_slice(stdCode, count);
}

ZTSTransSlice* HftMocker::stra_get_transaction(const char* stdCode, uint32_t count)
{
	return _replayer->get_transaction_slice(stdCode, count);
}

ZTSTickData* HftMocker::stra_get_last_tick(const char* stdCode)
{
	if (_ticks != NULL)
	{
		...
		if (it != _ticks->end())
		{
			ZTSTickData* lastTick = (ZTSTickData*)it->second;
			if (lastTick)
				lastTick->retain();
			return lastTick;
		}
