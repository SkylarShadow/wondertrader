...
public:
	//////////////////////////////////////////////////////////////////////////
	//IDataSink
	virtual void	handle_tick(const char* stdCode, ZTSTickData* curTick, uint32_t pxType) override;
	virtual void	handle_order_queue(const char* stdCode, ZTSOrdQueData* curOrdQue) override;
	virtual void	handle_order_detail(const char* stdCode, ZTSOrdDtlData* curOrdDtl) override;
	virtual void	handle_transaction(const char* stdCode, ZTSTransData* curTrans) override;

	virtual void	handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar) override;
	...
	virtual ZTSCommodityInfo* stra_get_comminfo(const char* stdCode) override;

	virtual ZTSKlineSlice* stra_get_bars(const char* stdCode, const char* period, uint32_t count) override;

	virtual ZTSTickSlice* stra_get_ticks(const char* stdCode, uint32_t count) override;

	virtual ZTSOrdDtlSlice*	stra_get_order_detail(const char* stdCode, uint32_t count) override;

	virtual ZTSOrdQueSlice*	stra_get_order_queue(const char* stdCode, uint32_t count) override;

	virtual ZTSTransSlice*	stra_get_transaction(const char* stdCode, uint32_t count) override;

	virtual ZTSTickData* stra_get_last_tick(const char* stdCode) override;
...
public:
	bool	init_hft_factory(ZTSVariant* cfg);
	void	install_hook();
...
private:
	...
	void	update_dyn_profit(const char* stdCode, ZTSTickData* newTick);
...
private:
	...
	typedef ZTSHashMap<std::string> CommodityMap;
