...
bool UftMocker::init_uft_factory(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
...
void UftMocker::handle_tick(const char* stdCode, ZTSTickData* curTick, uint32_t pxType)
{
	on_tick(stdCode, curTick);
}

void UftMocker::handle_order_detail(const char* stdCode, ZTSOrdDtlData* curOrdDtl)
{
	on_order_detail(stdCode, curOrdDtl);
}

void UftMocker::handle_order_queue(const char* stdCode, ZTSOrdQueData* curOrdQue)
{
	on_order_queue(stdCode, curOrdQue);
}

void UftMocker::handle_transaction(const char* stdCode, ZTSTransData* curTrans)
{
	on_transaction(stdCode, curTrans);
}

void UftMocker::handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void UftMocker::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (_strategy)
		_strategy->on_bar(this, stdCode, period, times, newBar);
}

void UftMocker::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void UftMocker::on_tick_updated(const char* stdCode, ZTSTickData* newTick)
{
...
void UftMocker::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	on_ordque_updated(stdCode, newOrdQue);
}

void UftMocker::on_ordque_updated(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_strategy)
		_strategy->on_order_queue(this, stdCode, newOrdQue);
}

void UftMocker::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	on_orddtl_updated(stdCode, newOrdDtl);
}

void UftMocker::on_orddtl_updated(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_strategy)
		_strategy->on_order_detail(this, stdCode, newOrdDtl);
}

void UftMocker::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
	on_trans_updated(stdCode, newTrans);
}

void UftMocker::on_trans_updated(const char* stdCode, ZTSTransData* newTrans)
{
	if (_strategy)
...
bool UftMocker::stra_cancel(uint32_t localid)
{
	postTask([this, localid](){
		...
		if (ordInfo._offset != 0)
		{
			...
			ZTSCommodityInfo* commInfo = _replayer->get_commodity_info(ordInfo._code);
...
void UftMocker::update_dyn_profit(const char* stdCode, ZTSTickData* newTick)
{
...
ZTSCommodityInfo* UftMocker::stra_get_comminfo(const char* stdCode)
{
	return _replayer->get_commodity_info(stdCode);
}

ZTSKlineSlice* UftMocker::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
...
ZTSTickSlice* UftMocker::stra_get_ticks(const char* stdCode, uint32_t count)
{
	return _replayer->get_tick_slice(stdCode, count);
}

ZTSOrdQueSlice* UftMocker::stra_get_order_queue(const char* stdCode, uint32_t count)
{
	return _replayer->get_order_queue_slice(stdCode, count);
}

ZTSOrdDtlSlice* UftMocker::stra_get_order_detail(const char* stdCode, uint32_t count)
{
	return _replayer->get_order_detail_slice(stdCode, count);
}

ZTSTransSlice* UftMocker::stra_get_transaction(const char* stdCode, uint32_t count)
{
	return _replayer->get_transaction_slice(stdCode, count);
}

ZTSTickData* UftMocker::stra_get_last_tick(const char* stdCode)
{
