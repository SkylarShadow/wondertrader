...
{
public:
	virtual void	handle_tick(const char* stdCode, ZTSTickData* curTick, uint32_t pxType) = 0;
	virtual void	handle_order_queue(const char* stdCode, ZTSOrdQueData* curOrdQue) {};
	virtual void	handle_order_detail(const char* stdCode, ZTSOrdDtlData* curOrdDtl) {};
	virtual void	handle_transaction(const char* stdCode, ZTSTransData* curTrans) {};
	virtual void	handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar) = 0;
 ...
 *	@count	K线条数
 */
typedef void(*FuncReadBars)(void* obj, ZTSBarStruct* firstBar, uint32_t count);
 ...
 *	@count		条数
 */
typedef void(*FuncReadTicks)(void* obj, ZTSTickStruct* firstItem, uint32_t count);
 ...
 *	@count		条数
 */
typedef void(*FuncReadOrdDtl)(void* obj, ZTSOrdDtlStruct* firstItem, uint32_t count);
 ...
 *	@count		条数
 */
typedef void(*FuncReadOrdQue)(void* obj, ZTSOrdQueStruct* firstItem, uint32_t count);
 ...
 *	@count		条数
 */
typedef void(*FuncReadTrans)(void* obj, ZTSTransStruct* firstItem, uint32_t count);
...
public:
	 ...
	 *	@cb		回调函数
	 */
	virtual bool loadFinalHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb) = 0;
	 ...
	 *	@cb		回调函数
	 */
	virtual bool loadRawHisBars(void* obj, const char* stdCode, ZTSKlinePeriod period, FuncReadBars cb) = 0;
...
private:
	...
	typedef vvt_hashmap<std::string, HftDataList<ZTSTickStruct>>		TickCache;
	typedef vvt_hashmap<std::string, HftDataList<ZTSOrdDtlStruct>>	OrdDtlCache;
	typedef vvt_hashmap<std::string, HftDataList<ZTSOrdQueStruct>>	OrdQueCache;
	typedef vvt_hashmap<std::string, HftDataList<ZTSTransStruct>>	TransCache;
	...
	typedef struct _BarsList
	{
		...
		{
			return sizeof(ZTSBarStruct)*_bars.size();
		}
...
private:
	 ...
	 *	从自定义数据文件缓存历史数据
	 */
	bool		cacheRawBarsFromBin(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bForBars = true);

	/*
	 *	从csv文件缓存历史数据
	 */
	bool		cacheRawBarsFromCSV(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed = true);
	 ...
	 *	从外部加载器缓存历史数据
	 */
	bool		cacheFinalBarsFromLoader(const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed = true);
	 ...
	 */
	bool		cacheIntegratedFutBarsFromBin(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed = true);

	/*
	 *	缓存复权股票K线数据
	 */
	bool		cacheAdjustedStkBarsFromBin(void* codeInfo, const std::string& key, const char* stdCode, ZTSKlinePeriod period, bool bSubbed = true);
	...
	void		dump_btstate(const char* stdCode, ZTSKlinePeriod period, uint32_t times, uint64_t stime, uint64_t etime, double progress, int64_t elapse);
	void		notify_state(const char* stdCode, ZTSKlinePeriod period, uint32_t times, uint64_t stime, uint64_t etime, double progress);
...
public:
	bool init(ZTSVariant* cfg, EventNotifier* notifier = NULL, IBtDataLoader* dataLoader = NULL);
	...
	ZTSKlineSlice* get_kline_slice(const char* stdCode, const char* period, uint32_t count, uint32_t times = 1, bool isMain = false);

	ZTSTickSlice* get_tick_slice(const char* stdCode, uint32_t count, uint64_t etime = 0);

	ZTSOrdDtlSlice* get_order_detail_slice(const char* stdCode, uint32_t count, uint64_t etime = 0);

	ZTSOrdQueSlice* get_order_queue_slice(const char* stdCode, uint32_t count, uint64_t etime = 0);

	ZTSTransSlice* get_transaction_slice(const char* stdCode, uint32_t count, uint64_t etime = 0);

	ZTSTickData* get_last_tick(const char* stdCode);
...
private:
	...
	std::map<std::string, ZTSTickStruct>	_day_cache;	//每日Tick缓存,当tick回放未开放时,会用到该缓存
