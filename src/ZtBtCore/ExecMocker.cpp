...
bool ExecMocker::init(ZTSVariant* cfg)
{
	...
	ZTSVariant* cfgExec = cfg->get("executer");
	if (cfgExec)
	{
...
ZTSCommodityInfo* ExecMocker::getCommodityInfo(const char* stdCode)
{
	return _replayer->get_commodity_info(stdCode);
}

ZTSSessionInfo* ExecMocker::getSessionInfo(const char* stdCode)
{
...
void ExecMocker::handle_bar_close(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void ExecMocker::handle_session_end(uint32_t curTDate)
{
	...
	ZTSLogger::info("Total entrust:{}, total quantity:{}, total cancels:{}, total cancel quantity:{}, total signals:{}", 
		_ord_cnt, _ord_qty, _cacl_cnt, _cacl_qty, _sig_cnt);
}

void ExecMocker::handle_tick(const char* stdCode, ZTSTickData* curTick, uint32_t pxType)
{
 	if (_last_tick)
	{
		_last_tick->release();
...
void ExecMocker::handle_init()
{
	...
	ZTSKlineSlice* kline = _replayer->get_kline_slice(_code.c_str(), basePeriod,  10, times, true);
	if (kline)
		kline->release();
	...
	ZTSLogger::info("Target position updated at the beginning: {}", _volunit);
}
...
void ExecMocker::handle_schedule(uint32_t uDate, uint32_t uTime)
{
	...
	ZTSLogger::info("Target position updated @{}.{}: {}", uDate, uTime, _volunit);
	_sig_cnt++;
}

ZTSTickSlice* ExecMocker::getTicks(const char* stdCode, uint32_t count, uint64_t etime /*= 0*/)
{
	return _replayer->get_tick_slice(stdCode, count, etime);
}

ZTSTickData* ExecMocker::grabLastTick(const char* stdCode)
{
...
OrderIDs ExecMocker::buy(const char* stdCode, double price, double qty, bool bForceClose /* = false */)
{
	...
	if(!ret.empty())
	{
		...
		_undone += (int32_t)qty;
		ZTSLogger::info("{}, undone orders updated: {}", __FUNCTION__, _undone);
	}
...
{
	ZTSLogger::log_dyn_raw("executer", _id.c_str(), LL_INFO, message);
}
...
void ExecMocker::handle_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double fireprice, double price, uint64_t ordTime)
{
	...
	ZTSLogger::info("{}, undone orders updated: {}", __FUNCTION__, _undone);
	ZTSLogger::info("Position updated: {}", _position);
