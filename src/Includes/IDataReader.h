...
public:
	 ...
	 */
	virtual void on_bar(const char* stdCode, ZTSKlinePeriod period, ZTSBarStruct* newBar) = 0;
 ...
 */
typedef void(*FuncReadBars)(void* obj, ZTSBarStruct* bars, uint32_t count);
...
public:
	 ...
	 *	@param sink	模块回调接口
	 */
	virtual void init(ZTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader = NULL) { _sink = sink; _loader = loader; }
	 ...
	 */
	virtual ZTSTickSlice*	readTickSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) = 0;
	 ...
	 *	@param etime	结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
	 */
	virtual ZTSOrdDtlSlice*	readOrdDtlSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) { return NULL; }
	/*
	 *	@brief	读取委托队列数据切片
	 *	@details 切片不会复制数据,只把缓存中的数据指针传递出来,所以叫做切片
	 *
	 ...
	 *	@param etime	结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
	 */
	virtual ZTSOrdQueSlice*	readOrdQueSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) { return NULL; }
	 ...
	 *	@param etime	结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
	 */
	virtual ZTSTransSlice*	readTransSlice(const char* stdCode, uint32_t count, uint64_t etime = 0) { return NULL; }
	 ...
	 */
	virtual ZTSKlineSlice*	readKlineSlice(const char* stdCode, ZTSKlinePeriod period, uint32_t count, uint64_t etime = 0) = 0;
