...
//////////////////////////////////////////////////////////////////////////
//线型类,指标用到
class ZTSLineInfo : public ZTSObject
{
public:
	static ZTSLineInfo* create(COLORREF clr, int width = 1, int style = 0)
	{
		ZTSLineInfo* pRet = new ZTSLineInfo();
...
//////////////////////////////////////////////////////////////////////////
//指标参数类
class ZTSExpressParams : public ZTSObject
{
public:
	static	ZTSExpressParams* create()
	{
		ZTSExpressParams* pRet = new ZTSExpressParams;
...
//////////////////////////////////////////////////////////////////////////
//指标线类
class ZTSExpressLine : public ZTSValueArray
{
public:
	static ZTSExpressLine*	create(uint32_t size, ZTSExpressLineType lineType = WELT_Polyline, uint32_t decimal = 0, uint32_t uStyle = ELS_LINE_VISIBLE|ELS_TITLE_VISIBLE)
	{
		ZTSExpressLine* pRet = new ZTSExpressLine;
	...
	ZTSExpressLine(): m_ayLineInfo(NULL){}
	...
	void		addLineInfo(ZTSLineInfo* lineInfo)
	{
		if(m_ayLineInfo == NULL)
		{
			m_ayLineInfo = ZTSArray::create();
		}
		m_ayLineInfo->append(lineInfo, true);
	}

	ZTSLineInfo*	getLineInfo(uint32_t idx = 0)
	{
		if(m_ayLineInfo == NULL || m_ayLineInfo->size()==0 || idx >= m_ayLineInfo->size())
			return NULL;

		return STATIC_CONVERT(m_ayLineInfo->at(idx), ZTSLineInfo*);
	}
	...
	void		setLineType(ZTSExpressLineType lineType){m_lineType = lineType;}
	ZTSExpressLineType getLineType() const{return m_lineType;}

protected:
	ZTSArray*		m_ayLineInfo;
	std::string		m_strLineTag;
	uint32_t		m_uStyle;

	uint32_t		m_uDecimal;
	std::string		m_strFormat;
	ZTSExpressLineType	m_lineType;
};
typedef vector<ZTSExpressLine*>	ZTSVecExpLines;


//////////////////////////////////////////////////////////////////////////
//指标类
class ZTSExpressData : public ZTSObject
{
public:
	static ZTSExpressData*	create(const char* title, ZTSExpressType eType = WET_Unique)
	{
		ZTSExpressData* pRet = new ZTSExpressData;
		pRet->setTitle(title);
		pRet->setType(eType);
		return pRet;
	}
...
protected:
	ZTSExpressData() :m_ayExpLines(NULL), m_dDevide(1.0), m_dBaseLine(0.0), m_bBaseLine(false){}

	virtual ~ZTSExpressData()
	{
...
public:
	...
	void		setType(ZTSExpressType eType){m_expType = eType;}
	ZTSExpressType	getType() const{return m_expType;}
	...
	uint32_t	addExpLine(ZTSExpressLine* line)
	{
		if(NULL == line)
			return INVALID_UINT32;

		if(NULL == m_ayExpLines)
			m_ayExpLines = ZTSArray::create();
	...
	ZTSExpressLine*	getExpLine(uint32_t idx)
	{
		if(NULL == m_ayExpLines || idx >= m_ayExpLines->size())
			return NULL;

		return STATIC_CONVERT(m_ayExpLines->at(idx), ZTSExpressLine*);
	}
	...
	bool		crossOver(uint32_t idx0, uint32_t idx1)
	{
		...
		ZTSExpressLine* line0 = STATIC_CONVERT(m_ayExpLines->at(idx0),ZTSExpressLine*);
		ZTSExpressLine* line1 = STATIC_CONVERT(m_ayExpLines->at(idx1),ZTSExpressLine*);
	...
	double		maxvalue(int32_t head, int32_t tail) const
	{
		...
		ZTSArray::ConstIterator it = m_ayExpLines->begin();
		for(; it != m_ayExpLines->end(); it++)
		{
			ZTSExpressLine* line = STATIC_CONVERT(*it, ZTSExpressLine*);
	...
	uint32_t	size() const
	{
		...
		return STATIC_CONVERT(m_ayExpLines->at(0), ZTSExpressLine*)->size();
	}
...
protected:
	ZTSArray*		m_ayExpLines;
	std::string		m_strExpTitle;
	ZTSExpressType	m_expType;
	uint32_t		m_uPrec;
	double			m_dDevide;
