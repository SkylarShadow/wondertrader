...
class ZTSSessionInfo : public ZTSObject
{
public:
	//交易时段
...
protected:
	ZTSSessionInfo(int32_t offset)
	{
		m_uOffsetMins = offset;
	}
	virtual ~ZTSSessionInfo(){}

public:
	const char* id() const{ return m_strID.c_str(); }
	const char* name() const{ return m_strName.c_str(); }

	static ZTSSessionInfo* create(const char* sid, const char* name, int32_t offset = 0)
	{
		ZTSSessionInfo* pRet = new ZTSSessionInfo(offset);
		pRet->m_strID = sid;
		pRet->m_strName = name;
		return pRet;
	}
