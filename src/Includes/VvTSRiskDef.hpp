...
class VvTSTradeStateInfo : public VvTSObject
{
protected:
	VvTSTradeStateInfo(){}

public:
	static VvTSTradeStateInfo* create(const char* code)
	{
		VvTSTradeStateInfo* pRet = new VvTSTradeStateInfo();
...
//组合资金数据
typedef struct _VvTSFundStruct
{
	...
	_VvTSFundStruct()
	{
		memset(this, 0, sizeof(_VvTSFundStruct));
...
class VvTSPortFundInfo : public VvTSObject
{
protected:
	VvTSPortFundInfo(){}

public:
	static VvTSPortFundInfo* create()
	{
		VvTSPortFundInfo* pRet = new VvTSPortFundInfo();
		return pRet;
	}

	VvTSFundStruct&	fundInfo(){ return _fund_info; }
	const VvTSFundStruct& fundInfo() const{ return _fund_info; }
