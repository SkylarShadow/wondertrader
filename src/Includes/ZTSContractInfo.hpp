...
class ZTSCommodityInfo: public ZTSObject
{
public:
	static ZTSCommodityInfo* create(const char* pid, const char* name, const char* exchg, const char* session, const char* trdtpl, const char* currency = "CNY")
	{
		ZTSCommodityInfo* ret = new ZTSCommodityInfo;
		ret->m_strName = name;
	...
	inline void		setSessionInfo(ZTSSessionInfo* sInfo) { m_pSession = sInfo; }
	inline ZTSSessionInfo* getSessionInfo() const { return m_pSession; }
...
protected:
	ZTSCommodityInfo():m_nFeeAlg(-1){}
	virtual ~ZTSCommodityInfo() {}
...
class ZTSContractInfo :	public ZTSObject
{
public:
	static ZTSContractInfo* create(const char* code, const char* name, const char* exchg, const char* pid)
	{
		ZTSContractInfo* ret = new ZTSContractInfo;
		ret->m_strCode = code;
		ret->m_strName = name;
		ret->m_strProduct = pid;
		ret->m_strExchg = exchg;
	...
	inline void setCommInfo(ZTSCommodityInfo* commInfo) { m_commInfo = commInfo; }
	inline ZTSCommodityInfo* getCommInfo() const { return m_commInfo; }
