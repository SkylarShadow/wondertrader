...
bool ParserAdapter::init(const char* id, ZTSVariant* cfg, IParserStub* stub, IBaseDataMgr* bgMgr)
{
	if (cfg == NULL)
		return false;
	...
	if (_parser_api)
	{
		...
		if (_parser_api->init(cfg))
		{
			...
			ZTSArray* ay = _bd_mgr->getContracts();
			for(auto it = ay->begin(); it != ay->end(); it++)
			{
				ZTSContractInfo* cInfo = STATIC_CONVERT(*it, ZTSContractInfo*);
