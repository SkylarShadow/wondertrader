...
bool UftStrategyMgr::loadFactories(const char* path)
{
	...
	{
		ZTSLogger::error("Directory {} of UFT strategy factory not exists", path);
		return false;
	}
	...
	for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++)
	{
		...
		if (pFact != NULL)
		{
			...
			fInfo._fact = pFact;
			ZTSLogger::info("UFT strategy factory[{}] loaded", pFact->getName());
	...
	ZTSLogger::info("{} UFT strategy factories in directory[{}] loaded", count, path);
