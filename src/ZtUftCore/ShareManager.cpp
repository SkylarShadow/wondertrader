...
bool ShareManager::initialize(const char* module)
{
	...
	{
		ZTSLogger::warn("ZtShareHelper {} not exist", module);
		return false;
	}
...
bool ShareManager::start_watching(uint32_t microsecs)
{
	...
	if (_inited && !_stopped && _worker == nullptr)
	{
		...
		ZTSLogger::info("Share domain is on watch");
	}
...
bool ShareManager::init_domain(const char* id)
{
	...
	_exchg = id;
	ZTSLogger::info("Share domain [{}] initialing {}", id, ret ? "succeed" : "failed");

	//初始化同步区
	ret = _init_master("sync", ".sync");
	ZTSLogger::info("Sync domain [sync] initialing {}", ret ? "succeed" : "failed");
