...
UftStraContext::UftStraContext(ZtUftEngine* engine, const char* name)
	: IUftStraCtx(name)
	, _engine(engine)
	, _strategy(NULL)
	, _tradingday(0)
...
void UftStraContext::on_tick(const char* stdCode, ZTSTickData* newTick)
{
	auto it = _positions.find(stdCode);
	if(it != _positions.end())
	{
		ZTSCommodityInfo* commInfo = newTick->getContractInfo()->getCommInfo();
...
void UftStraContext::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_strategy)
		_strategy->on_order_queue(this, stdCode, newOrdQue);
}

void UftStraContext::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_strategy)
		_strategy->on_order_detail(this, stdCode, newOrdDtl);
}

void UftStraContext::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
	if (_strategy)
		_strategy->on_transaction(this, stdCode, newTrans);
}

void UftStraContext::on_bar(const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (_strategy)
...
void UftStraContext::on_trade(uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double vol, double price)
{
	...
	ZTSContractInfo* cInfo = _engine->get_contract_info(stdCode);
	...
	if(isBuy)
	{
		...
		if(decimal::lt(pItem._volume, 0))
		{
			...
			for (uint32_t idx = pItem._valid_idx; idx < pItem._details.size(); idx++)
			{
				...
				//生成回合明细
				{
					...
					vvt_strcpy(rs._code, cInfo->getCode());
					vvt_strcpy(rs._exchg, cInfo->getExchg());
				...
				//落地成交明细
				{
					...
					vvt_strcpy(ts._code, cInfo->getCode());
					vvt_strcpy(ts._exchg, cInfo->getExchg());
					ts._direct = 1;
					ts._offset = 1;
					ts._price = price;
					ts._volume = maxQty;
		...
		if (decimal::gt(unhandle, 0))
		{
			...
			vvt_strcpy(ds._code, cInfo->getCode());
			vvt_strcpy(ds._exchg, cInfo->getExchg());
			ds._direct = 0;
...
void UftStraContext::on_order(uint32_t localid, const char* stdCode, bool isLong, uint32_t offset, double totalQty, double leftQty, double price, bool isCanceled /* = false */)
{
	...
	if(curOrd == NULL)
	{
		...
		vvt_strcpy(os._code, cInfo->getCode());
		vvt_strcpy(os._exchg, cInfo->getExchg());
		os._direct = 0;
		os._offset = offset;
		os._volume = totalQty;
		os._price = price;
...
ZTSCommodityInfo* UftStraContext::stra_get_comminfo(const char* stdCode)
{
	return _engine->get_commodity_info(stdCode);
}

ZTSKlineSlice* UftStraContext::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
	...
	ZTSKlineSlice* ret = _engine->get_kline_slice(_context_id, stdCode, basePeriod, count, times);
...
ZTSTickSlice* UftStraContext::stra_get_ticks(const char* stdCode, uint32_t count)
{
	ZTSTickSlice* ticks = _engine->get_tick_slice(_context_id, stdCode, count);
...
ZTSOrdDtlSlice* UftStraContext::stra_get_order_detail(const char* stdCode, uint32_t count)
{
	ZTSOrdDtlSlice* ret = _engine->get_order_detail_slice(_context_id, stdCode, count);
...
ZTSOrdQueSlice* UftStraContext::stra_get_order_queue(const char* stdCode, uint32_t count)
{
	ZTSOrdQueSlice* ret = _engine->get_order_queue_slice(_context_id, stdCode, count);
...
ZTSTransSlice* UftStraContext::stra_get_transaction(const char* stdCode, uint32_t count)
{
	ZTSTransSlice* ret = _engine->get_transaction_slice(_context_id, stdCode, count);
...
ZTSTickData* UftStraContext::stra_get_last_tick(const char* stdCode)
{
...
void UftStraContext::load_local_data()
{
	...
	std::string folder = fmtutil::format("{}{}/", ZtHelper::getOutputDir(), _name);
	...
	do
	{
		...
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_WARN, "{} detected, positions will be overwrited", mannualfile);

		ZTSVariant* manual = ZTSCfgLoader::load_from_file(mannualfile);
		if (manual == NULL)
		{
			ZTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR, "parsing mannual file {} failed", mannualfile);
			break;
		}

		ZTSVariant* ayDetails = manual->get("details");
		if(ayDetails == NULL)
			break;
		...
		if (_pos_blk._file->map(filename.c_str()))
		{
			...
			{
				ZTSVariant* objDetail = ayDetails->get(i);
				const char* exchg = objDetail->getCString("exchg");
				const char* code = objDetail->getCString("code");
				ZTSContractInfo* cInfo = _engine->get_basedata_mgr()->getContract(code, exchg);
				if(cInfo == NULL)
				{
					ZTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR, "{}.{} not exist, skip this details", exchg, code);
					continue;
				}

				uft::DetailStruct& ds = _pos_blk._block->_details[i];
				vvt_strcpy(ds._exchg, exchg);
				vvt_strcpy(ds._code, code);
		...
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_WARN, "loading mannual file {} done, {} details imported", mannualfile, _pos_blk._block->_size);
	...
	if(_pos_blk._block == NULL || _pos_blk._block->_date != _tradingday)
	{
		SpinLock lock(_pos_blk._mutex);
		std::string filename = folder + "position.membin";
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "loading local positions from {}", filename);
		bool isNew = false;
		...
		if (_pos_blk._file->map(filename.c_str()))
		{
			...
			{	
				ZTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO, "Clearing local position of {}", _pos_blk._block->_date);
				...
				for(uint32_t i = 0; i < _pos_blk._block->_size; i++)
				{
					...
					ZTSContractInfo* cInfo = _engine->get_basedata_mgr()->getContract(ds._code, ds._exchg);
					if (cInfo == NULL)
						continue;
	...
	if (_ord_blk._block == NULL || _ord_blk._block->_date != _tradingday)
	{
		...
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "loading local orders from {}", filename);
		bool isNew = false;
		if (!StdFile::exists(filename.c_str()))
		{
	...
	if (_trd_blk._block == NULL || _trd_blk._block->_date != _tradingday)
	{
		...
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "loading local trades from {}", filename);
		bool isNew = false;
		if (!StdFile::exists(filename.c_str()))
		{
	...
	if (_rnd_blk._block == NULL || _rnd_blk._block->_date != _tradingday)
	{
		...
		ZTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "loading local rouds from {}", filename);
		bool isNew = false;
		if (!StdFile::exists(filename.c_str()))
		{
