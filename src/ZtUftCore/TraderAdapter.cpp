...
bool TraderAdapter::init(const char* id, ZTSVariant* params, IBaseDataMgr* bdMgr, ActionPolicyMgr* policyMgr)
{
	...
	if (hInst == NULL)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Loading trading module %s failed", _id.c_str(), dllpath.c_str());
		return false;
	}
	else
	{
...
uint32_t TraderAdapter::doEntrust(ZTSEntrust* entrust)
{
	...
	//{
	//	ZTSContractInfo* cInfo = _bd_mgr->getContract(entrust->getCode(), entrust->getExchg());
	...
	if(ret < 0)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Order placing failed: {}", _id, ret);
		return UINT_MAX;
	}
...
OrderIDs TraderAdapter::buy(const char* stdCode, double price, double qty, int flag, bool bForceClose, ZTSContractInfo* cInfo /* = NULL */)
{
	...
	//{
	//	ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "{} is forbidden to trade", stdCode);
	//	return ret;
	//}
	...
	ZTSLogger::log_dyn("trader", _id.c_str(), LL_DEBUG, "[{}] Buying {} of quantity {}", _id.c_str(), stdCode, qty);
	...
	for (auto it = ruleGP.begin(); it != ruleGP.end(); it++)
	{
		...
		if (curRule._atype == AT_Open && !bForceClose)
		{
			...
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_DEBUG,
...
void TraderAdapter::onRspOrders(const ZTSArray* ayOrders)
{
	if (ayOrders)
	{
		...
		for (auto it = ayOrders->begin(); it != ayOrders->end(); it++)
		{
			...
			ZTSContractInfo* cInfo = _bd_mgr->getContract(orderInfo->getCode(), orderInfo->getExchg());
			if (cInfo == NULL)
				continue;
...
{
	ZTSLogger::log_dyn("trader", _id.c_str(), LL_DEBUG, "[{}] {} position updated, long:{}[{}]|{}[{}], short:{}[{}]|{}[{}]",
...
void TraderAdapter::onPushOrder(ZTSOrderInfo* orderInfo)
{
	...
	if (orderInfo->getOrderState() == WOS_Canceled && StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern.c_str(), true))
	{
		...
		//ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] {} qty of undone order updated, {} -> {}", _id.c_str(), stdCode.c_str(), oldQty, newQty);
		updateUndone(stdCode.c_str(), -qty);

		ZTSLogger::log_dyn("trader", _id.c_str(), LL_DEBUG, "[{}] Order {} of {} canceled:{}, action: {}, leftqty: {}",
