...
#include "../Includes/ZTSObject.hpp"
...
private:
	void	tradeToJson(const char* trader, uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo, std::string& output);
	void	orderToJson(const char* trader, uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo, std::string& output);

public:
	bool	init(ZTSVariant* cfg);

	void	notify(const char* trader, uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo);
	void	notify(const char* trader, uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo);
