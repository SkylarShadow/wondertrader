...
ZtUftRtTicker::ZtUftRtTicker(ZtUftEngine* engine)
	: _engine(engine)
	, _stopped(false)
	, _date(0)
	, _time(UINT_MAX)
...
ZtUftRtTicker::~ZtUftRtTicker()
{
}

void ZtUftRtTicker::init(const char* sessionID)
{
...
void ZtUftRtTicker::on_tick(ZTSTickData* curTick)
{
	if (_thrd == NULL)
	{
		if (_engine)
