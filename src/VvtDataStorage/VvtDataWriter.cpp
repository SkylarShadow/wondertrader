...
extern "C"
{
	...
	{
		IDataWriter* ret = new VvtDataWriter();
		return ret;
	}
...
VvtDataWriter::_TaskInfo::_TaskInfo(VvTSObject* data, uint64_t dtype, uint32_t flag/* = 0*/)
...
VvtDataWriter::_TaskInfo::_TaskInfo(const _TaskInfo& rhs)
...
VvtDataWriter::_TaskInfo::~_TaskInfo() 
{ 
	_obj->release(); 
}


VvtDataWriter::VvtDataWriter()
	: _terminated(false)
...
bool VvtDataWriter::isSessionProceeded(const char* sid)
{
...
bool VvtDataWriter::init(VvTSVariant* params, IDataWriterSink* sink)
{
	...
	_proc_chk.reset(new StdThread(boost::bind(&VvtDataWriter::check_loop, this)));

	pipe_writer_log(sink, LL_INFO, "VvtDataWriter initialized, root dir: {}, save_csv_tick: {}, async_mode: {}, log_group_size: {}, disable_history: {}, "
...
void DataManager::preloadRtCaches(const char* exchg)
{
	...
	uint32_t cnt = 0;
	uint32_t codecnt = 0;
	VvTSArray* ayCts = _bd_mgr->getContracts(exchg);
	if (ayCts != NULL && ayCts->size() > 0)
	{
		for (auto it = ayCts->begin(); it != ayCts->end(); it++)
		{
			VvTSContractInfo* ct = (VvTSContractInfo*)(*it);
			if (ct == NULL)
				continue;
			VvTSCommodityInfo* commInfo = _bd_mgr->getCommodity(ct);
			if(commInfo == NULL)
				continue;
	...
	if (ayCts != NULL)
		ayCts->release();
	pipe_writer_log(_sink, LL_INFO, "预加载%个品种的实时数据缓存文件{}个,耗时{}微秒", codecnt, cnt, VvTSLogger::fmtInt64(ticker.micro_seconds()));
	_preloaded = true;
}
*/

void VvtDataWriter::loadCache()
{
...
void* VvtDataWriter::resizeRTBlock(BoostMFPtr& mfPtr, uint32_t nCount)
{
	if (mfPtr == NULL)
		return NULL;
...
bool VvtDataWriter::writeTick(VvTSTickData* curTick, uint32_t procFlag)
{
	if (curTick == NULL)
		return false;
...
void VvtDataWriter::procTick(VvTSTickData* curTick, uint32_t procFlag)
{
	do
	{
...
bool VvtDataWriter::writeOrderQueue(VvTSOrdQueData* curOrdQue)
{
...
void VvtDataWriter::procQueue(VvTSOrdQueData* curOrdQue)
{
	do
	{
		VvTSContractInfo* ct = curOrdQue->getContractInfo();
		VvTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		if (blk->_size >= blk->_capacity)
		{
			...
			pBlockPair->_block = (RTOrdQueBlock*)resizeRTBlock<RTDayBlockHeader, VvTSOrdQueStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_queues[blk->_size], &curOrdQue->getOrdQueStruct(), sizeof(VvTSOrdQueStruct));
		blk->_size += 1;
...
bool VvtDataWriter::writeOrderDetail(VvTSOrdDtlData* curOrdDtl)
{
...
void VvtDataWriter::procOrder(VvTSOrdDtlData* curOrdDtl)
{
	do
	{
		VvTSContractInfo* ct = curOrdDtl->getContractInfo();
		VvTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		{
			pBlockPair->_file->sync();
			pBlockPair->_block = (RTOrdDtlBlock*)resizeRTBlock<RTDayBlockHeader, VvTSOrdDtlStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_details[blk->_size], &curOrdDtl->getOrdDtlStruct(), sizeof(VvTSOrdDtlStruct));
		blk->_size += 1;
...
bool VvtDataWriter::writeTransaction(VvTSTransData* curTrans)
{
...
void VvtDataWriter::procTrans(VvTSTransData* curTrans)
{
	do
	{
		VvTSContractInfo* ct = curTrans->getContractInfo();
		VvTSCommodityInfo* commInfo = ct->getCommInfo();
		...
		{
			pBlockPair->_file->sync();
			pBlockPair->_block = (RTTransBlock*)resizeRTBlock<RTDayBlockHeader, VvTSTransStruct>(pBlockPair->_file, blk->_capacity * 2);
			blk = pBlockPair->_block;
		}

		memcpy(&blk->_trans[blk->_size], &curTrans->getTransStruct(), sizeof(VvTSTransStruct));
		blk->_size += 1;
...
void VvtDataWriter::pushTask(const TaskInfo& task)
{
	if (!_async_proc)
		return;
		...
		_task_thrd.reset(new StdThread([this]() {
			while (!_terminated)
			{
				...
				while (!tempQueue.empty())
				{
					...
					{
					case 0: procTick((VvTSTickData*)curTask._obj, curTask._flag); break;
					case 1: procQueue((VvTSOrdQueData*)curTask._obj); break;
					case 2: procOrder((VvTSOrdDtlData*)curTask._obj); break;
					case 3: procTrans((VvTSTransData*)curTask._obj); break;
					default:
						break;
					}
					tempQueue.pop();
...
void VvtDataWriter::pipeToTicks(VvTSContractInfo* ct, VvTSTickData* curTick)
{
	...
	{
		pBlockPair->_file->sync();
		pBlockPair->_block = (RTTickBlock*)resizeRTBlock<RTDayBlockHeader, VvTSTickStruct>(pBlockPair->_file, blk->_capacity * 2);
		blk = pBlockPair->_block;
	...
	memcpy(&blk->_ticks[blk->_size], &curTick->getTickStruct(), sizeof(VvTSTickStruct));
	blk->_size += 1;
...
VvtDataWriter::OrdQueBlockPair* VvtDataWriter::getOrdQueBlock(VvTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSOrdQueStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_queues, 0, sizeof(VvTSOrdQueStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			do
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSOrdQueStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(VvTSOrdQueStruct));
...
VvtDataWriter::OrdDtlBlockPair* VvtDataWriter::getOrdDtlBlock(VvTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSOrdDtlStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_details, 0, sizeof(VvTSOrdDtlStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			for (;;)
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSOrdDtlStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(VvTSOrdDtlStruct));
					//文件大小不匹配,一般是因为capacity改了,但是实际没扩容
					//这是做一次扩容即可
					pBlock->_block->_capacity = oldCnt;
					pBlock->_block->_size = oldCnt;
...
VvtDataWriter::TransBlockPair* VvtDataWriter::getTransBlock(VvTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSTransStruct) * HFT_SIZE_STEP;
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_trans, 0, sizeof(VvTSTransStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			for (;;)
			{
				uint64_t uSize = sizeof(RTDayBlockHeader) + sizeof(VvTSTransStruct) * pBlock->_block->_capacity;
				uint64_t oldSize = pBlock->_file->size();
				if (oldSize != uSize)
				{
					uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) / sizeof(VvTSTransStruct));
					//文件大小不匹配,一般是因为capacity改了,但是实际没扩容
					//这是做一次扩容即可
					pBlock->_block->_capacity = oldCnt;
					pBlock->_block->_size = oldCnt;
...
VvtDataWriter::TickBlockPair* VvtDataWriter::getTickBlock(VvTSContractInfo* ct, uint32_t curDate, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if(pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path.c_str()))
		{
			...
			uint64_t uSize = sizeof(RTTickBlock) + sizeof(VvTSTickStruct) * HFT_SIZE_STEP;
			BoostFile bf;
			bf.create_new_file(path.c_str());
			bf.truncate_file((uint32_t)uSize);
			bf.close_file();
		...
		if (!isNew &&  pBlock->_block->_date != curDate)
		{
			...
			memset(&pBlock->_block->_ticks, 0, sizeof(VvTSTickStruct)*pBlock->_block->_capacity);
		}
		...
		else
		{
			...
			do
			{
				uint64_t uSize = sizeof(RTTickBlock) + sizeof(VvTSTickStruct) * pBlock->_block->_capacity;
				uint64_t realSz = pBlock->_file->size();
				if (realSz != uSize)
				{
					uint32_t realCap = (uint32_t)((realSz - sizeof(RTTickBlock)) / sizeof(VvTSTickStruct));
...
void VvtDataWriter::pipeToKlines(VvTSContractInfo* ct, VvTSTickData* curTick)
{
	...
	VvTSSessionInfo* sInfo = ct->getCommInfo()->getSessionInfo();
	...
	if (!_disable_min1)
	{
		...
		if (pBlockPair && pBlockPair->_block)
		{
			...
			{
				pBlockPair->_file->sync();
				pBlockPair->_block = (RTKlineBlock*)resizeRTBlock<RTKlineBlock, VvTSBarStruct>(pBlockPair->_file, blk->_capacity * 2);
				blk = pBlockPair->_block;
			}
...
template<typename T>
void VvtDataWriter::releaseBlock(T* block)
{
...
VvtDataWriter::KBlockPair* VvtDataWriter::getKlineBlock(VvTSContractInfo* ct, VvTSKlinePeriod period, bool bAutoCreate /* = true */)
{
	if (ct == NULL)
		return NULL;
	...
	if (pBlock->_block == NULL)
	{
		...
		if (!BoostFile::exists(path))
		{
			...
			uint64_t uSize = sizeof(RTKlineBlock) + sizeof(VvTSBarStruct) * totalMins;	//预分配按照K线条数分配
			BoostFile bf;
		...
		//	memset(&pBlock->_block->_bars, 0, sizeof(VvTSBarStruct)*pBlock->_block->_capacity);
		//}
...
VvTSTickData* VvtDataWriter::getCurTick(const char* code, const char* exchg/* = ""*/)
{
	if (strlen(code) == 0)
		return NULL;
...
bool VvtDataWriter::updateCache(VvTSContractInfo* ct, VvTSTickData* curTick, uint32_t procFlag)
{
...
void VvtDataWriter::transHisData(const char* sid)
{
	...
	if (strcmp(sid, CMD_CLEAR_CACHE) != 0)
	{
		...
		for (auto it = pCommSet->begin(); it != pCommSet->end(); it++)
		{
			...
			VvTSCommodityInfo* pCommInfo = _bd_mgr->getCommodity(exchg, pid);
			if (pCommInfo == NULL)
				continue;
			...
			for (auto code : codes)
			{
				VvTSContractInfo* ct = _bd_mgr->getContract(code.c_str(), exchg);
				if(ct)
	...
	if (_proc_thrd == NULL)
	{
		_proc_thrd.reset(new StdThread(boost::bind(&VvtDataWriter::proc_loop, this)));
	}
	else
	{
		_proc_cond.notify_all();
...
uint32_t VvtDataWriter::dump_bars_via_dumper(VvTSContractInfo* ct)
{
	...
	if (it != _tick_cache_idx.end())
	{
		...
		const VvTSTickStruct& ts = tci._tick;
...
bool VvtDataWriter::proc_block_data(const char* tag, std::string& content, bool isBar, bool bKeepHead /* = true */)
{
...
bool VvtDataWriter::dump_day_data(VvTSContractInfo* ct, VvTSBarStruct* newBar)
{
	std::stringstream ss;
	...
	if (f.create_or_open_file(filename.c_str()))
	{
		...
		if (bNew)
		{
			...
			f.write_file(newBar, sizeof(VvTSBarStruct));
		}
		else
		{
			//日线必须要检查一下
			...
			if (barcnt != 0)
			{
				VvTSBarStruct& oldBS = ((VvTSBarStruct*)content.data())[barcnt - 1];

				if (oldBS.date == newBar->date && memcmp(&oldBS, newBar, sizeof(VvTSBarStruct)) != 0)
				{
				...
				{
					content.append((char*)newBar, sizeof(VvTSBarStruct));
					barcnt++;
				}
			}
...
uint32_t VvtDataWriter::dump_bars_to_file(VvTSContractInfo* ct)
{
	if (ct == NULL)
		return 0;
	...
	if (!_disable_min1)
	{
		...
		if (kBlkPair != NULL && kBlkPair->_block->_size > 0)
		{
			...
			if (f.create_or_open_file(filename.c_str()))
			{
				...
				//最后将缓存清空
				//memset(kBlkPair->_block->_bars, 0, sizeof(VvTSBarStruct)*kBlkPair->_block->_size);
				kBlkPair->_block->_size = 0;
			}
			else
			{
	...
	while (!_terminated)
	{
		...
		if (fullcode.compare(CMD_CLEAR_CACHE) == 0)
		{
			...
			for (auto it = _tick_cache_idx.begin(); it != _tick_cache_idx.end(); it++)
			{
				const char* key = it->first.c_str();
				const StringVector& ay = StrUtil::split(key, ".");
				VvTSContractInfo* ct = _bd_mgr->getContract(ay[1].c_str(), ay[0].c_str());
				if (ct != NULL)
				{
					setCodes.insert(key);
					...
					if (tBlkPair->_block->_size > 0)
					{
						...
						{//////////////////////////////////////////////////////////////////////////
							...
							if (f.create_new_file(filename.c_str()))
							{
								...
								//最后将缓存清空
								//memset(tBlkPair->_block->_ticks, 0, sizeof(VvTSTickStruct)*tBlkPair->_block->_size);
