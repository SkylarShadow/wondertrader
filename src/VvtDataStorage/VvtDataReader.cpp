...
extern "C"
{
	...
	{
		IDataReader* ret = new VvtDataReader();
		return ret;
	}
...
void VvtDataReader::init(VvTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader /* = NULL */)
{
	...
	pipe_reader_log(sink, LL_INFO, "VvtDataReader initialized, rt dir is {}, hist dir is {}, adjust_flag is {}", _rt_dir, _his_dir, _adjust_flag);
...
bool VvtDataReader::loadStkAdjFactorsFromLoader()
{
	if (NULL == _loader)
		return false;
...
bool VvtDataReader::loadStkAdjFactorsFromFile(const char* adjfile)
{
...
VvTSTickSlice* VvtDataReader::readTickSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	...
	if (isToday)
	{
		...
		VvTSTickSlice* slice = VvTSTickSlice::create(stdCode, tBlock->_ticks + sIdx, cnt);
		return slice;
	}
	else
	{
...
VvTSOrdQueSlice* VvtDataReader::readOrdQueSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSOrdQueSlice* slice = VvTSOrdQueSlice::create(stdCode, rtBlock->_queues + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		VvTSOrdQueStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const VvTSOrdQueStruct& a, const VvTSOrdQueStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSOrdQueSlice* slice = VvTSOrdQueSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}

VvTSOrdDtlSlice* VvtDataReader::readOrdDtlSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSOrdDtlSlice* slice = VvTSOrdDtlSlice::create(stdCode, rtBlock->_details + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		VvTSOrdDtlStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const VvTSOrdDtlStruct& a, const VvTSOrdDtlStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSOrdDtlSlice* slice = VvTSOrdDtlSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}

VvTSTransSlice* VvtDataReader::readTransSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (isToday)
	{
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSTransSlice* slice = VvTSTransSlice::create(stdCode, rtBlock->_trans + sIdx, cnt);
		return slice;
	}
	else
	{
		...
		VvTSTransStruct* pTick = std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick, [](const VvTSTransStruct& a, const VvTSTransStruct& b) {
			if (a.action_date != b.action_date)
				return a.action_date < b.action_date;
			else
				return a.action_time < b.action_time;
		...
		uint32_t cnt = min(eIdx + 1, count);
		uint32_t sIdx = eIdx + 1 - cnt;
		VvTSTransSlice* slice = VvTSTransSlice::create(stdCode, tBlock->_items + sIdx, cnt);
		return slice;
	}
}


bool VvtDataReader::cacheFinalBarsFromLoader(void* codeInfo, const std::string& key, const char* stdCode, VvTSKlinePeriod period)
{
	if (NULL == _loader)
		return false;
	...
	bool ret = _loader->loadFinalHisBars(&barList, stdCode, period, [](void* obj, VvTSBarStruct* firstBar, uint32_t count) {
		BarsList* bars = (BarsList*)obj;
		bars->_factor = 1.0;
		bars->_bars.resize(count);
		memcpy(bars->_bars.data(), firstBar, sizeof(VvTSBarStruct)*count);
	});
...
bool VvtDataReader::cacheIntegratedBars(void* codeInfo, const std::string& key, const char* stdCode, VvTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	...
	for (auto it = secs.rbegin(); it != secs.rend(); it++)
	{
		...
		if (NULL != _loader)
		{
			...
			bLoaded = _loader->loadRawHisBars(&buffer, wCode.c_str(), period, [](void* obj, VvTSBarStruct* bars, uint32_t count) {
...
bool VvtDataReader::cacheAdjustedStkBars(void* codeInfo, const std::string& key, const char* stdCode, VvTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	...
	std::vector<VvTSBarStruct>* ayAdjusted = NULL;
	...
	do
	{
		...
		ayAdjusted = new std::vector<VvTSBarStruct>();
	...
	do
	{
		...
		if (NULL != _loader)
		{
			bLoaded = _loader->loadRawHisBars(&buffer, rawCode.c_str(), period, [](void* obj, VvTSBarStruct* bars, uint32_t count) {
				std::string* buff = (std::string*)obj;
				buff->resize(sizeof(VvTSBarStruct)*count);
				memcpy((void*)buff->c_str(), bars, sizeof(VvTSBarStruct)*count);
			});
		...
		if (pBar != NULL)
		{
			...
			std::vector<VvTSBarStruct>* ayRaw = new std::vector<VvTSBarStruct>();
			ayRaw->resize(curCnt);
			memcpy(ayRaw->data(), &firstBar[sIdx], sizeof(VvTSBarStruct)*curCnt);
			realCnt += curCnt;
...
bool VvtDataReader::cacheHisBarsFromFile(void* codeInfo, const std::string& key, const char* stdCode, VvTSKlinePeriod period)
{
	CodeHelper::CodeInfo* cInfo = (CodeHelper::CodeInfo*)codeInfo;
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo->_exchg, cInfo->_product);
	const char* stdPID = commInfo->getFullPid();
	...
	if (NULL != _loader)
	{
		bLoaded = _loader->loadRawHisBars(&buffer, stdCode, period, [](void* obj, VvTSBarStruct* bars, uint32_t count) {
			std::string* buff = (std::string*)obj;
			buff->resize(sizeof(VvTSBarStruct)*count);
			memcpy((void*)buff->c_str(), bars, sizeof(VvTSBarStruct)*count);
		});
...
VvTSKlineSlice* VvtDataReader::readKlineSlice(const char* stdCode, VvTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	...
	VvTSKlineSlice* slice = VvTSKlineSlice::create(stdCode, period, 1, NULL, 0);;
	VvTSBarStruct* head = NULL;
	uint32_t hisCnt = 0;
	uint32_t rtCnt = 0;
	uint32_t totalCnt = 0;
	std::string pname;
	...
	*/
	...
	if (bHasToday)
	{
		...
		if (kPair != NULL && kPair->_block && kPair->_block->_size>0)
		{
			//读取当日的数据
			VvTSBarStruct* pBar = NULL;
			pBar = std::lower_bound(kPair->_block->_bars, kPair->_block->_bars + (kPair->_block->_size - 1), bar, [period](const VvTSBarStruct& a, const VvTSBarStruct& b) {
				if (period == KP_DAY)
					return a.date < b.date;
				else
					return a.time < b.time;
			...
			if(cInfo._exright == 2/* && commInfo->isStock()*/)
			{
				...
				if (barsList._rt_cursor == UINT_MAX || idx > barsList._rt_cursor)
				{
					...
					memcpy(&barsList._bars[oldSize], &kPair->_block->_bars[sIdx], sizeof(VvTSBarStruct)* curCnt);
					for(uint32_t thisIdx = oldSize; thisIdx < newSize; thisIdx++)
					{
						VvTSBarStruct* pBar = &barsList._bars[thisIdx];
						pBar->open *= factor;
						pBar->high *= factor;
						pBar->low *= factor;
						pBar->close *= factor;
...
VvtDataReader::TickBlockPair* VvtDataReader::getRTTickBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
...
VvtDataReader::OrdDtlBlockPair* VvtDataReader::getRTOrdDtlBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
VvtDataReader::OrdQueBlockPair* VvtDataReader::getRTOrdQueBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
VvtDataReader::TransBlockPair* VvtDataReader::getRTTransBlock(const char* exchg, const char* code)
{
	thread_local static char key[64] = { 0 };
	fmtutil::format_to(key, "{}#{}", exchg, code);
...
VvtDataReader::RTKlineBlockPair* VvtDataReader::getRTKilneBlock(const char* exchg, const char* code, VvTSKlinePeriod period)
{
...
void VvtDataReader::onMinuteEnd(uint32_t uDate, uint32_t uTime, uint32_t endTDate /* = 0 */)
{
	//这里应该触发检查
	...
	for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++)
	{
		...
		if (barsList._period != KP_DAY)
		{
			if (!barsList._raw_code.empty())
			{
				...
				for (;;)
				{
					...
					VvTSBarStruct& nextBar = kBlk->_block->_bars[preCnt];
		...
		//		for (;;)
		//		{
		//			VvTSBarStruct& nextBar = barsList._bars[barsList._his_cursor + 1];
...
double VvtDataReader::getAdjFactorByDate(const char* stdCode, uint32_t date /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	if (!commInfo->isStock())
...
const VvtDataReader::AdjFactorList& VvtDataReader::getAdjFactors(const char* code, const char* exchg, const char* pid)
{
	...
	if (it == _adj_factors.end())
	{
		...
		if (_loader)
		{
			...
			_loader->loadAdjFactors(this, key, [](void* obj, const char* stdCode, uint32_t* dates, double* factors, uint32_t count) {
				VvtDataReader* self = (VvtDataReader*)obj;
