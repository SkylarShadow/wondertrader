...
extern "C"
{
	...
	{
		IBtDtReader* ret = new VvtBtDtReader();
		return ret;
	}
...
void VvtBtDtReader::init(VvTSVariant* cfg, IBtDtReaderSink* sink)
{
	_sink = sink;
	...
	pipe_btreader_log(_sink, LL_INFO, "VvtBtDtReader initialized, root data dir is {}", _base_dir);
}

bool VvtBtDtReader::read_raw_bars(const char* exchg, const char* code, VvTSKlinePeriod period, std::string& buffer)
{
	std::stringstream ss;
...
bool VvtBtDtReader::read_raw_ticks(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	std::stringstream ss;
...
bool VvtBtDtReader::read_raw_order_details(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	std::stringstream ss;
...
bool VvtBtDtReader::read_raw_order_queues(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	std::stringstream ss;
...
bool VvtBtDtReader::read_raw_transactions(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	std::stringstream ss;
