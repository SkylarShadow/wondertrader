...
bool json_to_variant(const rj::Value& root, ZTSVariant* params)
{
	if (root.IsObject() && params->type() != ZTSVariant::VT_Object)
		return false;

	if (root.IsArray() && params->type() != ZTSVariant::VT_Array)
		return false;
	...
	if (root.IsObject())
	{
		for (auto& m : root.GetObject())
		{
			...
			case rj::kObjectType:
			{
				ZTSVariant* subObj = ZTSVariant::createObject();
			...
			case rj::kArrayType:
			{
				ZTSVariant* subAy = ZTSVariant::createArray();
...
ZTSVariant* ZTSCfgLoader::load_from_json(const char* content)
{
	rj::Document root;
	root.Parse(content);
	...
	ZTSVariant* ret = ZTSVariant::createObject();
...
#include "../ZTSUtils/yamlcpp/yaml.h"
bool yaml_to_variant(const YAML::Node& root, ZTSVariant* params)
{
	if (root.IsNull() && params->type() != ZTSVariant::VT_Object)
		return false;

	if (root.IsSequence() && params->type() != ZTSVariant::VT_Array)
		return false;
...
ZTSVariant* ZTSCfgLoader::load_from_yaml(const char* content)
{
...
ZTSVariant* ZTSCfgLoader::load_from_content(const std::string& content, bool isYaml /* = false */)
{
	//加一个自动检测编码的逻辑
...
ZTSVariant* ZTSCfgLoader::load_from_file(const char* filename)
{
