...
#include "../VvtCore/SelStraContext.h"
...
bool VvtRtRunner::init(const char* logCfg /* = "logcfg.prop" */, bool isFile /* = true */, const char* genDir)
{
#ifdef _MSC_VER
	...
	if(isFile)
	{
		...
		VvTSLogger::init(path.c_str(), true, this);
	}
	else
	{
		VvTSLogger::init(logCfg, false, this);
	}
	

	VvtHelper::setInstDir(getBinDir());
	VvtHelper::setGenerateDir(StrUtil::standardisePath(genDir).c_str());
	return true;
}

void VvtRtRunner::registerEvtCallback(FuncEventCallback cbEvt)
{
	_cb_evt = cbEvt;
...
void VvtRtRunner::registerParserPorter(FuncParserEvtCallback cbEvt, FuncParserSubCallback cbSub)
{
...
void VvtRtRunner::registerExecuterPorter(FuncExecInitCallback cbInit, FuncExecCmdCallback cbExec)
{
	_cb_exec_init = cbInit;
	_cb_exec_cmd = cbExec;

	VvTSLogger::info("Callbacks of Extented Executer registration done");
}

void VvtRtRunner::registerCtaCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, 
		FuncSessionEvtCallback cbSessEvt, FuncStraCondTriggerCallback cbCondTrigger /* = NULL */)
{
	...
	VvTSLogger::info("Callbacks of CTA engine registration done");
}

void VvtRtRunner::registerSelCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar, FuncSessionEvtCallback cbSessEvt)
{
	_cb_sel_init = cbInit;
	_cb_sel_tick = cbTick;
	_cb_sel_calc = cbCalc;
	...
	VvTSLogger::info("Callbacks of SEL engine registration done");
}

void VvtRtRunner::registerHftCallbacks(FuncStraInitCallback cbInit, FuncStraTickCallback cbTick, FuncStraBarCallback cbBar, 
	...
	FuncStraOrdDtlCallback cbOrdDtl, FuncStraOrdQueCallback cbOrdQue, FuncStraTransCallback cbTrans, FuncSessionEvtCallback cbSessEvt, FuncHftPosCallback cbPosition)
{
	...
	VvTSLogger::info("Callbacks of HFT engine registration done");
}

bool VvtRtRunner::loadFinalHisBars(void* obj, const char* stdCode, VvTSKlinePeriod period, FuncReadBars cb)
{
	...
	default:
	{
		VvTSLogger::error("Unsupported period of extended data loader");
		return false;
	}
	}
}

bool VvtRtRunner::loadRawHisBars(void* obj, const char* stdCode, VvTSKlinePeriod period, FuncReadBars cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool VvtRtRunner::loadAllAdjFactors(void* obj, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
...
bool VvtRtRunner::loadAdjFactors(void* obj, const char* stdCode, FuncReadFactors cb)
{
	StdUniqueLock lock(_feed_mtx);
	if (_ext_adj_fct_loader == NULL)
		return false;
...
void VvtRtRunner::feedAdjFactors(const char* stdCode, uint32_t* dates, double* factors, uint32_t count)
{
	_feeder_fcts(_feed_obj, stdCode, dates, factors, count);
}


void VvtRtRunner::feedRawBars(VvTSBarStruct* bars, uint32_t count)
{
	if (_ext_fnl_bar_loader == NULL)
	{
		VvTSLogger::error("Cannot feed bars because of no extented bar loader registered.");
		return;
	}
...
bool VvtRtRunner::createExtParser(const char* id)
{
	...
	VvTSLogger::info("Extended parser created");
	return true;
}

bool VvtRtRunner::createExtExecuter(const char* id)
{
	ExpExecuter* executer = new ExpExecuter(id);
	executer->init();
	_cta_engine.addExecuter(ExecCmdPtr(executer));
	VvTSLogger::info("Extended Executer created");
	return true;
}

uint32_t VvtRtRunner::createCtaContext(const char* name, int32_t slippage /* = 0 */)
{
...
uint32_t VvtRtRunner::createHftContext(const char* name, const char* trader, bool bAgent, int32_t slippage /* = 0 */)
{
	...
	{
		VvTSLogger::error("Trader {} not exists, Binding trader to HFT strategy failed", trader);
	}
	return ctx->id();
}

uint32_t VvtRtRunner::createSelContext(const char* name, uint32_t date, uint32_t time, const char* period, int32_t slippage, const char* trdtpl /* = "CHINA" */, const char* session/* ="TRADING" */)
{
...
const char* VvtRtRunner::get_raw_stdcode(const char* stdCode)
{
...
CtaContextPtr VvtRtRunner::getCtaContext(uint32_t id)
{
	return _cta_engine.getContext(id);
}

HftContextPtr VvtRtRunner::getHftContext(uint32_t id)
{
	return _hft_engine.getContext(id);
}

SelContextPtr VvtRtRunner::getSelContext(uint32_t id)
{
	return _sel_engine.getContext(id);
}

void VvtRtRunner::ctx_on_bar(uint32_t id, const char* stdCode, const char* period, VvTSBarStruct* newBar, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtRtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtRtRunner::ctx_on_cond_triggered(uint32_t id, const char* stdCode, double target, double price, const char* usertag, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtRtRunner::ctx_on_init(uint32_t id, EngineType eType/* = ET_CTA*/)
{
	switch (eType)
	{
...
void VvtRtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate, bool isBegin /* = true */, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtRtRunner::ctx_on_tick(uint32_t id, const char* stdCode, VvTSTickData* newTick, EngineType eType /* = ET_CTA */)
{
	switch (eType)
	{
...
void VvtRtRunner::hft_on_channel_lost(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, CHNL_EVENT_LOST);
}

void VvtRtRunner::hft_on_channel_ready(uint32_t cHandle, const char* trader)
{
	if (_cb_hft_chnl)
		_cb_hft_chnl(cHandle, trader, CHNL_EVENT_READY);
}

void VvtRtRunner::hft_on_entrust(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool bSuccess, const char* message, const char* userTag)
{
	if (_cb_hft_entrust)
		_cb_hft_entrust(cHandle, localid, stdCode, bSuccess, message, userTag);
}

void VvtRtRunner::hft_on_order(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)
{
	if (_cb_hft_ord)
		_cb_hft_ord(cHandle, localid, stdCode, isBuy, totalQty, leftQty, price, isCanceled, userTag);
}

void VvtRtRunner::hft_on_trade(uint32_t cHandle, VvTUInt32 localid, const char* stdCode, bool isBuy, double vol, double price, const char* userTag)
{
	if (_cb_hft_trd)
		_cb_hft_trd(cHandle, localid, stdCode, isBuy, vol, price, userTag);
}

void VvtRtRunner::hft_on_position(uint32_t cHandle, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)
{
	if (_cb_hft_position)
		_cb_hft_position(cHandle, stdCode, isLong, prevol, preavail, newvol, newavail);
}

bool VvtRtRunner::config(const char* cfgFile, bool isFile /* = true */)
{
	...
	if (cfgBF->get("holiday"))
	{
		...
		VvTSLogger::log_raw(LL_INFO, "Holidays loaded");
	}
...
bool VvtRtRunner::initCtaStrategies()
{
...
bool VvtRtRunner::initSelStrategies()
{
	VvTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != VvTSVariant::VT_Object)
		return false;
...
bool VvtRtRunner::initHftStrategies()
{
	VvTSVariant* cfg = _config->get("strategies");
	if (cfg == NULL || cfg->type() != VvTSVariant::VT_Object)
		return false;
...
bool VvtRtRunner::initEngine()
{
	...
	if (_is_hft)
	{
		VvTSLogger::info("Trading environment initialized, engine name: HFT");
		_hft_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_hft_engine;
	}
	else if (_is_sel)
	{
		VvTSLogger::info("Trading environment initialized, engine name: SEL");
		_sel_engine.init(cfg, &_bd_mgr, &_data_mgr, &_hot_mgr, &_notifier);
		_engine = &_sel_engine;
	}
	else
	{
		VvTSLogger::info("Trading environment initialized, engine name: CTA");
...
bool VvtRtRunner::initDataMgr()
{
	...
	VvTSLogger::log_raw(LL_INFO, "Data manager initialized");
	return true;
}

bool VvtRtRunner::initParsers(VvTSVariant* cfgParsers)
{
	if (cfgParsers == NULL || cfgParsers->type() != VvTSVariant::VT_Array)
		return false;

	uint32_t count = 0;
	for (uint32_t idx = 0; idx < cfgParsers->size(); idx++)
	{
		VvTSVariant* cfgItem = cfgParsers->get(idx);
		if (!cfgItem->getBoolean("active"))
			continue;
...
bool VvtRtRunner::initExecuters(VvTSVariant* cfgExecuter)
{
	...
	for (uint32_t idx = 0; idx < cfgExecuter->size(); idx++)
	{
		...
		{
			VvtArbiExecuter* executer = new VvtArbiExecuter(&_exe_factory, id, &_data_mgr);
			if (!executer->init(cfgItem))
				return false;
...
bool VvtRtRunner::initEvtNotifier()
{
...
bool VvtRtRunner::initTraders(VvTSVariant* cfgTraders)
{
	if (cfgTraders == NULL || cfgTraders->type() != VvTSVariant::VT_Array)
		return false;
	
	uint32_t count = 0;
	for (uint32_t idx = 0; idx < cfgTraders->size(); idx++)
	{
		VvTSVariant* cfgItem = cfgTraders->get(idx);
		if (!cfgItem->getBoolean("active"))
			continue;
...
void VvtRtRunner::run(bool bAsync /* = false */)
{
	try
	{
		_parsers.run();
...
void VvtRtRunner::handleLogAppend(VvTSLogLevel ll, const char* msg)
{
	_notifier.notify_log(LOG_TAGS[ll-100], msg);
}

void VvtRtRunner::release()
{
	VvTSLogger::stop();
}

bool VvtRtRunner::initActionPolicy()
{
...
bool VvtRtRunner::addCtaFactories(const char* folder)
{
	return _cta_mgr.loadFactories(folder);
}

bool VvtRtRunner::addSelFactories(const char* folder)
{
	return _sel_mgr.loadFactories(folder);
}

bool VvtRtRunner::addExeFactories(const char* folder)
{
	return _exe_factory.loadFactories(folder);
}

bool VvtRtRunner::addHftFactories(const char* folder)
{
	return _hft_mgr.loadFactories(folder);
}

void VvtRtRunner::hft_on_order_queue(uint32_t id, const char* stdCode, VvTSOrdQueData* newOrdQue)
{
	if (_cb_hft_ordque)
		_cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void VvtRtRunner::hft_on_order_detail(uint32_t id, const char* stdCode, VvTSOrdDtlData* newOrdDtl)
{
	if (_cb_hft_orddtl)
		_cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void VvtRtRunner::hft_on_transaction(uint32_t id, const char* stdCode, VvTSTransData* newTrans)
{
	if (_cb_hft_trans)
...
void VvtRtRunner::parser_init(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_INIT, id);
}

void VvtRtRunner::parser_connect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_CONNECT, id);
}

void VvtRtRunner::parser_disconnect(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_DISCONNECT, id);
}

void VvtRtRunner::parser_release(const char* id)
{
	if (_cb_parser_evt)
		_cb_parser_evt(EVENT_PARSER_RELEASE, id);
}

void VvtRtRunner::parser_subscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, true);
}

void VvtRtRunner::parser_unsubscribe(const char* id, const char* code)
{
	if (_cb_parser_sub)
		_cb_parser_sub(id, code, false);
}

void VvtRtRunner::on_ext_parser_quote(const char* id, VvTSTickStruct* curTick, uint32_t uProcFlag)
{
...
void VvtRtRunner::executer_init(const char* id)
{
	if (_cb_exec_init)
		_cb_exec_init(id);
}

void VvtRtRunner::executer_set_position(const char* id, const char* stdCode, double target)
{
	if (_cb_exec_cmd)
