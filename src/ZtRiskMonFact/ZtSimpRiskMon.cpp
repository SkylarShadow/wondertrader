...
const char* ZtSimpleRiskMon::getName()
{
	return "ZtSimpleRiskMon";
}

const char* ZtSimpleRiskMon::getFactName()
{
	return FACT_NAME;
}

void ZtSimpleRiskMon::init(ZtPortContext* ctx, ZTSVariant* cfg)
{
	ZtRiskMonitor::init(ctx, cfg);
...
void ZtSimpleRiskMon::run()
{
	...
	_thrd.reset(new std::thread([this](){
		while (!_stopped)
		{
			...
			{
				ZTSPortFundInfo* fundInfo = _ctx->getFundInfo();
				const ZTSFundStruct& fs = fundInfo->fundInfo();
				/*
