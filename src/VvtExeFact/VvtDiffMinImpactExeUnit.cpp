/*!
 * \file VvtDiffMinImpactExeUnit.cpp
 *
 * \author Wesley
 * \date 2020/03/30
 *
...
VvtDiffMinImpactExeUnit::VvtDiffMinImpactExeUnit()
	: _last_tick(NULL)
	, _comm_info(NULL)
	, _price_mode(0)
	, _price_offset(0)
...
const char* VvtDiffMinImpactExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* VvtDiffMinImpactExeUnit::getName()
{
	return "VvtDiffMinImpactExeUnit";
}

void VvtDiffMinImpactExeUnit::init(ExecuteContext* ctx, const char* stdCode, VvTSVariant* cfg)
{
...
void VvtDiffMinImpactExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
	{
...
void VvtDiffMinImpactExeUnit::on_channel_ready()
{
...
void VvtDiffMinImpactExeUnit::on_channel_lost()
{
	
}

void VvtDiffMinImpactExeUnit::on_tick(VvTSTickData* newTick)
{
...
void VvtDiffMinImpactExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//如果是本地订单，则更新差量
	if (!_orders_mon.has_order(localid))
		return;
...
void VvtDiffMinImpactExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void VvtDiffMinImpactExeUnit::do_calc()
{
...
void VvtDiffMinImpactExeUnit::set_position(const char* stdCode, double newDiff)
{
...
void VvtDiffMinImpactExeUnit::clear_all_position(const char* stdCode)
{
	_ctx->writeLog("Diff execute unit do not support clear command");
	return;
}
