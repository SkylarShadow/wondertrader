...
VvtStockVWapExeUnit::VvtStockVWapExeUnit()
	:_last_tick(NULL)
...
const char * VvtStockVWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char * VvtStockVWapExeUnit::getName()
{
	return "VvtStockVWapExeUnit";
}

void VvtStockVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, VvTSVariant * cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元VvtStockVWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs).c_str());
...
void VvtStockVWapExeUnit::on_order(uint32_t localid, const char * stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void VvtStockVWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void VvtStockVWapExeUnit::on_tick(VvTSTickData * newTick)
{
...
void VvtStockVWapExeUnit::on_trade(uint32_t localid, const char * stdCode, bool isBuy, double vol, double price)
{//在ontick中触发
}
/*
下单结果回报
*/
void VvtStockVWapExeUnit::on_entrust(uint32_t localid, const char * stdCode, bool bSuccess, const char * message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void VvtStockVWapExeUnit::do_calc()
{
...
void VvtStockVWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
	_last_tick->retain();
...
void VvtStockVWapExeUnit::set_position(const char * stdCode, double newVol)
{
...
void VvtStockVWapExeUnit::on_channel_lost()
{
}
void VvtStockVWapExeUnit::clear_all_position(const char* stdCode) {
	if (_code.compare(stdCode) != 0)
		return;
	_is_clear = true;
	_target_pos = 0;
