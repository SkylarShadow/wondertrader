...
const char * VvtVWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char * VvtVWapExeUnit::getName()
{
	return "VvtVWapExeUnit";
}

void VvtVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, VvTSVariant * cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元VvtVWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs).c_str());
...
void VvtVWapExeUnit::on_order(uint32_t localid, const char * stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void VvtVWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void VvtVWapExeUnit::on_tick(VvTSTickData * newTick)
{
...
void VvtVWapExeUnit::on_trade(uint32_t localid, const char * stdCode, bool isBuy, double vol, double price)
{//在ontick中触发
}
/*
下单结果回报
*/
void VvtVWapExeUnit::on_entrust(uint32_t localid, const char * stdCode, bool bSuccess, const char * message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void VvtVWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
	_last_tick->retain();
...
void VvtVWapExeUnit::set_position(const char * stdCode, double newVol)
{
...
void VvtVWapExeUnit::on_channel_lost()
{
}
