...
VvtMinImpactExeUnit::VvtMinImpactExeUnit()
	: _last_tick(NULL)
	, _comm_info(NULL)
	, _price_mode(0)
	, _price_offset(0)
...
const char* VvtMinImpactExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* VvtMinImpactExeUnit::getName()
{
	return "VvtMinImpactExeUnit";
}

void VvtMinImpactExeUnit::init(ExecuteContext* ctx, const char* stdCode, VvTSVariant* cfg)
{
 ...
 *	isCanceled	是否已撤销
 */
void VvtMinImpactExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
	{
 ...
 *	交易通道就绪回调
 */
void VvtMinImpactExeUnit::on_channel_ready()
{
...
/*
	 *	交易通道丢失回调
	 */
void VvtMinImpactExeUnit::on_channel_lost()
{
 ...
 *	newTick	最新的tick数据
 */
void VvtMinImpactExeUnit::on_tick(VvTSTickData* newTick)
{
 ...
 *	price	成交价格
 */
void VvtMinImpactExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//不用触发,这里在ontick里触发吧
}
 ...
 *	下单结果回报
 */
void VvtMinImpactExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void VvtMinImpactExeUnit::do_calc()
{
 ...
 *	newVol	新的目标仓位
 */
void VvtMinImpactExeUnit::set_position(const char* stdCode, double newVol)
{
...
void VvtMinImpactExeUnit::clear_all_position(const char* stdCode)
{
