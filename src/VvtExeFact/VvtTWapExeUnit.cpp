...
const char* VvtTWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* VvtTWapExeUnit::getName()
{
	return "VvtTWapExeUnit";
}

void VvtTWapExeUnit::init(ExecuteContext* ctx, const char* stdCode, VvTSVariant* cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元VvtTWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒,间隔时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs, _fire_span).c_str());
}

void VvtTWapExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void VvtTWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void VvtTWapExeUnit::on_channel_lost()
{

}


void VvtTWapExeUnit::on_tick(VvTSTickData* newTick)
{
...
void VvtTWapExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//不用触发,这里在ontick里触发吧
}

void VvtTWapExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void VvtTWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
...
void VvtTWapExeUnit::set_position(const char* stdCode, double newVol)
{
