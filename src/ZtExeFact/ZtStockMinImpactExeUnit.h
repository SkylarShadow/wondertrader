/*!
 * \file ZtStockMinImpactExeUnit.h
 *
 * \author Wesley
 * \date 2020/03/30
 *
...
class ZtStockMinImpactExeUnit : public ExecuteUnit
{
private:
...
public:
	ZtStockMinImpactExeUnit();
	virtual ~ZtStockMinImpactExeUnit();
...
public:
	 ...
	 *	code	管理的合约代码
	 */
	virtual void init(ExecuteContext* ctx, const char* stdCode, ZTSVariant* cfg) override;
	 ...
	 *	newTick	最新的tick数据
	 */
	virtual void on_tick(ZTSTickData* newTick) override;
...
private:
	...
	ZTSCommodityInfo* _comm_info;
	ZTSSessionInfo* _sess_info;
...
public:
	...
	inline double get_minOrderQty(std::string stdCode)
	{
		...
		ZTSCommodityInfo* comm_info = _ctx->getCommodityInfo(stdCode.c_str());
