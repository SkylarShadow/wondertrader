...
ZtStockVWapExeUnit::ZtStockVWapExeUnit()
	:_last_tick(NULL)
...
const char * ZtStockVWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char * ZtStockVWapExeUnit::getName()
{
	return "ZtStockVWapExeUnit";
}

void ZtStockVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, ZTSVariant * cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元ZtStockVWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs).c_str());
...
void ZtStockVWapExeUnit::on_order(uint32_t localid, const char * stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void ZtStockVWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void ZtStockVWapExeUnit::on_tick(ZTSTickData * newTick)
{
...
void ZtStockVWapExeUnit::on_trade(uint32_t localid, const char * stdCode, bool isBuy, double vol, double price)
{//在ontick中触发
}
/*
下单结果回报
*/
void ZtStockVWapExeUnit::on_entrust(uint32_t localid, const char * stdCode, bool bSuccess, const char * message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void ZtStockVWapExeUnit::do_calc()
{
	...
	ZTSTickData* curTick = _last_tick;
	uint64_t now = TimeUtils::getLocalTimeNow();
...
void ZtStockVWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
	_last_tick->retain();
...
void ZtStockVWapExeUnit::set_position(const char * stdCode, double newVol)
{
...
void ZtStockVWapExeUnit::on_channel_lost()
{
}
void ZtStockVWapExeUnit::clear_all_position(const char* stdCode) {
	if (_code.compare(stdCode) != 0)
		return;
	_is_clear = true;
	_target_pos = 0;
