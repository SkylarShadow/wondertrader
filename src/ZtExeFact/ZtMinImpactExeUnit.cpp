...
ZtMinImpactExeUnit::ZtMinImpactExeUnit()
	: _last_tick(NULL)
	, _comm_info(NULL)
	, _price_mode(0)
	, _price_offset(0)
...
const char* ZtMinImpactExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* ZtMinImpactExeUnit::getName()
{
	return "ZtMinImpactExeUnit";
}

void ZtMinImpactExeUnit::init(ExecuteContext* ctx, const char* stdCode, ZTSVariant* cfg)
{
 ...
 *	isCanceled	是否已撤销
 */
void ZtMinImpactExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
	{
 ...
 *	交易通道就绪回调
 */
void ZtMinImpactExeUnit::on_channel_ready()
{
...
/*
	 *	交易通道丢失回调
	 */
void ZtMinImpactExeUnit::on_channel_lost()
{
 ...
 *	newTick	最新的tick数据
 */
void ZtMinImpactExeUnit::on_tick(ZTSTickData* newTick)
{
 ...
 *	price	成交价格
 */
void ZtMinImpactExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//不用触发,这里在ontick里触发吧
}
 ...
 *	下单结果回报
 */
void ZtMinImpactExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void ZtMinImpactExeUnit::do_calc()
{
 ...
 *	newVol	新的目标仓位
 */
void ZtMinImpactExeUnit::set_position(const char* stdCode, double newVol)
{
...
void ZtMinImpactExeUnit::clear_all_position(const char* stdCode)
{
