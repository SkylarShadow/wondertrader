...
const char* ZtTWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* ZtTWapExeUnit::getName()
{
	return "ZtTWapExeUnit";
}

void ZtTWapExeUnit::init(ExecuteContext* ctx, const char* stdCode, ZTSVariant* cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元ZtTWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒,间隔时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs, _fire_span).c_str());
}

void ZtTWapExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void ZtTWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void ZtTWapExeUnit::on_channel_lost()
{

}


void ZtTWapExeUnit::on_tick(ZTSTickData* newTick)
{
...
void ZtTWapExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//不用触发,这里在ontick里触发吧
}

void ZtTWapExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void ZtTWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
...
void ZtTWapExeUnit::set_position(const char* stdCode, double newVol)
{
