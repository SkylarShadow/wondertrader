/*!
 * \file ZtDiffMinImpactExeUnit.cpp
 *
 * \author Wesley
 * \date 2020/03/30
 *
...
ZtDiffMinImpactExeUnit::ZtDiffMinImpactExeUnit()
	: _last_tick(NULL)
	, _comm_info(NULL)
	, _price_mode(0)
	, _price_offset(0)
...
const char* ZtDiffMinImpactExeUnit::getFactName()
{
	return FACT_NAME;
}

const char* ZtDiffMinImpactExeUnit::getName()
{
	return "ZtDiffMinImpactExeUnit";
}

void ZtDiffMinImpactExeUnit::init(ExecuteContext* ctx, const char* stdCode, ZTSVariant* cfg)
{
...
void ZtDiffMinImpactExeUnit::on_order(uint32_t localid, const char* stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
	{
...
void ZtDiffMinImpactExeUnit::on_channel_ready()
{
...
void ZtDiffMinImpactExeUnit::on_channel_lost()
{
	
}

void ZtDiffMinImpactExeUnit::on_tick(ZTSTickData* newTick)
{
...
void ZtDiffMinImpactExeUnit::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	//如果是本地订单，则更新差量
	if (!_orders_mon.has_order(localid))
		return;
...
void ZtDiffMinImpactExeUnit::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void ZtDiffMinImpactExeUnit::do_calc()
{
...
void ZtDiffMinImpactExeUnit::set_position(const char* stdCode, double newDiff)
{
...
void ZtDiffMinImpactExeUnit::clear_all_position(const char* stdCode)
{
	_ctx->writeLog("Diff execute unit do not support clear command");
	return;
}
