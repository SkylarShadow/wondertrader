...
const char * ZtVWapExeUnit::getFactName()
{
	return FACT_NAME;
}

const char * ZtVWapExeUnit::getName()
{
	return "ZtVWapExeUnit";
}

void ZtVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, ZTSVariant * cfg)
{
	...
	ctx->writeLog(fmt::format("执行单元ZtVWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs).c_str());
...
void ZtVWapExeUnit::on_order(uint32_t localid, const char * stdCode, bool isBuy, double leftover, double price, bool isCanceled)
{
...
void ZtVWapExeUnit::on_channel_ready()
{
	_channel_ready = true;
...
void ZtVWapExeUnit::on_tick(ZTSTickData * newTick)
{
...
void ZtVWapExeUnit::on_trade(uint32_t localid, const char * stdCode, bool isBuy, double vol, double price)
{//在ontick中触发
}
/*
下单结果回报
*/
void ZtVWapExeUnit::on_entrust(uint32_t localid, const char * stdCode, bool bSuccess, const char * message)
{
	if (!bSuccess)
	{
		//如果不是我发出去的订单,我就不管了
...
void ZtVWapExeUnit::fire_at_once(double qty)
{
	if (decimal::eq(qty, 0))
		return;
	_last_tick->retain();
...
void ZtVWapExeUnit::set_position(const char * stdCode, double newVol)
{
...
void ZtVWapExeUnit::on_channel_lost()
{
}
