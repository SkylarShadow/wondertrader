...
public:
	bool init(const char* id, ZTSVariant* params, IBaseDataMgr* bdMgr, ActionPolicyMgr* policyMgr);
...
private:
	uint32_t doEntrust(ZTSEntrust* entrust);
	bool	doCancel(ZTSOrderInfo* ordInfo);

	inline void	printPosition(const char* stdCode, const PosItem& pItem);

	inline ZTSContractInfo* getContract(const char* stdCode);
	inline ZTSCommodityInfo* getCommodify(const char* stdCommID);

	const RiskParams* getRiskParams(const char* stdCode);

	void initSaveData();

	inline void	logTrade(uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo);
	inline void	logOrder(uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo);

	void	saveData(ZTSArray* ayFunds = NULL);
...
public:
	...
	uint32_t openLong(const char* stdCode, double price, double qty, int flag, ZTSContractInfo* cInfo = NULL);
	uint32_t openShort(const char* stdCode, double price, double qty, int flag, ZTSContractInfo* cInfo = NULL);
	uint32_t closeLong(const char* stdCode, double price, double qty, bool isToday, int flag, ZTSContractInfo* cInfo = NULL);
	uint32_t closeShort(const char* stdCode, double price, double qty, bool isToday, int flag, ZTSContractInfo* cInfo = NULL);
	
	OrderIDs buy(const char* stdCode, double price, double qty, int flag, bool bForceClose, ZTSContractInfo* cInfo = NULL);
	OrderIDs sell(const char* stdCode, double price, double qty, int flag, bool bForceClose, ZTSContractInfo* cInfo = NULL);
	...
	bool	checkSelfMatch(const char* stdCode, ZTSTradeInfo* tInfo);
...
public:
	...
	virtual void onRspEntrust(ZTSEntrust* entrust, ZTSError *err) override;
...
private:
	...
	typedef ZTSHashMap<std::string>	TradeStatMap;
