...
ZtArbiExecuter::ZtArbiExecuter(ZtExecuterFactory* factory, const char* name, IDataManager* dataMgr)
	: IExecCommand(name)
	, _factory(factory)
	, _data_mgr(dataMgr)
...
ZtArbiExecuter::~ZtArbiExecuter()
{
	if (_pool)
		_pool->wait();
}

void ZtArbiExecuter::setTrader(TraderAdapter* adapter)
{
	_trader = adapter;
	//设置的时候读取一下trader的状态
	if(_trader)
...
bool ZtArbiExecuter::init(ZTSVariant* params)
{
	if (params == NULL)
		return false;
...
ExecuteUnitPtr ZtArbiExecuter::getUnit(const char* stdCode, bool bAutoCreate /* = true */)
{
...
ZTSTickSlice* ZtArbiExecuter::getTicks(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_data_mgr == NULL)
		return NULL;
...
ZTSTickData* ZtArbiExecuter::grabLastTick(const char* stdCode)
{
	if (_data_mgr == NULL)
		return NULL;
...
double ZtArbiExecuter::getPosition(const char* stdCode, bool validOnly /* = true */, int32_t flag /* = 3 */)
{
	if (NULL == _trader)
		return 0.0;
...
double ZtArbiExecuter::getUndoneQty(const char* stdCode)
{
	if (NULL == _trader)
		return 0.0;
...
OrderMap* ZtArbiExecuter::getOrders(const char* stdCode)
{
	if (NULL == _trader)
		return NULL;
...
OrderIDs ZtArbiExecuter::buy(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
OrderIDs ZtArbiExecuter::sell(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
bool ZtArbiExecuter::cancel(uint32_t localid)
{
	if (!_channel_ready)
		return false;
...
OrderIDs ZtArbiExecuter::cancel(const char* stdCode, bool isBuy, double qty)
{
	if (!_channel_ready)
		return OrderIDs();
...
void ZtArbiExecuter::writeLog(const char* message)
{
...
ZTSCommodityInfo* ZtArbiExecuter::getCommodityInfo(const char* stdCode)
{
	return _stub->get_comm_info(stdCode);
}

ZTSSessionInfo* ZtArbiExecuter::getSessionInfo(const char* stdCode)
{
	return _stub->get_sess_info(stdCode);
}

uint64_t ZtArbiExecuter::getCurTime()
{
...
#pragma region 外部接口
void ZtArbiExecuter::on_position_changed(const char* stdCode, double diffPos)
{
...
void ZtArbiExecuter::set_position(const vvt_hashmap<std::string, double>& targets)
{
	/*
	 *	先要把目标头寸进行组合匹配
	 */
...
void ZtArbiExecuter::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void ZtArbiExecuter::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtArbiExecuter::on_order(uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled /* = false */)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtArbiExecuter::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtArbiExecuter::on_channel_ready()
{
	_channel_ready = true;
	SpinLock lock(_mtx_units);
...
void ZtArbiExecuter::on_channel_lost()
{
	_channel_ready = false;
	SpinLock lock(_mtx_units);
	for (auto it = _unit_map.begin(); it != _unit_map.end(); it++)
...
void ZtArbiExecuter::on_account(const char* currency, double prebalance, double balance, double dynbalance, 
...
void ZtArbiExecuter::on_position(const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail, uint32_t tradingday)
{
