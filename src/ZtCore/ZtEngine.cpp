...
void ZtEngine::set_date_time(uint32_t curDate, uint32_t curTime, uint32_t curSecs /* = 0 */, uint32_t rawTime /* = 0 */)
{
	_cur_date = curDate;
	_cur_time = curTime;
	_cur_secs = curSecs;
...
void ZtEngine::set_trading_date(uint32_t curTDate)
{
	_cur_tdate = curTDate; 
...
ZTSCommodityInfo* ZtEngine::get_commodity_info(const char* stdCode)
{
...
ZTSContractInfo* ZtEngine::get_contract_info(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	return _base_data_mgr->getContract(cInfo._code, cInfo._exchg);
}

std::string ZtEngine::get_rawcode(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	if (cInfo.hasRule())
	{
...
ZTSSessionInfo* ZtEngine::get_session_info(const char* sid, bool isCode /* = false */)
{
	if (!isCode)
		return _base_data_mgr->getSession(sid);

	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(sid, _hot_mgr);
	ZTSCommodityInfo* cInfo = _base_data_mgr->getCommodity(codeInfo._exchg, codeInfo._product);
	if (cInfo == NULL)
		return NULL;
...
void ZtEngine::on_tick(const char* stdCode, ZTSTickData* curTick)
{
	...
	push_task([this, code, price]{
		...
		{
			ZTSCommodityInfo* commInfo = get_commodity_info(code.c_str());
			double dynprofit = 0;
...
void ZtEngine::update_fund_dynprofit()
{
	ZTSFundStruct& fundInfo = _port_fund->fundInfo();
...
void ZtEngine::writeRiskLog(const char* message)
{
	static thread_local char szBuf[2048] = { 0 };
	auto len = vvt_strcpy(szBuf, "[RiskControl] ");
	vvt_strcpy(szBuf + len, message);
	ZTSLogger::log_raw_by_cat("risk", LL_INFO, szBuf);
}

uint32_t ZtEngine::getCurDate()
{
	return _cur_date;
}

uint32_t ZtEngine::getCurTime()
{
	return _cur_time;
}

uint32_t ZtEngine::getTradingDate()
{
	return _cur_tdate;
}

bool ZtEngine::isInTrading()
{
	return false;
}

void ZtEngine::setVolScale(double scale)
{
	double oldScale = _risk_volscale;
	_risk_volscale = scale;
	_risk_date = _cur_tdate;

	ZTSLogger::log_by_cat("risk", LL_INFO, "Position risk scale updated: {} - > {}", oldScale, scale);
	save_datas();
}

ZTSPortFundInfo* ZtEngine::getFundInfo()
{
	update_fund_dynprofit();
	save_datas();
...
void ZtEngine::init(ZTSVariant* cfg, IBaseDataMgr* bdMgr, ZtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier)
{
	...
	ZTSLogger::info("Running mode: Production");
	...
	ZTSVariant* cfgRisk = cfg->get("riskmon");
	if(cfgRisk)
	{
	...
	else
	{
		...
		//把更新时间间隔设置为5s
		_fund_udt_span = 5;
		ZTSLogger::log_raw(LL_WARN, "RiskMon is not configured, portfilio fund will be updated every 5s");
	}
}

void ZtEngine::on_session_end()
{
	//资金结算
	ZTSFundStruct& fundInfo = _port_fund->fundInfo();
	if (fundInfo._last_date < _cur_tdate)
	{
		std::string filename = ZtHelper::getPortifolioDir();
		filename += "funds.csv";
...
void ZtEngine::on_session_begin()
{

}

void ZtEngine::save_datas()
{
	...
	if (_port_fund != NULL)
	{//保存资金数据
		const ZTSFundStruct& fundInfo = _port_fund->fundInfo();
...
void ZtEngine::load_datas()
{
	_port_fund = ZTSPortFundInfo::create();
	...
	{//读取仓位
		...
		if (!jPos.IsNull() && jPos.IsArray())
		{
			for (const rj::Value& pItem : jPos.GetArray())
			{
				...
				ZTSLogger::debug("Porfolio position confirmed,{} -> {}", stdCode, pInfo->_volume);
			}
		}
		...
		ZTSLogger::debug("{} position info of portfolio loaded", _pos_map.size());
	}
...
ZTSTickSlice* ZtEngine::get_tick_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_tick_slice(code, count);
}

ZTSTickData* ZtEngine::get_last_tick(uint32_t sid, const char* stdCode)
{
	return _data_mgr->grab_last_tick(stdCode);
}

ZTSKlineSlice* ZtEngine::get_kline_slice(uint32_t sid, const char* stdCode, const char* period, uint32_t count, uint32_t times /* = 1 */, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* cInfo = _base_data_mgr->getCommodity(codeInfo._exchg, codeInfo._product);
	if (cInfo == NULL)
...
void ZtEngine::handle_push_quote(ZTSTickData* curTick)
{
...
double ZtEngine::get_cur_price(const char* stdCode)
{
	...
	if(it == _price_map.end())
	{
		...
		ZTSTickData* lastTick = _data_mgr->grab_last_tick(fCode.c_str());
		if (lastTick == NULL)
			return 0.0;

		ZTSContractInfo* cInfo = lastTick->getContractInfo();
...
double ZtEngine::get_day_price(const char* stdCode, int flag /* = 0 */)
{
	auto len = strlen(stdCode);
	char lastChar = stdCode[len - 1];
	//前复权直接读取标准合约代码
	...
	ZTSCommodityInfo* commInfo = get_commodity_info(fCode.c_str());
...
double ZtEngine::get_exright_factor(const char* stdCode, ZTSCommodityInfo* commInfo /* = NULL */)
{
	if (commInfo == NULL)
...
uint32_t ZtEngine::get_adjusting_flag()
{
	if (_data_mgr)
...
void ZtEngine::sub_tick(uint32_t sid, const char* stdCode)
{
...
void ZtEngine::load_fees(const char* filename)
{
	...
	{
		ZTSLogger::error("Fee templates file {} not exists", filename);
		return;
	}
	...
	if (cfg == NULL)
	{
		ZTSLogger::error("Fee templates file {} loading failed", filename);
		return;
	}
	...
	{
		ZTSVariant* cfgItem = cfg->get(fullPid.c_str());
		const StringVector& ay = StrUtil::split(fullPid, ".");
		ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(ay[0].c_str(), ay[1].c_str());
		if (commInfo == NULL)
			continue;
	...
	ZTSLogger::info("{} fee templates loaded", _fee_map.size());
}

double ZtEngine::calc_fee(const char* stdCode, double price, double qty, uint32_t offset)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	...
	if (it == _fee_map.end())
	{
		ZTSLogger::warn("Fee template of {} not found, return 0.0 as default", stdPID);
		return 0.0;
	}

	double ret = 0.0;
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(stdPID);
...
void ZtEngine::append_signal(const char* stdCode, double qty, bool bStandBy /* = true */)
{
	/*
...
void ZtEngine::do_set_position(const char* stdCode, double qty, double curPx /* = -1 */)
{
	PosInfoPtr& pInfo = _pos_map[stdCode];
	if (pInfo == NULL)
		pInfo.reset(new PosInfo);
	...
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	ZTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(codeInfo._exchg, codeInfo._product);
...
void ZtEngine::push_task(TaskItem task)
{
	{
...
bool ZtEngine::init_riskmon(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	//先看工作目录下是否有对应模块
	std::string dllpath = ZtHelper::getCWD() + module;
	//如果没有,则再看模块目录,即dll同目录下
	if (!StdFile::exists(dllpath.c_str()))
		dllpath = ZtHelper::getInstDir() + module;

	DllHandle hInst = DLLHelper::load_library(dllpath.c_str());
	if (hInst == NULL)
	{
		ZTSLogger::log_by_cat("risk", LL_ERROR, "Riskmon module {} loading failed", dllpath.c_str());
		return false;
	}
	...
	if (creator == NULL)
	{
		...
		ZTSLogger::log_by_cat("risk", LL_ERROR, "Riskmon module {} is not compatible", module.c_str());
		return false;
	}
	...
	_risk_mon.reset(new ZtRiskMonWrapper(_risk_fact._fact->createRiskMonotor(name), _risk_fact._fact));
	_risk_mon->self()->init(this, cfg);

	return true;
}

void ZtEngine::init_outputs()
{
	std::string folder = ZtHelper::getPortifolioDir();
...
void ZtEngine::log_trade(const char* stdCode, bool isLong, bool isOpen, uint64_t curTime, double price, double qty, double fee /* = 0.0 */)
{
	if (_trade_logs)
	{
		std::stringstream ss;
...
void ZtEngine::log_close(const char* stdCode, bool isLong, uint64_t openTime, double openpx, uint64_t closeTime, double closepx, double qty, double profit, double totalprofit /* = 0 */)
{
	if (_close_logs)
	{
		std::stringstream ss;
