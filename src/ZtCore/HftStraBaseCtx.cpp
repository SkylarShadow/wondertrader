...
HftStraBaseCtx::HftStraBaseCtx(ZtHftEngine* engine, const char* name, bool bAgent, int32_t slippage)
	: IHftStraCtx(name)
	, _engine(engine)
	, _data_agent(bAgent)
	, _slippage(slippage)
...
void HftStraBaseCtx::on_tick(const char* stdCode, ZTSTickData* newTick)
{
	if (_ud_modified)
	{
		save_userdata();
...
void HftStraBaseCtx::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_ud_modified)
	{
		save_userdata();
...
void HftStraBaseCtx::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_ud_modified)
	{
		save_userdata();
...
void HftStraBaseCtx::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
	if (_ud_modified)
	{
		save_userdata();
...
void HftStraBaseCtx::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (_ud_modified)
	{
		save_userdata();
...
OrderIDs HftStraBaseCtx::stra_buy(const char* stdCode, double price, double qty, const char* userTag, int flag /* = 0 */, bool bForceClose /* = false */)
{
	...
	if (strlen(cInfo._ruletag) > 0)
	{
		...
		ZTSContractInfo* ct = _engine->get_basedata_mgr()->getContract(code.c_str(), cInfo._exchg);
	...
	{
		ZTSContractInfo* ct = _engine->get_basedata_mgr()->getContract(cInfo._code, cInfo._exchg);
		if (ct == NULL)
		{
...
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _engine->get_hot_mgr());
	ZTSCommodityInfo* commInfo = _engine->get_basedata_mgr()->getCommodity(cInfo._exchg, cInfo._product);
...
ZTSCommodityInfo* HftStraBaseCtx::stra_get_comminfo(const char* stdCode)
{
...
ZTSKlineSlice* HftStraBaseCtx::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
...
ZTSTickSlice* HftStraBaseCtx::stra_get_ticks(const char* stdCode, uint32_t count)
{
...
ZTSOrdDtlSlice* HftStraBaseCtx::stra_get_order_detail(const char* stdCode, uint32_t count)
{
...
ZTSOrdQueSlice* HftStraBaseCtx::stra_get_order_queue(const char* stdCode, uint32_t count)
{
...
ZTSTransSlice* HftStraBaseCtx::stra_get_transaction(const char* stdCode, uint32_t count)
{
...
ZTSTickData* HftStraBaseCtx::stra_get_last_tick(const char* stdCode)
{
...
void HftStraBaseCtx::save_userdata()
{
	...
	{
		std::string filename = ZtHelper::getStraUsrDatDir();
		filename += "ud_";
		filename += _name;
		filename += ".json";
...
void HftStraBaseCtx::do_set_position(const char* stdCode, double qty, double price /* = 0.0 */, const char* userTag /*= ""*/)
{
	...
	ZTSCommodityInfo* commInfo = _engine->get_commodity_info(stdCode);
...
void HftStraBaseCtx::update_dyn_profit(const char* stdCode, ZTSTickData* newTick)
{
	auto it = _pos_map.find(stdCode);
	if (it != _pos_map.end())
	{
