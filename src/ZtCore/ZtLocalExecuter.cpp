...
ZtLocalExecuter::ZtLocalExecuter(ZtExecuterFactory* factory, const char* name, IDataManager* dataMgr)
	: IExecCommand(name)
	, _factory(factory)
	, _data_mgr(dataMgr)
...
ZtLocalExecuter::~ZtLocalExecuter()
{
	if (_pool)
		_pool->wait();
}

void ZtLocalExecuter::setTrader(TraderAdapter* adapter)
{
	_trader = adapter;
	//设置的时候读取一下trader的状态
	if(_trader)
...
bool ZtLocalExecuter::init(ZTSVariant* params)
{
	if (params == NULL)
		return false;
	 ...
	 */
	ZTSVariant* cfgClear = params->get("clear");
	if(cfgClear)
	{
		_auto_clear = cfgClear->getBoolean("active");
		ZTSVariant* cfgItem = cfgClear->get("includes");
		if(cfgItem)
		{
	...
	ZTSVariant* cfgGroups = params->get("groups");
	if (cfgGroups)
	{
		...
		for(const std::string& gpname : names)
		{
			...
			ZTSVariant* cfgGrp = cfgGroups->get(gpname.c_str());
	...
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Local executer inited, scale: {}, auto_clear: {}, strict_sync: {}, thread poolsize: {}, code_groups: {}",
		_scale, _auto_clear, _strict_sync, poolsize, _groups.size());

	return true;
}

ExecuteUnitPtr ZtLocalExecuter::getUnit(const char* stdCode, bool bAutoCreate /* = true */)
{
...
ZTSTickSlice* ZtLocalExecuter::getTicks(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_data_mgr == NULL)
		return NULL;
...
ZTSTickData* ZtLocalExecuter::grabLastTick(const char* stdCode)
{
	if (_data_mgr == NULL)
		return NULL;
...
double ZtLocalExecuter::getPosition(const char* stdCode, bool validOnly /* = true */, int32_t flag /* = 3 */)
{
	if (NULL == _trader)
		return 0.0;
...
double ZtLocalExecuter::getUndoneQty(const char* stdCode)
{
	if (NULL == _trader)
		return 0.0;
...
OrderMap* ZtLocalExecuter::getOrders(const char* stdCode)
{
	if (NULL == _trader)
		return NULL;
...
OrderIDs ZtLocalExecuter::buy(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
OrderIDs ZtLocalExecuter::sell(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
bool ZtLocalExecuter::cancel(uint32_t localid)
{
	if (!_channel_ready)
		return false;
...
OrderIDs ZtLocalExecuter::cancel(const char* stdCode, bool isBuy, double qty)
{
	if (!_channel_ready)
		return OrderIDs();
...
void ZtLocalExecuter::writeLog(const char* message)
{
...
ZTSCommodityInfo* ZtLocalExecuter::getCommodityInfo(const char* stdCode)
{
	return _stub->get_comm_info(stdCode);
}

ZTSSessionInfo* ZtLocalExecuter::getSessionInfo(const char* stdCode)
{
	return _stub->get_sess_info(stdCode);
}

uint64_t ZtLocalExecuter::getCurTime()
{
...
#pragma region 外部接口
void ZtLocalExecuter::on_position_changed(const char* stdCode, double diffPos)
{
	...
	{
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Target position of {} changed: {} -> {} : {} with scale:{}", stdCode, oldVol, newVol, traderTarget, _scale);
	}

	if (_trader && !_trader->checkOrderLimits(stdCode))
	{
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "{} is disabled", stdCode);
		return;
	}
...
void ZtLocalExecuter::set_position(const vvt_hashmap<std::string, double>& targets)
{
	/*
	 *	先要把目标头寸进行组合匹配
	 */
	...
	for (auto it = targets.begin(); it != targets.end(); it++)
	{
		...
		if (_trader && !_trader->checkOrderLimits(stdCode))
		{
			ZTSLogger::log_dyn("executer", _name.c_str(), LL_WARN, "{} is disabled due to entrust limit control ", stdCode);
			continue;
		}
	...
	for (auto it = _target_pos.begin(); it != _target_pos.end(); it++)
	{
		...
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "{} is not in target, set to 0 automatically", code);
	...
	if(_strict_sync)
	{
		for(const std::string& stdCode : _channel_holds)
		{
			...
			ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "{} is not in management, set to 0 due to strict sync mode", stdCode.c_str());
...
void ZtLocalExecuter::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void ZtLocalExecuter::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtLocalExecuter::on_order(uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled /* = false */)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtLocalExecuter::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtLocalExecuter::on_channel_ready()
{
	_channel_ready = true;
	SpinLock lock(_mtx_units);
...
void ZtLocalExecuter::on_channel_lost()
{
	_channel_ready = false;
	SpinLock lock(_mtx_units);
	for (auto it = _unit_map.begin(); it != _unit_map.end(); it++)
...
void ZtLocalExecuter::on_account(const char* currency, double prebalance, double balance, double dynbalance, 
...
void ZtLocalExecuter::on_position(const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail, uint32_t tradingday)
{
	...
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Prev hot contract of {}.{} on {} is {}", cInfo._exchg, cInfo._product, tradingday, prevCode);
	...
	{
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Position of {}, as prev hot contract, won't be cleared for it's in exclude list", stdCode);
		return;
	}
	...
	if(!_clear_includes.empty())
	{
		...
		{
			ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Position of {}, as prev hot contract, won't be cleared for it's not in include list", stdCode);
			return;
		}
	}

	//最后再进行自动清理
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "Position of {}, as prev hot contract, will be cleared", stdCode);
	ExecuteUnitPtr unit = getUnit(stdCode);
	if (unit)
	{
		if (_pool)
