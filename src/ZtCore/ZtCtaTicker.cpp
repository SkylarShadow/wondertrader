...
//////////////////////////////////////////////////////////////////////////
//WtTimeTicker
void ZtCtaRtTicker::init(IDataReader* store, const char* sessionID)
{
	_store = store;
	_s_info = _engine->get_session_info(sessionID);
	if(_s_info == NULL)
		ZTSLogger::fatal("Session {} is invalid, CtaTicker cannot run correctly", sessionID);
	else
		ZTSLogger::info("CtaTicker will drive engine with session {}", sessionID);

	TimeUtils::getDateTime(_date, _time);
}

void ZtCtaRtTicker::trigger_price(ZTSTickData* curTick)
{
	if (_engine )
	{
...
void ZtCtaRtTicker::on_tick(ZTSTickData* curTick)
{
	if (_thrd == NULL)
	{
	...
	{
		//ZTSLogger::info("行情时间{}小于本地时间{}", uTime, _time);
		trigger_price(curTick);
		return;
	}
	...
	else if (_cur_pos < minutes)
	{
		...
		if (_last_emit_pos < _cur_pos)
		{
			...
			ZTSLogger::info("Minute Bar {}.{:04d} Closed by data", _date, thisMin);
			if (_store)
...
void ZtCtaRtTicker::run()
{
	...
	ZTSLogger::info("Trading date confirmed: {}", curTDate);
	_engine->on_init();
	...
	_thrd.reset(new StdThread([this](){
		while(!_stopped)
		{
			...
			if (_time != UINT_MAX && _s_info->isInTradingTime(_time / 100000, true))
			{
				...
				if (now >= _next_check_time && _last_emit_pos < _cur_pos)
				{
					...
					if (thisMin == 0)
					{
						...
						_time = 0;
						ZTSLogger::info("Data automatically changed at time 00:00: {} -> {}", lastDate, _date);
					}
					...
					ZTSLogger::info("Minute bar {}.{:04d} closed automatically", _date, thisMin);
					if (_store)
						_store->onMinuteEnd(_date, thisMin, bEndingTDate ? _engine->getTradingDate() : 0);
			...
			else //if(offTime >= _s_info->getOpenTime(true) && offTime <= _s_info->getCloseTime(true))
			{
				...
				{
					ZTSLogger::warn("Tradingday {} will be ended forcely, last_emit_pos: {}, time: {}", _engine->getTradingDate(), _last_emit_pos.fetch_add(0), _time);
...
bool ZtCtaRtTicker::is_in_trading() const 
{
	if (_s_info == NULL)
		return false;
...
uint32_t ZtCtaRtTicker::time_to_mins(uint32_t uTime) const
{
	if (_s_info == NULL)
		return uTime;
