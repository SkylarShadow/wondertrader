...
SelStraContext::SelStraContext(ZtSelEngine* engine, const char* name, int32_t slippage)
...
void SelStraContext::on_bar_close(const char* stdCode, const char* period, ZTSBarStruct* newBar)
{
	if (_strategy)
		_strategy->on_bar(this, stdCode, period, newBar);
}

void SelStraContext::on_tick_updated(const char* stdCode, ZTSTickData* newTick)
{
