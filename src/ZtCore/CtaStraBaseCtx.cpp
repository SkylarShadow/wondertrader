...
CtaStraBaseCtx::CtaStraBaseCtx(ZtCtaEngine* engine, const char* name, int32_t slippage)
	: ICtaStraCtx(name)
	, _engine(engine)
	, _total_calc_time(0)
	, _emit_times(0)
...
{
	std::string filename = ZtHelper::getStraDataDir();
	filename += _name;
	filename += ".json";
		...
		if (!jCond.IsNull() && jCond.IsObject())
		{
			...
			for (auto& m : jItems.GetObject())
			{
				...
				for(auto& cItem : cListItem.GetArray())
				{
					...
					condInfo._field = (ZTSCompareField)cItem["field"].GetUint();
					condInfo._alg = (ZTSCompareType)cItem["alg"].GetUint();
...
//////////////////////////////////////////////////////////////////////////
//回调函数
void CtaStraBaseCtx::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void CtaStraBaseCtx::on_tick(const char* stdCode, ZTSTickData* newTick, bool bEmitStrategy /* = true */)
{
...
bool CtaStraBaseCtx::on_schedule(uint32_t curDate, uint32_t curTime)
{
	...
	for (auto it = _kline_tags.begin(); it != _kline_tags.end(); it++)
	{
		...
		ZTSSessionInfo* sInfo = _engine->get_session_info(stdCode.c_str(), true);
...
ZTSKlineSlice* CtaStraBaseCtx::stra_get_bars(const char* stdCode, const char* period, uint32_t count, bool isMain /* = false */)
{
	thread_local static char key[64] = { 0 };
	...
	ZTSKlineSlice* kline = _engine->get_kline_slice(_context_id, stdCode, basePeriod, count, times);
	if(kline)
	{
...
ZTSTickSlice* CtaStraBaseCtx::stra_get_ticks(const char* stdCode, uint32_t count)
{
	ZTSTickSlice* ret = _engine->get_tick_slice(_context_id, stdCode, count);
	if (ret)
		_engine->sub_tick(id(), stdCode);
...
ZTSTickData* CtaStraBaseCtx::stra_get_last_tick(const char* stdCode)
{
...
ZTSCommodityInfo* CtaStraBaseCtx::stra_get_comminfo(const char* stdCode)
{
...
double CtaStraBaseCtx::stra_get_position(const char* stdCode, bool bOnlyValid /* = false */, const char* userTag /* = "" */)
{
	...
	if(sit != _sig_map.end())
	{
		ZTSLogger::warn("{} has untouched signal, [userTag] will be ignored", stdCode);
