...
bool CtaStrategyMgr::loadFactories(const char* path)
{
	...
	{
		ZTSLogger::error("Directory {} of CTA strategy factory not exists", path);
		return false;
	}
	...
	for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++)
	{
		...
		if(fact != NULL)
		{
			...
			ZTSLogger::info("CTA strategy factory[{}] loaded", fact->getName());
	...
	ZTSLogger::info("{} CTA strategy factories in directory[{}] loaded", count, path);
