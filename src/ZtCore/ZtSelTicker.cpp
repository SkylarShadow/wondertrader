...
ZtSelRtTicker::ZtSelRtTicker(ZtSelEngine* engine)
	: _engine(engine)
	, _stopped(false)
	, _date(0)
	, _time(UINT_MAX)
...
ZtSelRtTicker::~ZtSelRtTicker()
{
}

void ZtSelRtTicker::init(IDataReader* store, const char* sessionID)
{
	_store = store;
...
void ZtSelRtTicker::trigger_price(ZTSTickData* curTick, uint32_t hotFlag /* = 0 */)
{
	if (_engine)
	{
...
void ZtSelRtTicker::on_tick(ZTSTickData* curTick, uint32_t hotFlag /* = 0 */)
{
	if (_thrd == NULL)
	{
