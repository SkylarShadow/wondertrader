...
//////////////////////////////////////////////////////////////////////////
#pragma region "ZtExecuterMgr"
void ZtExecuterMgr::enum_executer(EnumExecuterCb cb)
{
...
void ZtExecuterMgr::set_positions(vvt_hashmap<std::string, double> target_pos)
{
	if(_filter_mgr != NULL)
	{
		...
		for(auto& m : target_pos)
		{
			...
			if (!isFltd)
			{
				...
				{
					//输出日志
					ZTSLogger::info("[Filters] {} target position reset by code filter: {} -> {}", stdCode.c_str(), oldVol, desVol);
				}
			...
			{
				//输出日志
				ZTSLogger::info("[Filters] {} target position ignored by filter", stdCode.c_str());
			}
		}
	...
	for (auto& v : _executers)
	{
		...
		{
			ZTSLogger::info("[Filters] Executer {} is filtered, all signals will be ignored", executer->name());
			continue;
		}
...
void ZtExecuterMgr::handle_pos_change(const char* stdCode, double targetPos, double diffPos, const char* execid /* = "ALL" */)
{
	if(_filter_mgr != NULL)
	{
		...
		if (!isFltd)
		{
			...
			{
				//输出日志
				ZTSLogger::info("[Filters] {} target position reset by filter: {} -> {}", stdCode, oldVol, targetPos);
				//差量也要重算
		...
		{
			//输出日志
			ZTSLogger::info("[Filters] {} target position ignored by filter", stdCode);
			return;
		}
	}
	...
	for (auto& v : _executers)
	{
		...
		if (_filter_mgr && _filter_mgr->is_filtered_by_executer(executer->name()))
		{
			ZTSLogger::info("[Filters] All signals to executer {} are ignored by executer filter", executer->name());
			continue;
		}
...
void ZtExecuterMgr::handle_tick(const char* stdCode, ZTSTickData* curTick)
{
...
void ZtExecuterMgr::add_target_to_cache(const char* stdCode, double targetPos, const char* execid /* = "ALL" */)
{
...
void ZtExecuterMgr::commit_cached_targets(double scale /* = 1.0 */)
{
...
bool ZtExecuterMgr::load_router_rules(ZTSVariant* config)
{
	...
	{
		ZTSVariant* item = config->get(i);
		const char* straName = item->getCString("strategy");
		ZTSVariant* itemExec = item->get("executer");
		if(itemExec->isArray())
		{
			...
			for(uint32_t k = 0; k < cnt; k++)
			{
				...
				ZTSLogger::info("Signal of strategy {} will be routed to executer {}", straName, execId);
	...
	ZTSLogger::info("{} router rules loaded", _router_rules.size());

	return true;
}

#pragma endregion "ZtExecuterMgr"
