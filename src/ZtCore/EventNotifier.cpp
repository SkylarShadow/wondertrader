...
bool EventNotifier::init(ZTSVariant* cfg)
{
	if (!cfg->getBoolean("active"))
		return false;

	_url = cfg->getCString("url");
	std::string module = DLLHelper::wrap_module("ZtMsgQue", "lib");
	//先看工作目录下是否有对应模块
	...
	if (dllInst == NULL)
	{
		ZTSLogger::error("MQ module {} loading failed", dllpath.c_str());
		return false;
	}
	...
	if (_creator == NULL)
	{
		...
		ZTSLogger::error("MQ module {} is not compatible", dllpath.c_str());
		return false;
	}
	...
	ZTSLogger::info("EventNotifier initialized with channel {}", _url.c_str());
...
void EventNotifier::notify(const char* trader, uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo)
{
...
void EventNotifier::notify(const char* trader, uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo)
{
...
void EventNotifier::tradeToJson(const char* trader, uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo, std::string& output)
{
	if(trdInfo == NULL)
	{
		output = "{}";
...
void EventNotifier::orderToJson(const char* trader, uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo, std::string& output)
{
	if (ordInfo == NULL)
	{
		output = "{}";
