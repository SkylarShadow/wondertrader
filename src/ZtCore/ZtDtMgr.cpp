...
bool ZtDtMgr::initStore(ZTSVariant* cfg)
{
	if (cfg == NULL)
		return false;
	...
	if(hInst == NULL)
	{
		ZTSLogger::error("Loading data reader module {} failed", module.c_str());
		return false;
	}
	...
	if(funcCreator == NULL)
	{
		ZTSLogger::error("Loading data reader module {} failed, entrance function createDataReader not found", module.c_str());
	...
	if(_reader == NULL)
	{
		ZTSLogger::error("Creating instance of data reader module {} failed", module.c_str());
		DLLHelper::free_library(hInst);
		return false;
	}
...
bool ZtDtMgr::init(ZTSVariant* cfg, ZtEngine* engine, bool bForceCache /* = false */)
{
	_engine = engine;
	...
	ZTSLogger::info("Force to cache bars: {}", _force_cache ? "yes" : " no");

	return initStore(cfg->get("store"));
}

void ZtDtMgr::on_all_bar_updated(uint32_t updateTime)
{
	if (_bar_notifies.empty())
		return;

	ZTSLogger::debug("All bars updated, on_bar will be triggered");
...
IBaseDataMgr* ZtDtMgr::get_basedata_mgr()
{ 
	return _engine->get_basedata_mgr(); 
}

IHotMgr* ZtDtMgr::get_hot_mgr() 
{ 
	return _engine->get_hot_mgr(); 
}

uint32_t ZtDtMgr::get_date() 
{ 
	return _engine->get_date(); 
}

uint32_t ZtDtMgr::get_min_time()
{ 
	return _engine->get_min_time(); 
}

uint32_t ZtDtMgr::get_secs() 
{ 
	return _engine->get_secs(); 
}

void ZtDtMgr::reader_log(ZTSLogLevel ll, const char* message)
{
	ZTSLogger::log_raw(ll, message);
}

void ZtDtMgr::on_bar(const char* code, ZTSKlinePeriod period, ZTSBarStruct* newBar)
{
	...
	ZTSSessionInfo* sInfo = _engine->get_session_info(code, true);
...
void ZtDtMgr::handle_push_quote(const char* stdCode, ZTSTickData* newTick)
{
	if (newTick == NULL)
		return;
	...
	{
		ZTSHisTickData* tData = (ZTSHisTickData*)_ticks_adjusted->get(stdCode);
		if (tData == NULL)
			return;
...
ZTSTickData* ZtDtMgr::grab_last_tick(const char* code)
{
	if (_rt_tick_map == NULL)
		return NULL;
...
double ZtDtMgr::get_adjusting_factor(const char* stdCode, uint32_t uDate)
{
	if (_reader)
...
uint32_t ZtDtMgr::get_adjusting_flag()
{
...
ZTSTickSlice* ZtDtMgr::get_tick_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
	...
	if (it == _ticks_adjusted->end())
	{
		...
		ZTSTickSlice* slice = _reader->readTickSlice(pureStdCode.c_str(), 999999, etime);
		std::vector<ZTSTickStruct> ayTicks;
		ayTicks.resize(slice->size());
		std::size_t offset = 0;
		for (std::size_t bIdx = 0; bIdx < slice->get_block_counts(); bIdx++)
		{
			memcpy(&ayTicks[0] + offset, slice->get_block_addr(bIdx), slice->get_block_size(bIdx) * sizeof(ZTSTickStruct));
			offset += slice->get_block_size(bIdx);
		}

		//缓存的数据做一个复权处理
		for (ZTSTickStruct& tick : ayTicks)
		{
			tick.price *= factor;
			tick.open *= factor;
			tick.high *= factor;
		...
		//添加到缓存中
		ZTSHisTickData* hisTick = ZTSHisTickData::create(stdCode, false, factor);
		hisTick->getDataRef().swap(ayTicks);
		_ticks_adjusted->add(pureStdCode, hisTick, false);
	}

	ZTSHisTickData* hisTick = (ZTSHisTickData*)_ticks_adjusted->get(pureStdCode);
	...
	ZTSTickStruct* pTick = std::lower_bound(&ticks.front(), &ticks.back(), eTick, [](const ZTSTickStruct& a, const ZTSTickStruct& b) {
	...
	ZTSTickSlice* slice = ZTSTickSlice::create(stdCode, &ticks.front() + sIdx, cnt);
	return slice;
}

ZTSOrdQueSlice* ZtDtMgr::get_order_queue_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
ZTSOrdDtlSlice* ZtDtMgr::get_order_detail_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
ZTSTransSlice* ZtDtMgr::get_transaction_slice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
...
ZTSKlineSlice* ZtDtMgr::get_kline_slice(const char* stdCode, ZTSKlinePeriod period, uint32_t times, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_reader == NULL)
		return NULL;
	...
	//只有非基础周期的会进到下面的步骤
	ZTSSessionInfo* sInfo = _engine->get_session_info(stdCode, true);
	...
	if (kData == NULL || kData->size() < count)
	{
		...
		if (rawData != NULL && rawData->size() > 0)
		{
			...
			{
				kData = ZTSKlineData::create(stdCode, rawData->size());
				kData->setPeriod(period, 1);
				kData->setClosed(true);
				ZTSBarStruct* pBar = kData->getDataRef().data();
				for(uint32_t bIdx = 0; bIdx < rawData->get_block_counts(); bIdx++ )
				{
					memcpy(pBar, rawData->get_block_addr(bIdx), sizeof(ZTSBarStruct)*rawData->get_block_size(bIdx));
		...
		if (kData)
		{
			...
			if(times != 1)
				ZTSLogger::debug("{} bars of {} resampled every {} bars: {} -> {}", 
