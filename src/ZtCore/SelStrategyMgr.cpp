...
bool SelStrategyMgr::loadFactories(const char* path)
{
	...
	{
		ZTSLogger::error("Directory {} of SEL strategy factory not exists", path);
		return false;
	}
	...
	for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++)
	{
		...
		if (fact != NULL)
		{
			...
			ZTSLogger::info("SEL strategy factory[{}] loaded", fact->getName());
	...
	ZTSLogger::info("{} SEL strategy factories in directory[{}] loaded", count, path);
