...
SelStraBaseCtx::SelStraBaseCtx(ZtSelEngine* engine, const char* name, int32_t slippage)
	: ISelStraCtx(name)
	, _engine(engine)
	, _total_calc_time(0)
	, _emit_times(0)
...
//////////////////////////////////////////////////////////////////////////
//回调函数
void SelStraBaseCtx::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (newBar == NULL)
		return;
...
void SelStraBaseCtx::on_tick(const char* stdCode, ZTSTickData* newTick, bool bEmitStrategy /* = true */)
{
...
ZTSKlineSlice* SelStraBaseCtx::stra_get_bars(const char* stdCode, const char* period, uint32_t count)
{
	thread_local static char key[64] = { 0 };
	...
	ZTSKlineSlice* kline = _engine->get_kline_slice(_context_id, stdCode, basePeriod, count, times, etime);
...
ZTSTickSlice* SelStraBaseCtx::stra_get_ticks(const char* stdCode, uint32_t count)
{
	return _engine->get_tick_slice(_context_id, stdCode, count);
}

ZTSTickData* SelStraBaseCtx::stra_get_last_tick(const char* stdCode)
{
...
ZTSCommodityInfo* SelStraBaseCtx::stra_get_comminfo(const char* stdCode)
{
...
ZTSSessionInfo* SelStraBaseCtx::stra_get_sessinfo(const char* stdCode)
{
