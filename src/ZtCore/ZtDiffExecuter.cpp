...
ZtDiffExecuter::ZtDiffExecuter(ZtExecuterFactory* factory, const char* name, IDataManager* dataMgr, IBaseDataMgr* bdMgr)
	: IExecCommand(name)
	, _factory(factory)
	, _data_mgr(dataMgr)
...
ZtDiffExecuter::~ZtDiffExecuter()
{
	if (_pool)
		_pool->wait();
}

void ZtDiffExecuter::setTrader(TraderAdapter* adapter)
{
	_trader = adapter;
	//设置的时候读取一下trader的状态
	if(_trader)
...
bool ZtDiffExecuter::init(ZTSVariant* params)
{
	if (params == NULL)
		return false;
	...
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Diff executer inited, scale: {}, thread poolsize: {}", _name, _scale, poolsize);

	return true;
}

void ZtDiffExecuter::load_data()
{
	//读取执行器的理论部位，以及待执行的差量
	std::string filename = ZtHelper::getExecDataDir();
	filename += _name + ".json";
	...
	if(root.HasMember("targets"))
	{
		...
		for (const rj::Value& jItem : jTargets.GetArray())
		{
			...
			ZTSContractInfo* ct = _bd_mgr->getContract(cInfo._code, cInfo._exchg);
			if (ct == NULL)
			{
				ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Ticker {} is not valid", _name, stdCode);
				continue;
			}
...
ExecuteUnitPtr ZtDiffExecuter::getUnit(const char* stdCode, bool bAutoCreate /* = true */)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, NULL);
	std::string commID = codeInfo.stdCommID();

	ZTSVariant* policy = _config->get("policy");
	...
	if (bAutoCreate)
	{
		ZTSVariant* cfg = policy->get(des.c_str());
		...
		{
			ZTSLogger::error("Creating ExecUnit {} failed", name);
		}
		return unit;
	}
	else
...
ZTSTickSlice* ZtDiffExecuter::getTicks(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	if (_data_mgr == NULL)
		return NULL;
...
ZTSTickData* ZtDiffExecuter::grabLastTick(const char* stdCode)
{
	if (_data_mgr == NULL)
		return NULL;
...
double ZtDiffExecuter::getPosition(const char* stdCode, bool validOnly /* = true */, int32_t flag /* = 3 */)
{
	if (NULL == _trader)
		return 0.0;
...
double ZtDiffExecuter::getUndoneQty(const char* stdCode)
{
	if (NULL == _trader)
		return 0.0;
...
OrderMap* ZtDiffExecuter::getOrders(const char* stdCode)
{
	if (NULL == _trader)
		return NULL;
...
OrderIDs ZtDiffExecuter::buy(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
OrderIDs ZtDiffExecuter::sell(const char* stdCode, double price, double qty, bool bForceClose/* = false*/)
{
	if (!_channel_ready)
		return OrderIDs();
...
bool ZtDiffExecuter::cancel(uint32_t localid)
{
	if (!_channel_ready)
		return false;
...
OrderIDs ZtDiffExecuter::cancel(const char* stdCode, bool isBuy, double qty)
{
	if (!_channel_ready)
		return OrderIDs();
...
void ZtDiffExecuter::writeLog(const char* message)
{
	static thread_local char szBuf[2048] = { 0 };
	fmtutil::format_to(szBuf, "[{}] {}", _name.c_str(), message);
	ZTSLogger::log_dyn_raw("executer", _name.c_str(), LL_INFO, szBuf);
}

ZTSCommodityInfo* ZtDiffExecuter::getCommodityInfo(const char* stdCode)
{
	return _stub->get_comm_info(stdCode);
}

ZTSSessionInfo* ZtDiffExecuter::getSessionInfo(const char* stdCode)
{
	return _stub->get_sess_info(stdCode);
}

uint64_t ZtDiffExecuter::getCurTime()
{
...
#pragma region 外部接口
void ZtDiffExecuter::on_position_changed(const char* stdCode, double diffPos)
{
	...
	*/
	...
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Target position of {} changed additonally: {} -> {}, diff postion changed: {} -> {}", _name, stdCode, oldVol, targetPos, prevDiff, thisDiff);

	if (_trader && !_trader->checkOrderLimits(stdCode))
	{
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] {} is disabled", _name, stdCode);
		return;
	}
...
void ZtDiffExecuter::set_position(const vvt_hashmap<std::string, double>& targets)
{
	for (auto it = targets.begin(); it != targets.end(); it++)
	{
		...
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Target position of {} changed: {} -> {}, diff postion changed: {} -> {}", _name, stdCode, oldVol, newVol, prevDiff, thisDiff);

		if (_trader && !_trader->checkOrderLimits(stdCode))
		{
			ZTSLogger::log_dyn("executer", _name.c_str(), LL_WARN, "[{}] {} is disabled due to entrust limit control ", _name, stdCode);
			continue;
		}
	...
	for (auto it = _target_pos.begin(); it != _target_pos.end(); it++)
	{
		...
		ZTSContractInfo* cInfo = _bd_mgr->getContract(stdCode);
		if(cInfo == NULL)
			continue;

		if(pos != 0)
		{
			ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] {} is not in target, set to 0 automatically", _name, stdCode);
			...
			//ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[DiffExecuter][set_position][{}] {} is not in target, thisDiff: {}, prevDiff: {}, pos: {}, new thisDiff: {}", _name, stdCode, thisDiff, prevDiff, pos, thisDiff + pos);
...
void ZtDiffExecuter::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void ZtDiffExecuter::on_trade(uint32_t localid, const char* stdCode, bool isBuy, double vol, double price)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
	...
	ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Diff of {} updated by trade: {} -> {}", _name, stdCode, prevDiff, curDiff);
	save_data();
...
void ZtDiffExecuter::on_order(uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled /* = false */)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtDiffExecuter::on_entrust(uint32_t localid, const char* stdCode, bool bSuccess, const char* message)
{
	ExecuteUnitPtr unit = getUnit(stdCode, false);
	if (unit == NULL)
		return;
...
void ZtDiffExecuter::on_channel_ready()
{
	_channel_ready = true;
	//SpinLock lock(_mtx_units);
	...
	for(auto& v : _diff_pos)
	{
		...
		ZTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "[{}] Diff of {} recovered to {}", _name, stdCode, thisDiff);
	}
}

void ZtDiffExecuter::on_channel_lost()
{
	_channel_ready = false;
	//SpinLock lock(_mtx_units);
	for (auto it = _unit_map.begin(); it != _unit_map.end(); it++)
...
void ZtDiffExecuter::on_account(const char* currency, double prebalance, double balance, double dynbalance,
...
void ZtDiffExecuter::on_position(const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail, uint32_t tradingday)
{
