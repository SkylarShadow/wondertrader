...
void ZtHftEngine::init(ZTSVariant* cfg, IBaseDataMgr* bdMgr, ZtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier /* = NULL */)
{
...
void ZtHftEngine::run()
{
	...
	_tm_ticker = new ZtHftRtTicker(this);
...
void ZtHftEngine::handle_push_quote(ZTSTickData* newTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(newTick);
}

void ZtHftEngine::handle_push_order_detail(ZTSOrdDtlData* curOrdDtl)
{
...
void ZtHftEngine::handle_push_order_queue(ZTSOrdQueData* curOrdQue)
{
...
void ZtHftEngine::handle_push_transaction(ZTSTransData* curTrans)
{
...
void ZtHftEngine::sub_order_detail(uint32_t sid, const char* stdCode)
{
...
void ZtHftEngine::sub_order_queue(uint32_t sid, const char* stdCode)
{
	std::size_t length = strlen(stdCode);
	if (stdCode[length - 1] == SUFFIX_QFQ || stdCode[length - 1] == SUFFIX_HFQ)
		length--;
...
void ZtHftEngine::sub_transaction(uint32_t sid, const char* stdCode)
{
	std::size_t length = strlen(stdCode);
	if (stdCode[length - 1] == SUFFIX_QFQ || stdCode[length - 1] == SUFFIX_HFQ)
		length--;
...
void ZtHftEngine::on_tick(const char* stdCode, ZTSTickData* curTick)
{
...
void ZtHftEngine::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
...
void ZtHftEngine::on_session_begin()
{
	ZTSLogger::info("Trading day {} begun", _cur_tdate);
	ZtEngine::on_session_begin();
...
void ZtHftEngine::on_session_end()
{
...
void ZtHftEngine::on_minute_end(uint32_t curDate, uint32_t curTime)
{
	//已去掉高频策略的on_schedule
...
void ZtHftEngine::addContext(HftContextPtr ctx)
{
	uint32_t sid = ctx->id();
	_ctx_map[sid] = ctx;
}

HftContextPtr ZtHftEngine::getContext(uint32_t id)
{
...
ZTSOrdQueSlice* ZtHftEngine::get_order_queue_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_queue_slice(code, count);
}

ZTSOrdDtlSlice* ZtHftEngine::get_order_detail_slice(uint32_t sid, const char* code, uint32_t count)
{
	return _data_mgr->get_order_detail_slice(code, count);
}

ZTSTransSlice* ZtHftEngine::get_transaction_slice(uint32_t sid, const char* code, uint32_t count)
{
