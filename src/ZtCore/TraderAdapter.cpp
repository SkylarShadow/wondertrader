...
inline const char* formatAction(ZTSDirectionType dType, ZTSOffsetType oType)
{
	if(dType == WDT_LONG)
	{
		if (oType == WOT_OPEN)
...
bool TraderAdapter::init(const char* id, ZTSVariant* params, IBaseDataMgr* bdMgr, ActionPolicyMgr* policyMgr)
{
	if (params == NULL)
		return false;
	...
	//这里解析流量风控参数
	ZTSVariant* cfgRisk = params->get("riskmon");
	if (cfgRisk)
	{
		if (cfgRisk->getBoolean("active"))
		{
			...
			ZTSVariant* cfgPolicy = cfgRisk->get("policy");
			auto keys = cfgPolicy->memberNames();
			for (auto it = keys.begin(); it != keys.end(); it++)
			{
				const char* product = (*it).c_str();
				ZTSVariant*	vProdItem = cfgPolicy->get(product);
				...
				ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Risk control rule {} of trading channel loaded", _id.c_str(), product);
			}

			auto it = _risk_params_map.find("default");
			if (it == _risk_params_map.end())
			{
				ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] Some instruments may not be monitored due to no default risk control rule of trading channel", _id.c_str());
			}
		}
		else
		{
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] Risk control rule of trading channel not activated", _id.c_str());
		}
	}
	else
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] No risk control rule setup of trading channel", _id.c_str());
	}
	...
	//先看工作目录下是否有交易模块
	std::string dllpath = ZtHelper::getModulePath(module.c_str(), "traders", true);
	//如果没有,则再看模块目录,即dll同目录下
	if (!StdFile::exists(dllpath.c_str()))
		dllpath = ZtHelper::getModulePath(module.c_str(), "traders", false);
	DllHandle hInst = DLLHelper::load_library(dllpath.c_str());
	if (hInst == NULL)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Loading trading module {} failed", _id.c_str(), dllpath.c_str());
		return false;
	}
	else
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Trader module {} loaded", _id.c_str(), dllpath.c_str());
	}

	FuncCreateTrader pFunCreateTrader = (FuncCreateTrader)DLLHelper::get_symbol(hInst, "createTrader");
	if (NULL == pFunCreateTrader)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL, "[{}] Entrance function createTrader not found", _id.c_str());
		return false;
	}
	...
	if (NULL == _trader_api)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL, "[{}] Creating trading api failed", _id.c_str());
		return false;
	}
	...
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Entrance function deleteTrader not found", id);
		return false;
	}
...
bool TraderAdapter::initExt(const char* id, ITraderApi* api, IBaseDataMgr* bdMgr, ActionPolicyMgr* policyMgr)
{
	...
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Trader initializing failed", id);
		return false;
	}
...
void TraderAdapter::initSaveData()
{
	...
	folder += "//";*/
	std::stringstream ss;
	ss << ZtHelper::getBaseDir() << "traders/" << _id << "//";
...
void TraderAdapter::logTrade(uint32_t localid, const char* stdCode, ZTSTradeInfo* trdInfo)
{
...
void TraderAdapter::logOrder(uint32_t localid, const char* stdCode, ZTSOrderInfo* ordInfo)
{
...
void TraderAdapter::saveData(ZTSArray* ayFunds /* = NULL */)
{
	...
	{//资金保存
		...
		if(ayFunds && ayFunds->size() > 0)
		{
			...
			{
				ZTSAccountInfo* fundInfo = (ZTSAccountInfo*)(*it);
...
uint32_t TraderAdapter::doEntrust(ZTSEntrust* entrust)
{
	_trader_api->makeEntrustID(entrust->getEntrustID(), 64);

	ZTSContractInfo* cInfo = entrust->getContractInfo();
	...
	if(ret < 0)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] Order placing failed: {}", _id.c_str(), ret);
		return UINT_MAX;
	}
	else
	{
...
void TraderAdapter::updateUndone(const char* stdCode, double qty, bool bOuput /* = false */)
{
	...
	if (bOuput)
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] {} qty of undone order updated, {} -> {}", _id.c_str(), stdCode, oldQty, undone);
}

ZTSContractInfo* TraderAdapter::getContract(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, NULL);
	return _bd_mgr->getContract(cInfo._code, cInfo._exchg);
}

ZTSCommodityInfo* TraderAdapter::getCommodify(const char* stdCode)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, NULL);
...
bool TraderAdapter::checkCancelLimits(const char* stdCode)
{
	...
	ZTSTradeStateInfo* statInfo = (ZTSTradeStateInfo*)_stat_map->get(stdCode);
	if (statInfo && riskPara->_cancel_total_limits != 0 && statInfo->total_cancels() >= riskPara->_cancel_total_limits )
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] {} cancel {} times totaly, beyond boundary {} times, adding to excluding list",
	...
	if (it != _cancel_time_cache.end())
	{
		...
		if (cnt >= riskPara->_cancel_times_boundary)
		{
			...
			{
				ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] {} cancel {} times within {} seconds, beyond boundary {} times, adding to excluding list",
...
bool TraderAdapter::checkOrderLimits(const char* stdCode)
{
	...
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] {} entrust {} times totally, beyond boundary {} times, adding to excluding list",
	...
	if (it != _order_time_cache.end())
	{
		...
		if (cnt >= riskPara->_order_times_boundary)
		{
			...
			{
				ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, "[{}] {} entrust {} times within {} seconds, beyond boundary {} times, adding to excluding list",
...
OrderIDs TraderAdapter::buy(const char* stdCode, double price, double qty, int flag, bool bForceClose, ZTSContractInfo* cInfo /* = NULL */)
{
	OrderIDs ret;
	if (qty == 0)
		return ret;
	...
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
	...
	if (!sInfo->isInTradingTime(ZtHelper::getTime(), true))
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
			"[{}] Buying {} of quantity {}, {:04d} not in trading time", _id.c_str(), stdCode, qty, ZtHelper::getTime());
		return ret;
	}


	ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Buying {} of quantity {}", _id.c_str(), stdCode, qty);
	...
	if (statInfo == NULL)
	{
		statInfo = ZTSTradeStateInfo::create(stdCode);
	...
	for (auto it = ruleGP.begin(); it != ruleGP.end(); it++)
	{
		...
		if(curRule._atype == AT_Open && !bForceClose)
		{
			...
			if (curRule._limit_l != 0)
			{
				...
				{
					ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] {} long position opened today is up to limit {}", _id.c_str(), stdCode, curRule._limit_l);
					continue;
				}
				else
				{
			...
			if (curRule._limit != 0)
			{
				...
				{
					ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] {} position opened today is up to limit {}", _id.c_str(), stdCode, curRule._limit);
					continue;
				}
				else
				{
			...
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, 
		...
		else if(curRule._atype == AT_CloseToday)
		{
			...
			{
				ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
...
OrderIDs TraderAdapter::sell(const char* stdCode, double price, double qty, int flag, bool bForceClose, ZTSContractInfo* cInfo /* = NULL */)
{
	OrderIDs ret;
	if (qty == 0)
		return ret;
	...
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, 
			"[{}] Selling {} of quantity {}, {:04d} not in trading time", _id.c_str(), stdCode, qty, ZtHelper::getTime());
		return ret;
	}

	ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Selling {} of quantity {}", _id.c_str(), stdCode, qty);
	...
	for (auto it = ruleGP.begin(); it != ruleGP.end(); it++)
	{
		...
		if (curRule._atype == AT_Open && !bForceClose)
		{
			...
			if (curRule._limit_s != 0)
			{
				...
				{
					ZTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "[{}] {} short position opened today is up to limit {}", _id.c_str(), stdCode, curRule._limit_l);
					continue;
				}
				else
				{
...
bool TraderAdapter::doCancel(ZTSOrderInfo* ordInfo)
{
	if (ordInfo == NULL || !ordInfo->isAlive())
		return false;

	ZTSContractInfo* cInfo = ordInfo->getContractInfo();
	ZTSCommodityInfo* commInfo = cInfo->getCommInfo();
	std::string stdCode;
	...
	ZTSEntrustAction* action = ZTSEntrustAction::create(ordInfo->getCode(), cInfo->getExchg());
...
bool TraderAdapter::cancel(uint32_t localid)
{
	...
	{
		SpinLock lock(_mtx_orders);
		ordInfo = (ZTSOrderInfo*)_orders->grab(localid);
		if (ordInfo == NULL)
			return false;
	}
...
OrderIDs TraderAdapter::cancel(const char* stdCode, bool isBuy, double qty /* = 0 */)
{
	...
	if (_orders != NULL && _orders->size() > 0)
	{
		for (auto it = _orders->begin(); it != _orders->end(); it++)
		{
			ZTSOrderInfo* orderInfo = (ZTSOrderInfo*)it->second;
...
uint32_t TraderAdapter::openLong(const char* stdCode, double price, double qty, int flag, ZTSContractInfo* cInfo /* = NULL */)
{
	ZTSEntrust* entrust = ZTSEntrust::create(stdCode, qty, price);
	...
	entrust->setOrderFlag((ZTSOrderFlag)(WOF_NOR + flag));
...
uint32_t TraderAdapter::openShort(const char* stdCode, double price, double qty, int flag, ZTSContractInfo* cInfo /* = NULL */)
{
	ZTSEntrust* entrust = ZTSEntrust::create(stdCode, qty, price);
	entrust->setContractInfo(cInfo == NULL ? getContract(stdCode) : cInfo);
	if (decimal::eq(price, 0.0))
...
uint32_t TraderAdapter::closeLong(const char* stdCode, double price, double qty, bool isToday, int flag, ZTSContractInfo* cInfo /* = NULL */)
{
	ZTSEntrust* entrust = ZTSEntrust::create(stdCode, qty, price);
	entrust->setContractInfo(cInfo == NULL ? getContract(stdCode) : cInfo);
	if (decimal::eq(price, 0.0))
...
uint32_t TraderAdapter::closeShort(const char* stdCode, double price, double qty, bool isToday, int flag, ZTSContractInfo* cInfo /* = NULL */)
{
	ZTSEntrust* entrust = ZTSEntrust::create(stdCode, qty, price);
	entrust->setContractInfo(cInfo == NULL ? getContract(stdCode) : cInfo);
	if (decimal::eq(price, 0.0))
...
void TraderAdapter::handleEvent(ZTSTraderEvent e, int32_t ec)
{
	if(e == ZTE_Connect)
	{
		if(ec == 0)
		{
			_trader_api->login(_cfg->getCString("user"), _cfg->getCString("pass"), ZT_PRODUCT);
		}
		else
		{
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,"[{}] Trading channel connecting failed: {}", _id.c_str(), ec);
		}
	}
	else if(e == ZTE_Close)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,"[{}] Trading channel disconnected: {}", _id.c_str(), ec);
...
void TraderAdapter::onLoginResult(bool bSucc, const char* msg, uint32_t tradingdate)
{
	if(!bSucc)
	{
		...
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,"[{}] Trader login failed: {}", _id.c_str(), msg);
	...
	{
		_state = AS_LOGINED;
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,"[{}] Trader login succeed, trading date: {}", _id.c_str(), tradingdate);
...
void TraderAdapter::onRspEntrust(ZTSEntrust* entrust, ZTSError *err)
{
	if (err && err->getErrorCode() != WEC_NONE)
	{
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR, err->getMessage());
		ZTSContractInfo* cInfo = entrust->getContractInfo();
		ZTSCommodityInfo* commInfo = cInfo->getCommInfo();
		std::string stdCode;
		if (commInfo->getCategoty() == CC_FutOption || commInfo->getCategoty() == CC_SpotOption)
...
void TraderAdapter::onRspAccount(ZTSArray* ayAccounts)
{
	...
	if(ayAccounts)
	{
		...
		for (auto sink : _sinks)
		{
			...
			{
				ZTSAccountInfo* fundInfo = (ZTSAccountInfo*)ayAccounts->at(idx);
	...
	if(_state == AS_TRADES_QRYED)
	{
		...
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Trading channel ready", _id.c_str());
		for (auto& sink : _sinks)
...
void TraderAdapter::onRspPosition(const ZTSArray* ayPositions)
{
	if (ayPositions && ayPositions->size() > 0)
	{
		...
		{
			ZTSPositionItem* pItem = (ZTSPositionItem*)(*it);
			ZTSContractInfo* cInfo = pItem->getContractInfo();
			if (cInfo == NULL)
				continue;
	...
	ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,"[{}] Position data updated", _id.c_str());
...
void TraderAdapter::onRspOrders(const ZTSArray* ayOrders)
{
	if (ayOrders)
	{
		...
		{
			ZTSOrderInfo* orderInfo = (ZTSOrderInfo*)(*it);
			if (orderInfo == NULL)
				continue;

			ZTSContractInfo* cInfo = orderInfo->getContractInfo();
			if (cInfo == NULL)
				continue;
			...
			ZTSTradeStateInfo* statInfo = (ZTSTradeStateInfo*)_stat_map->get(stdCode.c_str());
			if (statInfo == NULL)
			{
				statInfo = ZTSTradeStateInfo::create(stdCode.c_str());
				_stat_map->add(stdCode, statInfo, false);
			}
			TradeStatInfo& statItem = statInfo->statInfo();
			if (isBuy)
...
{
	ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] {} position updated, long:{}[{}]|{}[{}], short:{}[{}]|{}[{}]",
...
void TraderAdapter::onRspTrades(const ZTSArray* ayTrades)
{
	if (ayTrades)
	{
		...
		{
			ZTSTradeInfo* tInfo = (ZTSTradeInfo*)(*it);

			ZTSContractInfo* cInfo = tInfo->getContractInfo();
			if (cInfo == NULL)
				continue;
		...
		{
			const char* stdCode = it->first.c_str();
			ZTSTradeStateInfo* pItem = (ZTSTradeStateInfo*)it->second;
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, 
...
bool TraderAdapter::checkSelfMatch(const char* stdCode, ZTSTradeInfo* tInfo)
{
	if (tInfo == NULL)
		return false;
	...
	if (it != _trade_refs.end())
	{
		...
		if (oid.compare(refid) != 0)
		{
			...
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL, 
...
inline const char* stateToName(ZTSOrderState woState)
{
...
void TraderAdapter::onPushOrder(ZTSOrderInfo* orderInfo)
{
	if (orderInfo == NULL)
		return;
	...
	ZTSLogger::log_dyn("trader", _id.c_str(), LL_DEBUG, "[{}] Order notified, instrument: {}, usertag: {}, state: {}", _id.c_str(), stdCode.c_str(), orderInfo->getUserTag(), stateToName(orderInfo->getOrderState()));
	...
	if (StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern.c_str(), true))
	{
		...
		if (orderInfo->getOrderState() == WOS_Canceled )
		{
			...
			ZTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] Order {} of {} canceled:{}, action: {}, leftqty: {}",
...
void TraderAdapter::onPushTrade(ZTSTradeInfo* tradeRecord)
{
	ZTSContractInfo* cInfo = tradeRecord->getContractInfo();
	if (cInfo == NULL)
		return;
...
{
	if(err)
		ZTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,"[{}] Error of trading channel occured: {}", _id.c_str(), err->getMessage());
...
{
	ZTSLogger::log_dyn_raw("trader", _id.c_str(), ll, message);
}
...
bool TraderAdapterMgr::addAdapter(const char* tname, TraderAdapterPtr& adapter)
{
	...
	if(it != _adapters.end())
	{
		ZTSLogger::error("Same name of trading channels: {}", tname);
		return false;
	}
...
void TraderAdapterMgr::run()
{
	...
	ZTSLogger::info("{} trading channels started", _adapters.size());
}
