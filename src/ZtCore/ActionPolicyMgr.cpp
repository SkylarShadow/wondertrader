...
bool ActionPolicyMgr::init(const char* filename)
{
	...
	for (auto it = keys.begin(); it != keys.end(); it++)
	{
		...
		ZTSVariant*	vGpItem = cfg->get(gpName);
		ActionRuleGroup& gp = _rules[gpName];

		ZTSVariant* vOrds = vGpItem->get("order");
		if(vOrds != NULL && vOrds->isArray())
		{
			for (uint32_t i = 0; i < vOrds->size(); i++)
			{
				ZTSVariant* vObj = vOrds->get(i);
				ActionRule aRule;
				...
				{
					ZTSLogger::error("Loading action policy failed: unrecognized type {}", action);
					continue;
				}
		...
		ZTSVariant* filters = vGpItem->get("filters");
...
const ActionRuleGroup& ActionPolicyMgr::getActionRules(const char* pid)
{
...

	{
		...
		if (it == _rules.end())
		{
			...
			ZTSLogger::error("Action policy group {} not exists, changed to default group", gpName.c_str());
		}
