...
void ZtCtaEngine::run()
{
	_tm_ticker = new ZtCtaRtTicker(this);
	...
	//启动之前,先把运行中的策略落地
	{
		...
		std::string filename = ZtHelper::getBaseDir();
...
void ZtCtaEngine::init(ZTSVariant* cfg, IBaseDataMgr* bdMgr, ZtDtMgr* dataMgr, IHotMgr* hotMgr, EventNotifier* notifier /* = NULL */)
{
	ZtEngine::init(cfg, bdMgr, dataMgr, hotMgr, notifier);
	...
	ZTSLogger::info("Engine task poolsize is {}", poolsize);
}

void ZtCtaEngine::addContext(CtaContextPtr ctx)
{
	uint32_t sid = ctx->id();
	_ctx_map[sid] = ctx;
}

CtaContextPtr ZtCtaEngine::getContext(uint32_t id)
{
...
void ZtCtaEngine::on_init()
{
	...
	for (auto it = _ctx_map.begin(); it != _ctx_map.end(); it++)
	{
		...
		ctx->enum_position([this, ctx, exec_ids](const char* stdCode, double qty){
			...
			if (!bFilterd)
			{
				...
				{
					//输出日志
					ZTSLogger::info("[Filters] Target position of {} of strategy {} reset by strategy filter: {} -> {}", 
			...
			{
				//输出日志
				ZTSLogger::info("[Filters] Target position of {} of strategy {} ignored by strategy filter", stdCode, ctx->name());
			}
		}, true);
	}
	...
	{
		ZTSLogger::log_by_cat("risk", LL_INFO, "Risk scale of portfolio is {:.2f}", _risk_volscale);
		bRiskEnabled = true;
	}
	...
	//	ZTSLogger::info("Portfolio initial position of {} is {}", stdCode.c_str(), pos);
	//}
...
void ZtCtaEngine::on_session_begin()
{
...
void ZtCtaEngine::on_session_end()
{
	ZtEngine::on_session_end();
...
void ZtCtaEngine::on_schedule(uint32_t curDate, uint32_t curTime)
{
	//去检查一下过滤器
	...
	if(!decimal::eq(_risk_volscale, 1.0) && _risk_date == _cur_tdate)
	{
		ZTSLogger::log_by_cat("risk", LL_INFO, "Risk scale of strategy group is {:.2f}", _risk_volscale);
		bRiskEnabled = true;
	}
	...
	for(auto& m : _pos_map)
	{
		...
		if (target_pos.find(stdCode) == target_pos.end())
		{
			...
			{
				//这里是通知ZtEngine去更新组合持仓数据
				append_signal(stdCode.c_str(), 0, true);

				ZTSLogger::error("Instrument {} not in target positions, setup to 0 automatically", stdCode.c_str());
			}
...
void ZtCtaEngine::handle_push_quote(ZTSTickData* newTick)
{
	if (_tm_ticker)
		_tm_ticker->on_tick(newTick);
}

void ZtCtaEngine::handle_pos_change(const char* straName, const char* stdCode, double diffPos)
{
	//这里是持仓增量,所以不用处理未过滤的情况,因为增量情况下,不会改变目标diffQty
	if(_filter_mgr.is_filtered_by_strategy(straName, diffPos, true))
	{
		//输出日志
		ZTSLogger::info("[Filters] Target position of {} of strategy {} ignored by strategy filter", stdCode, straName);
		return;
	}
...
void ZtCtaEngine::on_tick(const char* stdCode, ZTSTickData* curTick)
{
	ZtEngine::on_tick(stdCode, curTick);
			...
			if (cit != _ctx_map.end())
			{
				...
				else
				{
					...
					else //(opt == 2)
					{
						if (adjTick == nullptr)
						{
							ZTSTickData* adjTick = ZTSTickData::create(curTick->getTickStruct());
							ZTSTickStruct& adjTS = adjTick->getTickStruct();
...
void ZtCtaEngine::on_bar(const char* stdCode, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	...
	ZTSLogger::info("KBar [{}] @ {} closed", key, period[0] == 'd' ? newBar->date : newBar->time);
}

bool ZtCtaEngine::isInTrading()
{
	return _tm_ticker->is_in_trading();
}

uint32_t ZtCtaEngine::transTimeToMin(uint32_t uTime)
{
	return _tm_ticker->time_to_mins(uTime);
}

ZTSCommodityInfo* ZtCtaEngine::get_comm_info(const char* stdCode)
{
...
ZTSSessionInfo* ZtCtaEngine::get_sess_info(const char* stdCode)
{
	CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
...
uint64_t ZtCtaEngine::get_real_time()
{
	return TimeUtils::makeTime(_cur_date, _cur_raw_time * 100000 + _cur_secs);
}

void ZtCtaEngine::notify_chart_marker(uint64_t time, const char* straId, double price, const char* icon, const char* tag)
{
	if (_notifier)
		_notifier->notify_chart_marker(time, straId, price, icon, tag);
}

void ZtCtaEngine::notify_chart_index(uint64_t time, const char* straId, const char* idxName, const char* lineName, double val)
{
	if (_notifier)
		_notifier->notify_chart_index(time, straId, idxName, lineName, val);
}

void ZtCtaEngine::notify_trade(const char* straId, const char* stdCode, bool isLong, bool isOpen, uint64_t curTime, double price, const char* userTag)
{
	if (_notifier)
