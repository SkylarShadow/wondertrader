...
HftStraContext::HftStraContext(ZtHftEngine* engine, const char* name, bool bAgent, int32_t slippage)
...
void HftStraContext::on_tick(const char* stdCode, ZTSTickData* newTick)
{
...
void HftStraContext::on_order_queue(const char* stdCode, ZTSOrdQueData* newOrdQue)
{
	if (_strategy)
...
void HftStraContext::on_order_detail(const char* stdCode, ZTSOrdDtlData* newOrdDtl)
{
	if (_strategy)
...
void HftStraContext::on_transaction(const char* stdCode, ZTSTransData* newTrans)
{
	if (_strategy)
...
void HftStraContext::on_bar(const char* code, const char* period, uint32_t times, ZTSBarStruct* newBar)
{
	if (_strategy)
