...
extern "C"
{
	...
	{
		IBtDtReader* ret = new VvtBtDtReaderAD();
		return ret;
	}
...
VvtBtDtReaderAD::VvtBtDtReaderAD()
{
}
...
void VvtBtDtReaderAD::init(VvTSVariant* cfg, IBtDtReaderSink* sink)
{
	_sink = sink;
	...
	pipe_btreader_log(_sink, LL_INFO, "VvtBtDtReaderAD initialized, root data dir is {}", _base_dir);
}

bool VvtBtDtReaderAD::read_raw_bars(const char* exchg, const char* code, VvTSKlinePeriod period, std::string& buffer)
{
	//直接从LMDB读取
...
bool VvtBtDtReaderAD::read_raw_ticks(const char* exchg, const char* code, uint32_t uDate, std::string& buffer)
{
	//直接从LMDB读取
...
VvtBtDtReaderAD::VvtLMDBPtr VvtBtDtReaderAD::get_k_db(const char* exchg, VvTSKlinePeriod period)
{
...
VvtBtDtReaderAD::VvtLMDBPtr VvtBtDtReaderAD::get_t_db(const char* exchg, const char* code)
{
