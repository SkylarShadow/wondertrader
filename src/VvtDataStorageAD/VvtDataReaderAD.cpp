...
extern "C"
{
	...
	{
		IDataReader* ret = new VvtDataReaderAD();
		return ret;
	}
...
VvtDataReaderAD::VvtDataReaderAD()
	: _last_time(0)
	, _base_data_mgr(NULL)
	, _hot_mgr(NULL)
{
...
void VvtDataReaderAD::init(VvTSVariant* cfg, IDataReaderSink* sink, IHisDataLoader* loader /* = NULL */)
{
	...
	pipe_reader_log(sink, LL_INFO, "VvtDataReaderAD initialized, root data folder is {}", _base_dir);
}

VvTSTickSlice* VvtDataReaderAD::readTickSlice(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
...
std::string VvtDataReaderAD::read_bars_to_buffer(const char* exchg, const char* code, VvTSKlinePeriod period)
{
	//直接从LMDB读取
...
bool VvtDataReaderAD::cacheBarsFromStorage(const std::string& key, const char* stdCode, VvTSKlinePeriod period, uint32_t count)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
...
void VvtDataReaderAD::update_cache_from_lmdb(BarsList& barsList, const char* exchg, const char* code, VvTSKlinePeriod period, uint32_t& lastBarTime)
{
...
VvTSBarStruct* VvtDataReaderAD::get_rt_cache_bar(const char* exchg, const char* code, VvTSKlinePeriod period)
{
...
VvTSKlineSlice* VvtDataReaderAD::readKlineSlice(const char* stdCode, VvTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
...
void VvtDataReaderAD::onMinuteEnd(uint32_t uDate, uint32_t uTime, uint32_t endTDate /* = 0 */)
{
	//这里应该触发检查
...
VvtDataReaderAD::VvtLMDBPtr VvtDataReaderAD::get_k_db(const char* exchg, VvTSKlinePeriod period)
{
...
VvtDataReaderAD::VvtLMDBPtr VvtDataReaderAD::get_t_db(const char* exchg, const char* code)
{
