...
extern "C"
{
	...
	{
		IRdmDtReader* ret = new VvtRdmDtReaderAD();
		return ret;
	}
...
VvtRdmDtReaderAD::VvtRdmDtReaderAD()
	: _base_data_mgr(NULL)
	, _hot_mgr(NULL)
{
}
...
void VvtRdmDtReaderAD::init(VvTSVariant* cfg, IRdmDtReaderSink* sink)
{
	...
	pipe_rdmreader_log(sink, LL_INFO, "VvtRdmDtReaderAD initialized, root data folder is {}", _base_dir);
}

VvTSTickSlice* VvtRdmDtReaderAD::readTickSliceByCount(const char* stdCode, uint32_t count, uint64_t etime /* = 0 */)
{
	//TODO: 以后再来实现吧
	return NULL;
}

VvTSTickSlice* VvtRdmDtReaderAD::readTickSliceByRange(const char* stdCode, uint64_t stime, uint64_t etime /* = 0 */)
{
...
VvTSKlineSlice* VvtRdmDtReaderAD::readKlineSliceByCount(const char* stdCode, VvTSKlinePeriod period, uint32_t count, uint64_t etime /* = 0 */)
{
	//TODO: 以后再来实现吧
	return NULL;
}

VvTSTickSlice* VvtRdmDtReaderAD::readTickSliceByDate(const char* stdCode, uint32_t uDate )
{
	//TODO: 以后再来实现吧
	return NULL;
}

VvTSKlineSlice* VvtRdmDtReaderAD::readKlineSliceByRange(const char* stdCode, VvTSKlinePeriod period, uint64_t stime, uint64_t etime /* = 0 */)
{
	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
	VvTSCommodityInfo* commInfo = _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
	std::string stdPID = StrUtil::printf("%s.%s", cInfo._exchg, cInfo._product);
...
VvtRdmDtReaderAD::VvtLMDBPtr VvtRdmDtReaderAD::get_k_db(const char* exchg, VvTSKlinePeriod period)
{
...
VvtRdmDtReaderAD::VvtLMDBPtr VvtRdmDtReaderAD::get_t_db(const char* exchg, const char* code)
{
