...
extern "C"
{
	...
	{
		IDataWriter* ret = new VvtDataWriterAD();
		return ret;
	}
...
VvtDataWriterAD::VvtDataWriterAD()
	: _terminated(false)
...
bool VvtDataWriterAD::init(VvTSVariant* params, IDataWriterSink* sink)
{
...
void VvtDataWriterAD::loadCache()
{
...
void* VvtDataWriterAD::resizeRTBlock(BoostMFPtr& mfPtr, uint32_t nCount)
{
	if (mfPtr == NULL)
		return NULL;
...
bool VvtDataWriterAD::writeTick(VvTSTickData* curTick, uint32_t procFlag)
{
	if (curTick == NULL)
		return false;
...
void VvtDataWriterAD::pushTask(TaskInfo task)
{
	if(_async_task)
	{
...
void VvtDataWriterAD::pipeToTicks(VvTSContractInfo* ct, VvTSTickData* curTick)
{
	//直接落地
...
void VvtDataWriterAD::pipeToDayBars(VvTSContractInfo* ct, const VvTSBarStruct& bar)
{
	//直接落地
...
void VvtDataWriterAD::pipeToM1Bars(VvTSContractInfo* ct, const VvTSBarStruct& bar)
{
	//直接落地
...
void VvtDataWriterAD::pipeToM5Bars(VvTSContractInfo* ct, const VvTSBarStruct& bar)
{
	//直接落地
...
void VvtDataWriterAD::updateBarCache(VvTSContractInfo* ct, VvTSTickData* curTick)
{
...
VvTSTickData* VvtDataWriterAD::getCurTick(const char* code, const char* exchg/* = ""*/)
{
	if (strlen(code) == 0)
		return NULL;
...
bool VvtDataWriterAD::updateTickCache(VvTSContractInfo* ct, VvTSTickData* curTick, uint32_t procFlag)
{
...
VvtDataWriterAD::VvtLMDBPtr VvtDataWriterAD::get_k_db(const char* exchg, VvTSKlinePeriod period)
{
...
VvtDataWriterAD::VvtLMDBPtr VvtDataWriterAD::get_t_db(const char* exchg, const char* code)
{
