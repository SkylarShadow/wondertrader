### 0.9.9
* （**重要**）VvtDtCore新增一个ShmCaster，通过一个长度为8000的内存映射文件，转发实时行情数据（tick、逐笔、订单等）
* （**重要**）新增一个ParserShm模块，配合ShmCaster，实时解析ShmCaster的数据接入实时行情数据
* （**重要**）VvtDtMgr增加了一个强制缓存的控制，开启以后，所有K线都有一份缓存，基础周期的K线也有一份副本
...
* VvtDtHelper模块新增了对股票level2数据的处理接口，如read_dsb_order_details和store_order_details等
* 新增了一个copy_bins_linux.sh，可以自动复制linux编译的二进制文件到对应的VvtPy目录
* 其他细节完善和bug修正

### 0.9.7
* （**重要**）VvtDataReader完善了对后复权数据处理的逻辑（原来有可能导致读取不到足够数量的数据）
...
* VvtDataWriter细化了K线闭合的逻辑，增加了对无成交tick的控制选项（By Hej）
...
### 0.9.6
* （**重要**）完善了VvtDtServo，增加了实时行情的接入，以及对实时tick和K线的订阅的支持
...
### 0.9.0(重大版本)
* 将数据读写模块VvtDataReader和VvtDataWriter统一整合为VvtDataStorage，并将回测框架和VvtDtServo中的随机读取模块整合到该模块中
* （**重要**）新增一个基于LMDB实现的数据存储引擎VvtDataStorageAD，主要针对7*24小时交易品种的存储场景
...
* （**重要**）优化了VvTSBaseDataMgr加载合约的流程，支持将合约直接当成品种进行处理，主要为了适应一些每个合约的交易规则都不同的场景
...
### 0.8.0(大版本)
* （**重要**）实现了ExtDataLoder的机制，实盘和回测框架都可以通过应用层的扩展数据加载器加载历史数据（详见VvtBtCore/HisDataReplayer和VvtDataStorage/WtDtReader）
...
* 将VvtDataReader和VvtDataWriter合并成VvtDataStorage，把相同存储机制的reader和writer放在一个工程下，方便后面扩展别的存储机制的模块
...
* VvtDtHelper模块中，将trans_bars和trans_ticks接口取消，改用store_bars和store_ticks代替，采用连续整块内存转储的方式
* 其他细节优化和bug修正

### 0.7.1
* 回测框架的CTAMocker和HFTMocker都增加了单步调度切入机制，用于单步控制回测的进度（该机制的主要目的是针对强化学习的通用接口提供对接方式）
* VvtDtPorter支持了扩展Parser，用户可以用VvtPy中直接向datakit底层推送实时行情
* 执行单元WtExecUnit新增一个接口clear_all_position，用于清理某个合约的全部持仓（净头寸机制下，全部锁仓时，正常执行单元不会平仓，该接口就是用于告知执行单元，多空头寸也要全部清掉）
* 其他的细节优化和Bug修正

### 0.7.0
* 新增一个VvtDtServo模块，用于提供直接访问WonderTrader落地的数据文件的接口，主要包括按照时间区间获取数据get_bars_by_range、get_ticks_by_range，以及按照条数获取数据get_bars_by_count、get_ticks_by_count两大类接口
* 根据VvtDtServo的需求，优化了VvtDataWriter模块数据处理流程
...
* VvTSTools工程下，新增一个CsvReader类，方便从csv文件加载历史数据
...
* 交易通道合规风控代码细节优化
* 支持模块名自动根据平台补全后缀
* 本地执行器VvtLocalExecuter支持针对品种设置执行策略
* VvtDtHelper模块新增一个resamle_bars接口，用于对K线进行重采样
* 内部逻辑增加了对次主力合约的支持，代码格式如CFFEX.IF.2ND
* 完善了回测框架CTA策略回测时对于数据的兼容性
* 完善了tick回测时对于tick数据不够时的兼容性
* 完善了回测框架条件单处理的细节
* VvtBtPorter的dump_bars接口修改为通过回调返回数据，而不是直接导出到csv
* VvtPorter新增dump_bars接口，用法同VvtBtPorter
* 其他Bug修正和细节优化

### 0.6.4
* 性能调优
* 调整了CTAMocker中的一些细节
* 优化了C接口模块获取数据的接口如cta_get_bars、cta_get_ticks，可以提升调用层(VvtPy)的性能
* TraderCTPOpt模块中CashIn为权利金净收入，合并到平仓盈亏字段中
* VvtDtHelper模块增加了trans_bars和trans_ticks，用于通过C接口直接将数据转储到dsb文件
* VvtPorter新增了ExpParser和ExpExecuter两个类，用于与C接口的外接行情接入模块和执行模块进行交互
* VvtPorter模块接口增加向底层推送行情数据的接口parser_push_quote
* 完善了VvtDtCore中行情接入模块订阅过滤规则，统一改成filter参数
* VvtBtPorter模块和VvtDtHelper模块，从CSV导入K线数据，添加了对更多字段的支持
* CAT策略新增获取资金数据接口stra_get_fund_data
* CTAMocker和SelMocker暴露设置滑点的地方
* 增加自动补全动态库文件名的机制，目前主要适配好了风控模块、行情解析模块、交易模块、数据读写模块
* VvtDataWriter增加一些配置项，用于控制不同类型的数据是否落地
...
* VvtDtHelper增加两个接口,read_dsb_ticks用于读取dsb格式的历史tick数据，read_dsb_bars用于读取dsb格式的历史K线数据
...
* `VvtPorter`和`VvtBtPorter`两个C接口粘合模块，初步完成了C接口对股票`Level2`数据的支持
* 其他代码细节的完善

### 0.5.4
* `VvtBtPorter`、`VvtPorter`、`VvtExecMon`的初始化接口，全部改成支持传文件名和文件内容两种方式
...
* 新增一个`VvtDtHelper`模块，主要提供数据辅助功能，目前主要是提供`csv`和二进制文件的互转，后面还会加入数据库、二进制、`csv`的互转接口
...
* 历史数据新增了对`Mysql`数据库的支持，涉及到的模块包括数据读取模块`VvtDataReader`、数据落地模块`VvtDataWriter`、回测框架`VvtBtCore`，`Mysql`库版本为`6.11`
...
* `demos`目录下，将`python`的demo迁移到`VvtPy`之下，然后新增一个`C++`版本demo的解决方案，提供了cta策略、高频策略、执行单元、选股策略四个demo，并提供了一个回测入口程序，用于本地调试
* 日志模块调整,主要是利用`fmtlib`优化了一些日志输出的细节
* 完成了高频策略引擎C接口导出

### 0.4.0
* 新增一个**选股调度引擎**，用于调度应用层的选股策略，得到目标组合以后，提供自动执行服务，暂时只支持日级别以上的调度周期，执行会放到第二天
* 因为新增了选股调度引擎，所以全面重构`VvtPorter`和`VvtBtPorter`导出的接口函数，以便调用的时候区分
* 新增一个**独立的执行器模块**`VvtExecMon`，并导出C接口提供服务。主要是剥离了策略引擎逻辑，提供单纯的执行服务，方便作为单纯的执行通道，嫁接到其他框架之下
